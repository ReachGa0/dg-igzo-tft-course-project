...
## ADR-092：R04 33/36 锚点失败关闭 G3

- 日期：2026-08-04
- 状态：R04 在静态报告登记提交 `017b295` 上唯一运行并返回 33/36、E0/FAIL；报告 SHA-256 为 `18ea1e1afc3327170667ea4ec0b80871217b8037f7a7cbe9dfb9a970274ca8f4`，独立 checker 未运行。
- 四个登记开源进程全部返回 0，原生轴、完整两路线表、108 行指标、两份差异表、两图和 26 个产物哈希均通过。因此 R03 的输出轴合同缺陷已被 R04 克服，当前失败不能归因于工具、收敛、parser 或路线不一致。
- 冻结锚点两路线 VOH 约 `0.0912 V`、VOL 约 `0.0206 V`、最大增益约 `0.884`，0 个单位增益交点；瞬态输出约 `0.022--0.057 V` 且无 rise/fall crossing。静态和瞬态锚点验收失败，最终完成门派生失败。
- 决定接受该 33/36 为真实阶段门失败，完整保留所有原始/处理结果，不换最佳案例、不放宽阈值、不重跑 R04，也不运行 PASS-only 独立 checker。C00 未关闭，C01/C02/C03、版图、PEX 和 HZO 保持关闭。
- 首次失败态项目检查的 17/767 scope 白名单失败以 `62be137b...c8c8` 归档；修正仅扩展历史 scope allowlist 并绑定归档。任何后续恢复都必须建立单独的新合同，明确设计变量与验收口径并另行评审。

## ADR-091：R04 50/50 静态 PASS 登记推送后才运行电路

- 日期：2026-08-04
- 状态：R04 静态合同在推送同步实现提交 `fa04d29` 上唯一返回 50/50 PASS、E3；runner 和独立 checker 未运行。
- 报告 `results/reports/c00_active_load_inverter_contract_r04.json` SHA-256 为 `1ec066117a2ff7c6837a1da603c8519c66ad760196f96f9d1209030f29c3fd56`，记录 `HEAD=origin/main=fa04d296221e05544c708f21a5089d292a4c49aa`、0 模拟器进程和 0 电路网表。
- 50 项通过只关闭 R03 失败与 16 原生产物绑定、重复 TIME 处理、原生轴覆盖、状态边界选择和不变电路合同的静态结构门。它不表示锚点、VTC、增益、噪声容限、瞬态 crossing、延迟或功耗通过。
- 决定遵循“已推送实现 -> 唯一静态报告 -> 已推送报告哈希登记 -> 唯一 runner”的顺序。runner 启动前必须确认新 `HEAD=origin/main` 已同步且不同于 `fa04d29`，R04 报告哈希和五源哈希不变，运行路径完全不存在。
- runner 只允许两个 ngspice 和两个 GPL Xyce 严格串行进程。任何 FAIL 必须保留全部已生成网表、日志、manifest、raw/表/图和报告；不得改锚点、阈值或后验选例，不得开启 C01+、版图、PEX 或 HZO。

## ADR-090：R04 用原生轴覆盖合同替代原生点数门

- 日期：2026-08-04
- 状态：R04 静态合同实现完成，`contract_implemented/E0`；50 项静态 checker、36 项 runner 和 29 项独立 checker 均未运行。
- R03 失败保留提交 `77d611e` 已推送同步。R04 新命名空间哈希绑定 R03 五个源文件、50/50 静态报告、24/36 runner 报告和 16 个原生产物；R03 不修改、不重跑。
- Xyce 在 `.PRINT TRAN` 中隐式输出 `TIME`，因此 R04 不再显式请求该列。parser 仅当重复 TIME 列数值逐行相同时折叠；重复的其他列或不同的 TIME 列必须失败。
- 原生数据的必要条件是：所有向量具有同一物理行数、数值有限、自变量轴严格递增且覆盖登记端点。原生自适应点数无需等于后处理网格；只有插值后的每案例 601 行、5 ns 网格保持冻结。
- runner 必须根据实际状态选择证据边界。只有 36/36 才能使用成功叙述；任一登记检查失败都使用失败边界，并以 `AcceptanceFailure` 或实际异常类型记录原因。
- 2-TFT 端口、portable IGZO 候选、18/36 案例、锚点、提取、验收阈值、工具哈希和四串行进程预算全部不变。先提交推送实现，再唯一运行零进程静态合同；静态报告登记推送前不运行电路。

## ADR-089：保留 C00 R03 四进程失败，R04 只修正输出轴合同

- 日期：2026-08-04
- 状态：R03 runner 唯一返回 24/36、E0/FAIL；16 个原始产物已生成并待本登记提交，独立 checker 未运行，R03 不重跑。
- 静态 PASS 登记提交 `d0f6ef7` 推送同步后，四个严格串行进程均返回 0：两个 ngspice、两个 GPL Xyce。报告 `results/reports/c00_active_load_inverter_r03.json` SHA-256 为 `0527fdc5aed413b9cbda065ee1ded34dca705c9a6c0f2cf8865ea6869a82766c`；首个失败是 `raw:four_native_tables_parse`，计数 101/632/101/230，11 个后续门未到达。
- 失败根因是 R03 runner 合同而非工具退出：Xyce `.PRN` 同时出现隐式和显式 TIME，115 个物理行被 parser 计成 230；R03 又要求原生点数至少 601，和后续固定注册网格插值不一致。16 个网表、日志、command manifest 和 raw/PRN 哈希必须原样保留；归一化表、指标、图片和独立报告缺失。
- R03 runner 报告失败分支继承 future-PASS 的边界字符串，因此机器登记将其标为不可接受；不能把该字符串或四个返回码 0 解释成完整电路结果。只读内存诊断也不构成正式锚点或阈值结果。
- 决定建立 R04 新命名空间，只允许修正 Xyce 隐式 TIME 解析、原生轴覆盖判定和失败状态边界文本；R03 的材料范围、拓扑、输入、锚点、提取、阈值、工具路线、四进程预算和失败证据不可改写。R04 静态门通过前不重新运行任何模拟器。

## ADR-088：R03 50/50 静态 PASS 先登记推送再开放四进程 runner

- 日期：2026-08-04
- 状态：R03 静态合同唯一返回 50/50 PASS、E3；报告与 ready 状态待本提交推送，runner 和独立 checker 未运行。
- 实现提交 `d449b2d` 推送同步后唯一执行静态 checker。报告 `results/reports/c00_active_load_inverter_contract_r03.json` SHA-256 为 `0e3d20a0fa4f40088b6370f171ebdfcdfab30783c344a1484e7b66061f441b39`，记录 `HEAD=origin/main=d449b2dadf51fc2b675fb954b9d620e3e52317a9`、0 个模拟器进程和 0 个电路网表。
- 50 项通过只确认 R02 五源/报告/blocker/项目失败不可变绑定、同步 Git 快照加后续登记哈希的非自引用次序，以及不变的 2-TFT、18/36 案例、锚点、提取、阈值、50/36/29 门、两路线、四进程预算与失败保留。它不是 VTC、瞬态、功耗、物理参数、实验校准、原生 Level 15/61、方程身份或下游证据。
- 静态登记阶段的三次 765/766 项目检查失败分别来自：误开 C00 M01 planning-only 入口、预期结果路径与实际时间顺序不一致、第二份失败记录误插入冻结 R02 block。三份归档哈希为 `e71f0678...4505`、`a8a0438d...6045`、`fd32160b...7b77`，均记录 0 个模拟器进程；修正仅限机器登记和项目检查器，不改变合同、结果、输入、阈值或预算。
- 决定保持“已推送实现提交 -> 唯一静态报告 -> 已推送报告登记提交 -> 唯一 runner”的顺序。只有本登记提交与 `origin/main` 同步，且新 HEAD 不等于静态报告中的实施快照时，才允许恰好四个串行 ngspice/Xyce 进程；失败必须保留，C01+、版图、PEX、SnO 与 HZO 仍关闭。

## ADR-087：R03 用同步报告快照替代 tracked 提交哈希自引用

- 日期：2026-08-03
- 状态：R03 合同实现完成，E0；50 项静态合同、36 项 runner 和 29 项独立 checker 均未运行。
- R03 使用独立配置、五个源码和全部输出命名空间，哈希绑定 R02 实现提交 `216c6a7`、保留提交 `747f384`、R02 五源、50/50 E3 报告 `e820af3b...1a99`、runner 阻塞报告 `9c810623...6ea51` 及项目检查失败 `e6fb9087...f4c6`。R02 不修改、不重跑。
- 静态 checker 只能在本地 HEAD 与 origin/main 同步时通过，并把二者写入静态报告。未来 runner 不要求任何 tracked 值等于包含它的提交哈希；它要求当前 HEAD/origin 同步、已登记静态报告哈希正确、报告内实施提交快照同步且不同于后续登记提交。runner 报告同样记录同步快照，未来独立 checker 使用报告哈希和该快照复核已发布状态。
- 该规则形成“已推送实施提交 -> 静态报告 -> 已推送报告登记提交 -> runner”的可验证顺序，不引入 Git 子进程，也不计入四个模拟器进程预算。2-TFT 输入、token-safe 范围、18/36 案例、锚点、提取、阈值、50/36/29 门、两路线、四进程串行预算和失败保留均与 R02 相同。
- 当前 `contract_implemented/E0` 只证明代码和边界存在；R03 静态报告、运行目录、网表、数值表、图片和独立报告均不存在。先提交推送实现，再唯一运行静态 checker；50/50 E3 及报告哈希另行提交推送前不得执行电路、C01+、版图、PEX、SnO 或 HZO。
...
- 日期：2026-08-03
- 状态：R02 合同实现完成，E0；50 项静态合同、36 项 runner 和 29 项独立 checker 均未运行。
- R02 使用独立配置、五个源码和全部输出命名空间，哈希绑定 R01 实现提交 `4097b9d`、失败登记提交 `60cdcbc`、R01 五源和 46/48 报告 `53f408e...a08493`。R01 状态与报告继续为 E0/FAIL且不得重跑。
- 唯一语义修正是把禁止词检查从任意子串改为按 `[A-Za-z_][A-Za-z0-9_]*` 提取 ASCII 标识符后的不区分大小写精确相等。静态门新增 R01 不可变失败绑定和仅修正范围两项，因此从 48 增为 50；runner 36 和独立 checker 29 不变。
- 2-TFT 端口、IGZO portable 候选、18 个 DC/36 个瞬态案例、锚点 `v200_t100_r0125_c1000`、提取、验收阈值、ngspice/GPL-Xyce 路线、四进程串行预算和失败保留全部与 R01 相同。版本化 cell/output 名称只用于隔离，不改变电路输入。
- 本实现不是 50/50 静态 PASS，更不是网表执行、VTC、瞬态、功耗、噪声容限、物理参数、实验校准、原生 Level 15/61 或下游证据。先提交推送实现，再唯一运行 R02 静态 checker；静态 PASS 另行提交前不得启动 runner、C01+、版图、PEX、SnO 或 HZO。

## ADR-084：C00 R01 静态词法失败保留，R02 只修正 token 匹配

- 日期：2026-08-03
- 状态：R01 唯一静态执行 46/48、E0/FAIL；runner 与独立 checker 未运行。
- 实现提交 `4097b9d` 推送同步后运行 48 项零进程静态门。唯一主失败 `netlist:forbidden_scopes_absent` 是普通子串规则把禁止词 `nor` 匹配到 DC 控制节点 `NORM`；最终汇总门连带失败。报告 SHA-256 为 `53f408e870e5f84993dd635cf95086c9b4db263ea5b8bead6cc2f5ed44a08493`。
- 其余 46 项通过；报告明确记录 0 个模拟器进程、0 个正式电路网表，C00 运行目录和数值输出均不存在。因此这不是拓扑、模型、锚点、阈值、收敛或电路性能失败。
- R01 配置、源码与报告冻结且不重跑。R02 必须使用新配置/源码/输出命名空间，绑定本失败，只将禁止范围扫描改为 ASCII 标识符整词比较；不得改变 2-TFT 端口、18/36 案例、锚点、提取、验收阈值、四进程预算或证据边界。
- R02 静态 PASS 状态提交前，runner、C01+、版图、PEX、SnO 和 HZO 全部关闭。

## ADR-083：C00 R01 先冻结 2-TFT 两路线合同，静态提交门先于电路执行

- 日期：2026-08-03
- 状态：已实现，E0；48 项静态合同、36 项 runner 和 29 项独立 checker 均未运行。
- 反相器固定为两个 `IGZO_DG_BEHAVIORAL_R03_PORTABLE`：驱动端口 `D/TG/BG/S=VOUT/VIN/0/0`，负载端口 `VDD/V_TOP_LOAD/VDD/VOUT`。驱动宽度 60 um、沟道长 10 um，负载宽度按 `Wload/Wdriver` 计算；正式路径不使用电阻负载。
- P6 小网格冻结 `VDD=0.1/0.2 V`、负载顶栅比例 `0.5/0.75/1.0`、尺寸比 `0.125/0.25/0.5` 和 `Cload=0.5/1 pF`，共 18 个 DC、36 个瞬态计划案例。锚点预先固定为 `v200_t100_r0125_c1000`，不得按运行结果替换。
- 提取冻结 VTC、VOH/VOL、VM、最大增益、单位增益交点、噪声裕量、端点静态功耗、双边 50% 延迟和第二周期动态功耗。两个路线都必须让锚点通过登记门；路线差异完整落盘但只作诊断，不是可调 PASS 阈值。
- 未来执行预算为 ngspice DC/瞬态与 GPL Xyce DC/瞬态各一份网表和进程，四进程严格串行；失败网表、日志、raw、部分表和报告均排他保留。AIM-Spice、TCAD、SnO、HZO、C01+、版图和 PEX 均关闭。
- T02 entry evidence 按机器事实登记为 `bidirectional_verified/E2`，不提升为 E3。当前实现仅证明合同代码存在，不是 48/48 静态 PASS，更不是电路仿真、物理参数、实验校准或下游证据。先提交推送实现，再唯一运行静态 checker；静态 PASS 另行提交前不得生成或执行正式电路网表。
