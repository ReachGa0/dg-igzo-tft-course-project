...
- 最后更新：2026-08-01
- 当前阶段：`T03_P2_DIT_EQUATION_SMOKE_E3_PARTIAL_P2`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门已在冻结 E2 教学模型上关闭；T03-P4-L 和数值 P1 组均已完整关闭。P2 只完成有文献来源的 DIT 输入合同与准静态界面方程冒烟，仍为 partial；正式三点转移扫描和 bulk traps 未运行。T02/T03 的 VTH、Delta VTH、gm、耦合斜率和电流均只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图、T02-A/B/C、T03-P4-L、完整数值 P1 和 T03-P2-DIT 方程冒烟证据包已通过 369 项项目检查。
...
- [x] T03-P2-DIT 已以 22 项静态合同冻结四行 E1 文献来源、bottom 单界面、`Q_it=-q*D_it*(Potential-Psi_neutral)`、`Psi_neutral=0 V` 教学假设和未来 `8.43e11/3.07e12/6.02e12 cm^-2 eV^-1` 三点；合同检查不运行器件仿真。
...
- [x] P2 只标为 partial：上述 E3 证明输入/方程和落盘证据可复算，不证明正式 DIT 敏感性、SS/VTH/Ioff 趋势、bulk tail/deep traps、动态捕获-发射、迟滞、实验校准或完整 T03。

## 后续领域实现

- [x] T01 单栅 IGZO 漂移扩散（教学参数 E2 数值门完成；不等同实验标定或物理参数验证）。
- [x] T02 双栅电流与阈值耦合（冻结教学模型数值门完成；不等同实验标定、物理电容比、迟滞或紧凑模型验证）。
- [ ] T03 五组器件参数分析（部分完成：P1 DONE、P4 DONE、P2 DIT contract/equation smoke DONE；P2 formal/bulk traps、P3、P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P2-DIT 当前只验证单个代表 `D_it=3.07e12 cm^-2 eV^-1` 的准静态 bottom-interface 方程和零陷阱极限。0.7787 的代表/零陷阱电流比只是冒烟响应方向，不是正式扫描趋势、物理 Ioff/Ion、实测 DIT 或陷阱参数标定。
...
只进入另立的 `T03-P2-DIT-FORMAL` 最小三点 transfer sensitivity：严格使用已冻结的 `8.43e11/3.07e12/6.02e12 cm^-2 eV^-1`、共同偏压/提取口径和独立验收，输出 SS/VTH/Ioff 等数值代理及边界。该三点门通过前不启动 bulk traps、P3/P5、SPICE 或版图。
