...
- 最后更新：2026-08-02
- 当前阶段：`M00_R01_HOLDOUT_GM_FAIL_DECISION_REQUIRED`
- 整体状态：`YELLOW`
- 当前原则：M00 R01 已按提交前合同唯一运行一次。优化器正常终止，train/holdout 聚合与逐曲线线性/对数误差、VTH、零漏压、单调和参数边界均通过，但 `L=12 um` holdout 的 gm 相对误差 `0.512384` 超过冻结上限 `0.50`，因此运行器 21/24、E0/FAIL。失败证据完整保留，PASS-only 独立检查未运行，候选模型未生成。不得重跑 R01、放宽门槛或用 holdout 调参；R02 研究路线明确前 M01、SPICE、电路、版图、PEX 和 HZO 继续关闭
...
- [ ] M00/M01 紧凑模型拟合与双轨对照（M00 合同 E3；R01 正式拟合 E0/FAIL，M01 关闭）。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
5. M00 R01 的 `L=12 um` holdout gm 相对误差 `0.512384` 超过冻结 `0.50`，阶段 DAG 已停在模型门；R02 必须先有不利用该 holdout 调参的研究依据和新合同。

## 下一步

先提交并推送 M00 R01 E0/FAIL 证据，然后在以下研究路线间作出明确决定：保持 R01 失败并等待新的合格数据，或先建立不改变原 split/阈值、且不利用 R01 holdout 选择参数的新 R02 模型结构合同。决定前不得重跑 R01、运行独立 PASS-only 检查、M01、SPICE、电路、版图、PEX 或 HZO。
