...
- 下一步：提交并推送 R08 合同实施；随后唯一运行 36 项静态合同。只有静态 PASS 状态再次提交后，才允许唯一运行 R08 工具/parser runner；R07 及其独立检查不重跑，正式 M01 器件 DC 和下游继续关闭。
