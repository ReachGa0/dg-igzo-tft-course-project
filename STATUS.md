...
- 最后更新：2026-07-31
- 当前阶段：`T02_A_LIMIT_REGRESSION_E2`
- 整体状态：`YELLOW`
- 当前原则：T01 完整数值门与 T02-A 顶栅极限回归已通过；禁用模式必须移除整个顶栈并返回 T01，`VTG=0 V` 仍是有限电容耦合。G0 继续禁止实验拟合、物理 Ion/Ioff 和定量双栅预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图和 T02-A 证据包已通过 248 项项目检查。
...
- [ ] T02 双栅电流与阈值耦合（T02-A 极限回归已完成；非零顶栅曲线族、Delta VTH 和 gm 待完成）。
- [ ] T03 五组器件参数分析。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
进入 T02-B 最小非零双栅偏压族：在已冻结的对称顶栈上，只改变顶栅电压并保持 VDS、底栅、几何、缺陷、接触和温度不变，先验证电流/内部状态方向和守恒。T02-B 通过前不启动完整双向曲线族、T03、SPICE 或版图。
