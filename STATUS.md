...
- 最后更新：2026-08-02
- 当前阶段：`T03_P3_CONTACT_V2_E3_COMPLETE_P5_CONTRACT_NEXT`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门及 T03-P1/P2/P3/P4 已在冻结教学模型边界内关闭。P3 V1 的 12 器件、243 次收敛 DC、156 点、3 状态和 18 VTK 仍因零漏压相对守恒适用域错误以 24/25 保持 E0/FAIL，且未运行独立检查；全部失败证据继续保留。V2 先以 34/34 E3 静态合同保持三点、物理输入、偏压、提取、阈值和预算不变，再唯一运行一次并完成相同 12/243/156/3/18 规模，运行器 25/25 PASS、E2，独立落盘检查 20/20 PASS、E3。P3 因此只在二维、准静态、未校准的对称 lumped series-resistance 教学代理边界内关闭；完整 T03 仍缺 P5。所有接触电阻和电流仍只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
只建立正式隔离 T03-P5 温度敏感性输入合同。先冻结至少三个温度点、随温度变化与保持不变的教学模型项、共同偏压与提取方法、失败保留、输出路径、资源预算和证据边界；合同通过并提交前不得运行 P5 DEVSIM。M00/M01、SPICE、电路、版图、PEX 和 HZO 继续关闭。
