...
- 最后更新：2026-08-02
- 当前阶段：`T03_P2_BULK_TRAPS_FORMAL_V2_FAILED_E0_PARTIAL_P2`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门已在冻结 E2 教学模型上关闭；T03-P4-L 和数值 P1 组均已完整关闭。P2 的 bottom-interface DIT 子阶段已达 E3；bulk-trap 的 30/30 静态合同、三案例方程冒烟运行器与独立落盘检查已通过。正式 V1 完成 8 器件/328 次收敛 DC/248 点但保持 E0/FAIL，证据已完整保留。V2 合同 23/23 E3 静态 PASS 后，正式 V2 完成 8 器件、440 次收敛 DC、360 点、8 状态和 48 VTK；最高 NTA 已包络 VTH，但诊断 VTH=1.4666676 V 对应的 `VTH+0.2 V` gm 评价点 1.6666676 V 超出 1.7 V 网格可做中心差分的范围，因此运行器仍为 E0/FAIL。V2 失败包已保留且没有运行独立检查，完整 P2 仍为 partial，阶段 DAG 暂停等待恢复决策。T02/T03 的 VTH、Delta VTH、SS、gm、耦合斜率和电流均只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图、T02-A/B/C、T03-P4-L、完整数值 P1、T03-P2-DIT 界面证据包、bulk-trap 静态合同、方程冒烟、V1/V2 失败保留和 V2 正式隔离合同已通过 444 项项目检查。
...
- [x] P2 继续标为 partial：E3 正式结果只关闭线性化 bottom-interface DIT 子阶段，不证明 bulk tail/deep traps、能量分布占据、动态捕获-发射、迟滞、实验校准或完整 T03。
...
- [ ] T03 五组器件参数分析（部分完成：P1 DONE、P4 DONE、P2 interface DIT substage DONE、P2 bulk equation smoke DONE、P2 bulk formal V1/V2 failures preserved、V2 contract DONE；正式 bulk 恢复决策与通过结果、P3、P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P2-BULK-TRAPS 正式 V1 的 8/328/248 和 V2 的 8/440/360 都是已计算的 E0/FAIL 证据，不是阶段 PASS。V2 已包络 VTH 但 gm 中心差分网格不足；未运行独立检查，也没有正式指标或图。两次失败都不是物理 DOS、SS/VTH/Ion/Ioff、动态捕获-发射或完整 P2 证据。
- 尚未生成的有源负载网表、标准单元版图和 LVS 报告。

## 当前阻塞

1. T03-P2 bulk 正式 V2 阶段门失败：最高 NTA 的 `VTH+0.2 V` gm 评价点缺少中心差分上邻点。按自动执行规则暂停，等待是否建立统一扩展网格的恢复合同这一明确决定。
2. 主 IGZO 原始 Id-Vg/Id-Vd、批次和完整条件尚待老师确认；这阻止定量拟合，但不阻止教学参数 T01。
3. 双栅实测数据缺失时只能做文献约束敏感性。
4. 需要老师确认 DEVSIM、单极性有源负载逻辑和 KLayout 几何 LVS 的接受度。
5. 复杂电路跨线层和顶栅实际工艺尚未明确。

## 下一步

暂停在 V2 失败门并等待恢复决定。若批准恢复，必须先另立合同，统一扩展所有 8 个器件的栅压网格并保持 NTA/NGA 点、方程、提取方法和阈值不变；合同通过后才允许完整重跑，运行器 PASS 后才运行独立检查。此前不启动 P3/P5、M00/M01、SPICE、电路、版图、PEX 或 HZO。
