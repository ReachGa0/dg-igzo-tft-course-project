...
- 最后更新：2026-07-30
- 当前阶段：`S00_DATA_AUDIT_COMPLETE`
- 整体状态：`YELLOW`
- 当前原则：S00 审计已完成；G0 仅允许教学参数 T01，不允许实验拟合或定量双栅预测
...
从 T01 的一个最小单栅漂移扩散案例开始：使用 `IGZO_T01_TEACHING_BASELINE_V1`，明确记录未提供的温度、接触和原始 I-V 条件，不并行铺开大电路。收到条件完整原始数据后，再修订 G0 和启动定量标定。
