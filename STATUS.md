...
- 最后更新：2026-08-02
- 当前阶段：`T03_P2_BULK_TRAPS_FORMAL_V3_CONTRACT_PASS_E3_NOT_RUN_PARTIAL_P2`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门已在冻结 E2 教学模型上关闭；T03-P4-L 和数值 P1 组均已完整关闭。P2 的 bottom-interface DIT 子阶段已达 E3；bulk-trap 的 30/30 静态合同、三案例方程冒烟运行器与独立落盘检查已通过。正式 V1/V2 分别完成 8/328/248 和 8/440/360，但均保持 E0/FAIL，精确输入、运行器和全部失败证据已保留。用户批准后建立 V3：只把全部 8 个器件的共同栅压上限从 1.7 V 扩展到 1.8 V，密度点、方程、提取方法和阈值不变；24/24 静态合同 PASS、E3、`NOT_RUN_BY_CONTRACT_CHECK`，冻结 8 器件、456 次计划 DC 和 376 个计划点。完整 P2 仍为 partial，下一门仅为 V3 正式运行。T02/T03 的 VTH、Delta VTH、SS、gm、耦合斜率和电流均只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图、T02-A/B/C、T03-P4-L、完整数值 P1、T03-P2-DIT 界面证据包、bulk-trap 静态合同、方程冒烟、V1/V2 失败保留和 V3 正式隔离合同已通过 449 项项目检查。
...
- [ ] T03 五组器件参数分析（部分完成：P1 DONE、P4 DONE、P2 interface DIT substage DONE、P2 bulk equation smoke DONE、P2 bulk formal V1/V2 failures preserved、V3 contract DONE；正式 V3 运行与独立通过结果、P3、P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P2-BULK-TRAPS 正式 V1 的 8/328/248 和 V2 的 8/440/360 都是已计算的 E0/FAIL 证据，不是阶段 PASS。V3 的 8/456/376 只是静态合同冻结的计划量，尚未运行。三者都不是物理 DOS、SS/VTH/Ion/Ioff、动态捕获-发射或完整 P2 证据。
- 尚未生成的有源负载网表、标准单元版图和 LVS 报告。

## 当前阻塞

1. T03-P2 bulk 正式 V3 只完成静态合同，尚未运行；正式敏感性与独立落盘门仍未关闭。
...
只运行 V3 合同定义的正式隔离 NTA/NGA transfer sensitivity；只有运行器 PASS 后才运行独立落盘检查。V3 两门都通过并完成 P2 边界记录前，不启动 P3/P5、M00/M01、SPICE、电路、版图、PEX 或 HZO。
