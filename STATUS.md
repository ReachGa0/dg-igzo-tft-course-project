...
- 最后更新：2026-08-03
- 当前阶段：`M00_R02_STRUCTURE_CONTRACT_E3_RUNNER_NEXT`
- 整体状态：`YELLOW`
- 当前原则：M00 R01 保持 21/24、E0/FAIL且全部证据不可改写。R02 已以 27/27 静态合同 PASS、E3 冻结唯一结构变化：因训练侧只有一个非参考长度条件却有两个长度自由度，固定教学核一阶几何因子为 `Lref/L`、移除 `length_exponent`，只保留一个训练拟合长度系数；原 9/163 train、4/70 holdout、全部指标及 `0.50` gm 门不变。该合同没有运行拟合或仿真；下一步只实现并自测 R02 执行链，提交前不得正式优化，M01 和下游继续关闭
...
- [ ] M00/M01 紧凑模型拟合与双轨对照（R01 正式拟合 E0/FAIL；R02 结构合同 E3、拟合 E0 未运行；M01 关闭）。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- M00 R02 的 27/27 只证明结构恢复合同静态一致；`Lref/L` 固定指数、10 个计划系数和独立输出路径尚未经过正式拟合或 holdout 评分，不能写成 R02/M00 PASS、物理缩放规律或电路模型。
...
5. M00 R01 的 `L=12 um` holdout gm 相对误差 `0.512384` 超过冻结 `0.50`；R02 静态合同已经建立，但版本化运行器、独立检查器和正式结果均尚未实现，M00 仍未关闭。

## 下一步

先提交并推送 R02 结构恢复合同。随后只实现版本化 R02 正式运行器与不导入运行器/SciPy 的独立检查器，并完成合成自测；执行链提交推送前不运行正式数据优化。之后 R02 只允许一次正式运行，runner PASS 后才执行独立落盘检查。不得重跑 R01、改变原 split/阈值或启动 M01、SPICE、电路、版图、PEX、HZO。
