...
- 最后更新：2026-08-02
- 当前阶段：`T03_P5_TEMPERATURE_CONTRACT_E3_READY_FORMAL_RUN_NEXT`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门及 T03-P1/P2/P3/P4 已在冻结教学模型边界内关闭，P3 V1 失败继续保留。T03-P5 正式合同已冻结 250/300/350 K，唯一随温度改变的实现项是既有 Scharfetter-Gummel 电子电流中的 `V_t=k_B*T`；迁移率保持 35.5 cm2/(V*s)，DOS、能带、介电、接触、陷阱、几何和偏压均不变。23/23 静态检查为 E3 且明确 `NOT_RUN_BY_CONTRACT_CHECK`，因此 3 器件、123 次 DC、93 点、3 状态和 18 VTK 都仍是计划量。完整 T03 仍缺正式 P5 运行和独立检查。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] P5 合同预注册 3 个新器件、123 次计划 DC、93 个计划 transfer 点、3 个计划 `VTG=1 V` 状态、18 个计划 VTK、300 K 对 T02-C 回归、失败拒绝覆盖和独立落盘复核；23/23 静态检查 PASS、E3、`NOT_RUN_BY_CONTRACT_CHECK`，本阶段未运行 DEVSIM。

## 后续领域实现

- [x] T01 单栅 IGZO 漂移扩散（教学参数 E2 数值门完成；不等同实验标定或物理参数验证）。
- [x] T02 双栅电流与阈值耦合（冻结教学模型数值门完成；不等同实验标定、物理电容比、迟滞或紧凑模型验证）。
- [ ] T03 五组器件参数分析（部分完成：P1/P2/P3/P4 DONE；P3 V1 FAIL 保留且 V2 E2/E3 PASS；P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P5 的 23/23 PASS 只是静态合同证据，不是温度器件仿真。250/300/350 K 是项目规定的三点教学范围，`V_t` 是唯一温度项；不能据此声称 IGZO 迁移率温度律、激活能、物理 SS/VTH/漏电、工作温区、自热或实验校准。
...
在本合同提交并推送后，只运行一次完整正式隔离 T03-P5 温度敏感性。只有运行器 PASS 才执行不导入运行器或 DEVSIM 的独立落盘检查；任一门失败即保留全部证据并停在 P5。M00/M01、SPICE、电路、版图、PEX 和 HZO 继续关闭。
