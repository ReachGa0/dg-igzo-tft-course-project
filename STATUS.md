...
- 最后更新：2026-08-03
- 当前阶段：`M01_PREFLIGHT_CHAIN_READY_NO_NUMERICAL_EXECUTION`
- 整体状态：`YELLOW`
- 当前原则：M00 R01 保持 21/24、E0/FAIL且全部证据不可改写。R02 的 27/27 静态合同冻结固定 `Lref/L`、10 个参数及不变的 9/163 train、4/70 holdout、全部指标和 `0.50` gm 门；唯一正式运行的 runner 为 24/24、E2，独立落盘复核为 20/20、E3。M00 仅在冻结、结构正则化的 IGZO 教学数值曲线和局部有效域内关闭。M01 revision-3 双仿真器合同已 32/32、E3 静态通过，v1/v2 失败保留。用户随后披露本机 AIM-Spice 副本未获授权，因此它不得进入正式证据链；R01 工具/来源预检执行链已实现但尚未运行，证据仍为 E0。该链只允许一次 `ngspice --version`，对 AIM-Spice 只读文件指纹且禁止启动，禁止任何器件网表和数值曲线。提交推送后先保留预期的工具/来源门失败，再另建开源第二路线恢复合同；M01 数值执行、电路、版图、PEX 和 HZO 继续关闭。
...
- [x] M01 R01 工具/来源预检配置、运行器和 Make 入口已实现但未运行：13 项预检绑定已提交 revision-3 合同和两个冻结 IGZO 候选，拒绝覆盖，唯一允许的子进程是 `ngspice --version`；AIM-Spice 仅作静态文件指纹读取，授权来源不可审计和批处理入口未建立两门预注册为失败。此前在用户披露前仅探索过帮助/版本参数，没有传入网表或生成数值结果，也不得作为正式证据。
...
- [ ] M00/M01 紧凑模型拟合与双轨对照（R01 正式拟合保持 E0/FAIL；R02 runner 24/24 E2、独立检查 20/20 E3，M00 仅在教学数值域内关闭；M01 合同 32/32 E3，工具/来源预检链 E0 已实现未运行，器件级双路线尚未运行）。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- M01 R01 预检配置和运行器的存在不是仿真结果。用户披露的未授权 AIM-Spice 副本不得作为工具资格、Level 15 数值证据或第二路线通过；开源候选工具在独立合同、提交和正式运行前也不得写成 M01 结果。
...
5. M00 R01 的 `L=12 um` holdout gm 相对误差 `0.512384` 超过冻结 `0.50`，该失败永久保留；M01 原合同已冻结，但本机 AIM-Spice 来源不可审计且可复现 batch/CLI 未建立，原第二路线预期在工具/来源预检失败。该失败必须先落盘，开源替代路线另行冻结前不得运行器件网表。

## 下一步

先提交并推送 M01 R01 工具/来源预检执行链；随后只运行一次该预检，预期保留 AIM-Spice 授权来源和 batch/CLI 两项失败，且不传入任何器件网表。失败收口提交后，再建立开源第二仿真器恢复合同，冻结工具来源、版本、候选映射、语法、输出和证据边界。不得重跑 R01/R02、改变原 split/阈值，或启动 M01 数值对照、C00、电路、版图、PEX、HZO。
