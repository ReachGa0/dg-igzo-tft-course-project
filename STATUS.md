...
- 最后更新：2026-08-01
- 当前阶段：`T03_P4_L_CHANNEL_LENGTH_E3`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门已在冻结 E2 教学模型上关闭；T03-P4-L 已完成一个 E3 独立复核的沟道长度敏感性组，理想 1/L 诊断 FAIL 但不是完成门。T02/T03 的 VTH、Delta VTH、gm 和电流均只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图、T02-A/B/C 证据包和 T03-P4-L 证据包已通过 312 项项目检查。
...
- [ ] T03 五组器件参数分析（部分完成：P4-L DONE；P1/P2/P3/P5 未实现）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
为 P1/P2/P3/P5 中的一个组冻结新的最小单变量合同，至少三个点，沿用共同偏压与提取口径，只改变一个物理变量；合同通过后只运行该组，不同时铺开其他组，也不启动 SPICE 或版图。
