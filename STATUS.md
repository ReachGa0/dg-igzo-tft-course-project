...
- 最后更新：2026-08-03
- 当前阶段：`M01_CONTRACT_E3_NO_SIM_EXECUTION`
- 整体状态：`YELLOW`
- 当前原则：M00 R01 保持 21/24、E0/FAIL且全部证据不可改写。R02 的 27/27 静态合同冻结固定 `Lref/L`、10 个参数及不变的 9/163 train、4/70 holdout、全部指标和 `0.50` gm 门；唯一正式运行的 runner 为 24/24、E2，独立落盘复核为 20/20、E3。M00 仅在冻结、结构正则化的 IGZO 教学数值曲线和局部有效域内关闭；两个近下界系数是诊断，ngspice/AIM-Spice 候选只生成未执行。M01 revision-3 双仿真器对照合同已 32/32、E3 静态通过，合同没有调用模拟器；v1/v2 合同失败报告保留。下一步只做合同声明的工具预检和两路器件级 DC，对照未通过前电路、版图、PEX 和 HZO 继续关闭。
...
- [ ] M00/M01 紧凑模型拟合与双轨对照（R01 正式拟合保持 E0/FAIL；R02 runner 24/24 E2、独立检查 20/20 E3，M00 仅在教学数值域内关闭；M01 合同 32/32 E3，双仿真器尚未运行）。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
5. M00 R01 的 `L=12 um` holdout gm 相对误差 `0.512384` 超过冻结 `0.50`，该失败永久保留；M01 合同已冻结，但 AIM-Spice 可复现 batch/CLI 入口和两路正式输出尚未预检/运行。

## 下一步

先提交并推送 M01 revision-3 合同及两份历史失败归档；随后只做声明的工具/语法预检，再按 247 个冻结行运行两条器件级路线。任何工具或路线失败均保留并停止；不得重跑 R01/R02、改变原 split/阈值或启动电路、版图、PEX、HZO。
