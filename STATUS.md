...
- 最后更新：2026-08-01
- 当前阶段：`T03_P1_BIAS_E3_PARTIAL_P1`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门已在冻结 E2 教学模型上关闭；T03-P4-L 已完整关闭，T03-P1-BIAS 也已完成 E3 独立复核，但 P1 仍缺少单独的上下栅电容比子阶段。T02/T03 的 VTH、Delta VTH、gm、耦合斜率和电流均只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图、T02-A/B/C、T03-P4-L 和 T03-P1-BIAS 证据包已通过 332 项项目检查。
...
- [x] T03-P1-BIAS 只关闭固定副栅偏压子阶段；对称 `Ctop/Cbottom=1` 是未扫描的几何输入代理，不是物理电容比提取，完整 P1 和完整 T03 均未关闭。

## 后续领域实现

- [x] T01 单栅 IGZO 漂移扩散（教学参数 E2 数值门完成；不等同实验标定或物理参数验证）。
- [x] T02 双栅电流与阈值耦合（冻结教学模型数值门完成；不等同实验标定、物理电容比、迟滞或紧凑模型验证）。
- [ ] T03 五组器件参数分析（部分完成：P4-L DONE；P1-BIAS DONE 但 P1-CAP-RATIO TODO；P2/P3/P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P1-BIAS 只改变固定底栅偏压。0.600083 至 -0.155482 V 的 VTH 代理和 -0.940554 V/V 的斜率不得称为实测阈值、物理电容比、实验耦合系数或电路可用参数；本结果不完成 P1，也不替代 P1-CAP-RATIO。
...
冻结 `T03-P1-CAP-RATIO` 的独立输入合同：至少五个比值点，先明确它与 P4 几何/介质变量的归属和固定量，再只运行这一子阶段。合同通过前不运行该扫描；P1 完成前不启动 P2/P3/P5、SPICE 或版图。
