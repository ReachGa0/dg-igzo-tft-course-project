...
- 最后更新：2026-08-02
- 当前阶段：`T03_P3_CONTACT_V2_CONTRACT_E3_RUN_NEXT`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门、T03-P4-L、数值 P1/P2 已在冻结教学模型边界内关闭。P3 V1 完成 12 器件、243 次收敛 DC、156 点、3 状态和 18 VTK，但因把相对守恒门错误用于 9 个零漏压数值噪声点而以 24/25 保持 E0/FAIL；全部输入、运行器、日志、表、图、状态、报告和失败归档均保留，V1 独立检查未运行。V2 34/34 E3 静态恢复合同只澄清适用域并使用独立路径：不变的 `1e-5` 相对门用于 `VDS>0`，不变的 `1e-16 A/cm` 绝对门用于 `VDS=0`；`R_pair*W=0/0.5/4.5 kOhm*um`、物理输入、偏压、提取和 12 器件/243 DC/156 点预算均未改变，且 `simulation=NOT_RUN_BY_CONTRACT_CHECK`。完整 T03 仍缺 P3 V2 正式运行和 P5；所有接触电阻和电流仍只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] V2 静态恢复合同保持两个阈值、三个接触点、全部器件方程、偏压、提取和 12/243/156/3/18 预算不变，只将相对守恒门限定到 `external VDS>0`、绝对电流门限定到 `external VDS=0`，并改用独立 V2 输出/归档路径。34/34 PASS、E3、`NOT_RUN_BY_CONTRACT_CHECK`；P3 仍未完成。
...
- [ ] T03 五组器件参数分析（部分完成：P1/P2/P4 DONE；P3 V1 FAIL 保留、V2 CONTRACT E3、V2 RUN TODO；P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P3 V1 的 12 器件、243 DC、156 点和 3 状态是已完成但 E0/FAIL 的计算证据，不是 P3 PASS；V2 的同组数量当前仍只是 34/34 静态合同计划量。V1 失败不能因 V2 合同而改标；两者都不证明项目 TLM 提取、Ti/Ni 金属比较、势垒/注入、物理 Ion 或接触参数校准。
...
只运行一次正式 T03-P3 V2 对称串联接触电阻代理敏感性。必须使用已冻结的 `0/0.5/4.5 kOhm*um` 总源漏对称口径、12 器件/243 DC/156 点协议和不变阈值；相对守恒只在 `VDS>0` 判门，零漏压只按绝对电流判门。运行器 PASS 后才执行独立持久化证据检查。若任一门失败，保留全部产物并暂停，不进入 P5；M00/M01、SPICE、电路、版图、PEX 和 HZO 继续关闭。
