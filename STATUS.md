...
- 最后更新：2026-08-02
- 当前阶段：`T03_COMPLETE_E3_M00_CONTRACT_NEXT`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 与 T03-P1/P2/P3/P4/P5 数值门已在冻结教学模型边界内关闭，P2/P3 历史失败继续保留。P5 只改变既有 Scharfetter-Gummel 电流中的 `V_t=k_B*T`，完成 3 器件、123 次收敛 DC、93 点、3 状态和 18 VTK；运行器 14/14 E2、独立检查 15/15 E3。迁移率、DOS、能带、介电、接触、陷阱、几何和偏压未随温度变化，因此这些结果不是物理温度参数。下一步只建立 M00 教学紧凑模型合同；G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
只建立正式 M00 教学紧凑模型输入与验证合同，先冻结数据资格与禁止用途、train/holdout、模型双轨、提取/优化、线性/对数误差、输出、失败保留和证据边界。合同通过并提交前不做紧凑模型拟合或 SPICE；M01、电路、版图、PEX 和 HZO 继续关闭。
