...
- 最后更新：2026-07-31
- 当前阶段：`T02_B_MINIMAL_BIAS_E2`
- 整体状态：`YELLOW`
- 当前原则：T02-B 只验证冻结教学模型在 `VDS=0.01 V`、`VBG=0 V`、`VTG=0/0.1/0.2/0.3 V` 的正向响应。负偏压、反向扫描、双向曲线族、Delta VTH/gm 和完整 T02 仍未验证；G0 继续禁止实验拟合和定量双栅预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图和 T02-A/B 证据包已通过 265 项项目检查。
...
- [ ] T02 双栅电流与阈值耦合（T02-A/B 已完成；T02-C 双向曲线族、Delta VTH、gm 和代表状态待完成）。
- [ ] T03 五组器件参数分析。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
进入 T02-C 前先冻结双向偏压合同：分别固定顶栅扫底栅、固定底栅扫顶栅，明确负偏压与回程路径、阈值和 gm 提取口径、状态点和笔记本算力上限。合同通过后再运行 T02-C；此前不启动 T03、SPICE 或版图。
