...
- 最后更新：2026-08-02
- 当前阶段：`T03_P3_CONTACT_CONTRACT_E3_RUN_NEXT`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门、T03-P4-L、数值 P1/P2 已在冻结教学模型边界内关闭。P3 的 30/30 E3 静态合同已冻结 `R_pair*W=0/0.5/4.5 kOhm*um`、对称源漏分摊、自洽器件-电路耦合、transfer/output 偏压、线性区提取、失败保留和 12 器件/243 DC/156 点计划；合同明确 `NOT_RUN_BY_CONTRACT_CHECK`。两个非零值只来自不同器件论文的 E1 TLM 数量级，并被显式映射为项目总源漏串联电阻教学代理，不是项目测量、TLM 提取、Ti/Ni 参数或势垒/注入证据。完整 T03 仍缺 P3 正式运行和 P5；所有 VTH、SS、gm、接触电阻和电流仍只能按各合同称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] T03-P3-CONTACT-RESISTANCE 以 2 行 E1 TLM 来源冻结 `R_pair*W=0/0.5/4.5 kOhm*um` 三点；0 为精确理想接触回归，两个非零点只借用论文数值大小并按项目教学约定映射为总源漏串联电阻，再各半分到源/漏，不继承论文金属或 Rc 定义。
- [x] P3 合同冻结零点直连理想欧姆接触、非零点通过 DEVSIM circuit node 与两个外部电阻自洽耦合；势垒高度、热发射、隧穿、接触区材料/网格、陷阱、迁移率、几何和温度扫描均关闭。
- [x] P3 合同计划 3 个 transfer 器件和 9 个独立 output 器件，共 12 器件、243 次 DC、93 个 transfer 点、63 个 output 点、3 状态和 18 VTK。30/30 静态检查 PASS、E3、`NOT_RUN_BY_CONTRACT_CHECK`；这些数量不是仿真结果，P3 仍未完成。
...
- [ ] T03 五组器件参数分析（部分完成：P1/P2/P4 DONE；P3 CONTRACT E3、RUN TODO；P5 TODO）。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- T03-P3 的 12 器件、243 DC、156 点和 3 状态目前全部是静态合同计划量。30/30 PASS 不等于接触仿真、项目 TLM 提取、Ti/Ni 金属比较、势垒/注入验证、物理 Ion 或 P3 完成。
...
只实现并运行正式 T03-P3 对称串联接触电阻代理敏感性。必须使用已冻结的 `0/0.5/4.5 kOhm*um` 总源漏对称口径、12 器件/243 DC/156 点协议和预注册门槛；运行器 PASS 后才执行独立持久化证据检查。若任一门失败，保留全部产物并暂停，不进入 P5；M00/M01、SPICE、电路、版图、PEX 和 HZO 继续关闭。
