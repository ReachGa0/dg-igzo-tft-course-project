...
- 最后更新：2026-07-31
- 当前阶段：`T01_DA_MESH_REFINEMENT_VERIFIED`
- 整体状态：`YELLOW`
- 当前原则：T01-D-A 已在教学参数下完成 VDS=0.01 V、VGS=0.5/1.0 V 的界面法向网格收敛；该受限 E2 结果只打开 T01-D-B Id-Vd，完整 T01、实验拟合和定量双栅预测仍未完成
...
- [ ] T01 单栅 IGZO 漂移扩散（T01-A/B/C/D-A 已完成；T01-D-B 的完整 Id-Vd 和 T01-D-C 的状态图集合/参数提取仍未开始）。
- [ ] T02 双栅电流与阈值耦合。
- [ ] T03 五组器件参数分析。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
...
进入 T01-D-B：继承 T01-D-A 通过的界面网格口径，运行冻结的多 VGS `Id-Vd` 点并检查收敛、守恒和输出曲线趋势；随后 T01-D-C 补齐关态/中间态/开态状态图与受限参数提取。T01-D 关闭前不启动 T02、SPICE 或版图工作；收到条件完整原始数据后，再修订 G0 和启动定量标定。
