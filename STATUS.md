...
- 最后更新：2026-07-31
- 当前阶段：`T01_DB_IDVD_VERIFIED`
- 整体状态：`YELLOW`
- 当前原则：T01-D-B 已在教学参数下完成冻结的 4 条 interface_4x Id-Vd 曲线及 2 条 interface_8x 高栅压复核曲线；该受限 E2 结果只打开 T01-D-C 状态图与受限参数提取，完整 T01、实验拟合和定量双栅预测仍未完成
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同和 17 阶段依赖图已通过 196 项项目检查。
...
- [ ] T01 单栅 IGZO 漂移扩散（T01-A/B/C/D-A/D-B 已完成；T01-D-C 的状态图集合、受限参数提取与完整 T01 门仍未开始）。
- [ ] T02 双栅电流与阈值耦合。
- [ ] T03 五组器件参数分析。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
...
进入 T01-D-C：继承 T01-D-B 已验证的 interface_4x 正式网格和偏压点，补齐关态/中间态/开态状态图、明确可提取与不可提取的指标，并关闭完整 T01 阶段门。T01-D 关闭前不启动 T02、SPICE 或版图工作；收到条件完整原始数据后，再修订 G0 和启动定量标定。
