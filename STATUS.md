...
- 最后更新：2026-07-31
- 当前阶段：`T01_B_LOW_VDS_SMOKE_VERIFIED`
- 整体状态：`YELLOW`
- 当前原则：T01-B 已完成教学参数下的零偏压和 VGS=0 V、VDS<=0.01 V 漂移扩散烟雾验证；完整 T01、实验拟合和定量双栅预测仍未完成
...
- [ ] T01 单栅 IGZO 漂移扩散（T01-A/B 已完成；T01-C VGS continuation 和 T01-D 完整输出仍未开始）。
- [ ] T02 双栅电流与阈值耦合。
- [ ] T03 五组器件参数分析。
- [ ] M00/M01 紧凑模型标定与双轨对照。
- [ ] C00/C01 单极性标准单元。
...
从 T01-C 的 VGS 续算开始：继承 T01-B 的 VGS=0 V、VDS=0.01 V 收敛状态，按冻结的 -1.0 到 1.0 V 栅压阶梯运行并保存每点收敛、端口电流和状态；T01-C 通过前不启动 T02、SPICE 或版图工作。收到条件完整原始数据后，再修订 G0 和启动定量标定。
