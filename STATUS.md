...
- 最后更新：2026-08-02
- 当前阶段：`M00_INPUT_VALIDATION_CONTRACT_E3_FIT_NEXT`
- 整体状态：`YELLOW`
- 当前原则：M00 输入与验证合同已以 25/25 纯静态检查 PASS、E3，冻结 13 个源表、5 个正式数据集、9 条训练曲线/163 个计分点、4 条整条件 holdout/70 个计分点、7 个零漏压不变量点、11 个有界系数、线性/对数误差门和失败保留。正式运行器与不导入运行器/SciPy 的独立检查器已实现，合成核自测 PASS，但尚未使用正式训练数据优化，M00 模型证据仍为 E0。下一步只允许一次正式 M00 教学紧凑代理拟合，通过后再做独立落盘检查；M01、SPICE、电路、版图、PEX 和 HZO 继续关闭
...
- [ ] M00/M01 紧凑模型拟合与双轨对照（M00 输入/验证合同 E3 已完成，正式拟合仍 E0）。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
- M00 静态合同的 13 表、9/163 train、4/70 holdout、7 个零偏压点和 11 个系数都是已冻结的未运行合同量，不是拟合结果。未来通过 M00 也只能称与项目教学模型数值曲线一致，不是实验标定、外部独立验证、物理参数提取、双仿真器验证或电路可用性。
...
按已提交前冻结的 M00 合同只运行一次正式教学紧凑代理拟合：优化只可读取 9 条 train/163 点，完成后一次性评估未参与优化的 4 条 holdout/70 点。运行器 PASS 后再执行不导入运行器的独立落盘检查；任一门 FAIL 即保留全部证据并停止。本阶段不运行 TCAD 或 SPICE，M01、电路、版图、PEX 和 HZO 继续关闭。
