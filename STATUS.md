...
- 最后更新：2026-08-03
- 当前阶段：`M01_R01_PREFLIGHT_E0_FAIL_OPEN_SOURCE_RECOVERY_CONTRACT_NEXT`
- 整体状态：`YELLOW`
- 当前原则：M00 R01 保持 21/24、E0/FAIL且全部证据不可改写。R02 的 27/27 静态合同冻结固定 `Lref/L`、10 个参数及不变的 9/163 train、4/70 holdout、全部指标和 `0.50` gm 门；唯一正式运行的 runner 为 24/24、E2，独立落盘复核为 20/20、E3。M00 仅在冻结、结构正则化的 IGZO 教学数值曲线和局部有效域内关闭。M01 revision-3 双仿真器合同仍是 32/32、E3 静态证据，v1/v2 失败保留。执行链提交 `a6386d2` 推送后，R01 工具/来源预检唯一运行一次并按合同得到 11/13、E0/FAIL：ngspice 指纹、无网表参数和 `ngspice-42` 版本门通过；AIM-Spice 只读文件指纹且未启动，因用户披露的未授权来源不可审计及可复现 batch/CLI 未建立而失败。唯一子进程是 `ngspice --version`，没有器件网表、数值曲线或电路输出。下一步只建立并提交开源第二仿真器恢复合同；M01 数值执行、C00、电路、版图、PEX 和 HZO 继续关闭。
...
- [ ] M00/M01 紧凑模型拟合与双轨对照（R01 正式拟合保持 E0/FAIL；R02 runner 24/24 E2、独立检查 20/20 E3，M00 仅在教学数值域内关闭；M01 合同 32/32 E3，但 R01 工具/来源预检 11/13、E0/FAIL，器件级双路线尚未运行）。
- [ ] C00/C01 单极性标准单元。
- [ ] C02/C03 环振和全加器。
...
5. M00 R01 的 `L=12 um` holdout gm 相对误差 `0.512384` 超过冻结 `0.50`，该失败永久保留；M01 R01 工具/来源预检也已以 11/13、E0/FAIL 落盘。本机 AIM-Spice 来源不可审计且可复现 batch/CLI 未建立，原第二路线关闭；开源替代路线另行冻结并提交前不得运行器件网表。

## 下一步

先提交并推送 M01 R01 工具/来源预检失败证据；随后只建立开源第二仿真器恢复合同，冻结官方软件包来源、许可证、版本与二进制哈希、可复现批处理入口、行为模型映射、输出、失败保留和 no-device-execution 边界。合同静态通过并提交前不得运行任何 M01 器件网表；不得重跑 M00 R01/R02 或 M01 R01 预检、改变原 split/阈值，或启动 C00、电路、版图、PEX、HZO。
