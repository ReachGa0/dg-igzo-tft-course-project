...
- 最后更新：2026-07-31
- 当前阶段：`T02_C_BIDIRECTIONAL_E2`
- 整体状态：`YELLOW`
- 当前原则：完整 T02 数值门已在冻结 E2 教学模型上关闭，允许进入 T03 受控参数敏感性；T02-C 的 VTH、Delta VTH、gm 和耦合斜率只能称为数值代理。G0 继续禁止实验拟合、物理参数验证、物理 Ion/Ioff 和定量电路预测
...
- [x] 文档、配置、目录合同、AI 入口、分章报告合同、17 阶段依赖图和 T02-A/B/C 证据包已通过 286 项项目检查。
...
进入 T03 前先冻结第一个最小单变量敏感性合同。优先只选择一个参数组、至少三个点，保持 T02-C 的几何、偏压网格、提取口径和其余物理参数不变；合同通过后再运行该组，不同时铺开陷阱、接触、几何/介质和温度，也不启动 SPICE 或版图。
