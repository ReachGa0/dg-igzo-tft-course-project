...
- 用户要求按最终 HTML 课程报告生成演讲稿。读取 `AGENTS.md`、正式 `课程报告.html`、STATUS 和现有 10 页 PPT 的实际内容，新增 `ppt/答辩演讲稿.md`。
- 讲稿逐页对应 PPT，建议约 8 分钟，覆盖二维边界、T02、五组 T03、M00/M01、C00 静态/瞬态负结果、交付边界和结论；另附 5 个关于 C00 失败、教学参数、两路线、恢复合同和下游关闭的追问答复。

### 边界

- 讲稿以 26 图、26 页正式 HTML 报告为主体口径。C00 R04 保持 33/36、E0/FAIL；未生成新图、未修改 PPT、未运行 TCAD、SPICE 或下游流程。
...
- 用户反馈精简报告中存在过大的主题页空白，要求改为紧凑排版，并删除之前的最短精华版。
- `scripts/build_condensed_report.py` 移除固定最小页高和强制主题分页；标题、表格、图和边界框保持避免拆分，图高收敛，使内容在打印页内连续流动。
- 删除 `report/src/项目精华版.xhtml`、`scripts/build_summary_report.py`、`report/final/项目精华版.html` 与 PDF；README 和 STATUS 不再把它列为交付入口。
- Edge headless 重新导出精简报告 PDF，`pypdf` 计数为 14 页；中段页面截图确认相邻主题连续衔接、图文未覆盖。临时截图不纳入交付。

### 边界

- 本次只改变文档版式和交付目录，不运行 TCAD、SPICE 或下游流程，不改写任何数值结果或 C00 R04 失败结论。

---

## 2026-08-04 | Codex GPT-5 | 22页课程项目精简报告

### 目标与输入

- 用户明确需要约 20--30 页、可独立阅读的压缩完整报告，而非仅供答辩的精华摘要。
- 读取既有完整报告、提交版架构、状态、T01/T02、五组 T03、M00/M01 与 C00 R04 的已冻结图表和结论；不修改物理输入、仿真结果、失败归档或历史证据。

### 修改与验证

- 新增 `scripts/build_condensed_report.py`，生成自包含 `report/final/项目精简报告.html` 与 PDF。内容固定为 20 个主题页：范围、二维模型、T01/T02、五组 T03、M00/M01、C00 拓扑与静态/瞬态负结果、失败解释、复现和结论。
- Edge headless 成功导出 PDF；`pypdf` 计数为 22 页。桌面截图复核首屏和分页，无文字覆盖；临时截图未纳入交付。

### 边界

- 精简报告仅重组既有证据。C00 R04 仍为 33/36、E0/FAIL；C01--C03、版图、PEX 和 HZO 未运行且不被写成完成。
- 下一步：运行项目与报告检查，通过后提交并推送此文档交付更新。

---

## 2026-08-04 | Codex GPT-5 | 最终报告排版修正与精华版

### 修改

- 用户反馈最终 HTML 出现穿模。桌面与窄屏只读截图复核后，修改 `report/src/实验报告_草稿.xhtml`：目录使用无序外部标记以消除重复编号，标题固定行高，表格允许长词断开并在窄屏横向滚动，图表避免跨页，打印时避免标题孤立。
- 新增 `report/src/项目精华版.xhtml` 与 `scripts/build_summary_report.py`；构建自包含的 `report/final/项目精华版.html` 和 PDF。精华版只保留研究问题、T02/T03、M00/M01、C00 R04 失败和提交边界，引用既有图，不产生新数值。

### 验证

- Edge headless 重新导出完整报告 PDF 和精华版 PDF；桌面完整报告及手机宽度精华版截图确认未出现覆盖。临时截图未纳入交付。
- `make check` 为 767 PASS；`make report-check` 为 12 章、5 附录、0 占位符、32 图 PASS；无 TCAD、SPICE、C00 或下游进程。

---

## 2026-08-04 | Codex GPT-5 | 提交版架构收敛

### 目标与决策

- 用户要求清理项目架构，仅保留已完成工作。经盘点，C01--C03、layout/PEX 与 HZO 只有计划、README 或机器配置，没有正式结果；T01--T03、M00/M01、C00 R04 和报告交付均有证据链。
- 未删除已跟踪的计划配置、空脚手架或历史失败，因为 `make check`、版本化合同和报告可复现性依赖这些归档。它们改为明确的“未纳入本次提交”状态，而非伪装成已完成或待冲刺工作。

### 修改

- 新增 `SUBMISSION_ARCHITECTURE.md`，冻结提交版 DAG：`S00 -> T01/T02/T03 -> M00/M01 -> C00 R04 FAIL -> R00`，并列出不纳入的 C01--C03、版图、PEX 和 HZO。
- `README.md`、`ARCHITECTURE.md`、`PROJECT_PLAN.md` 与 `STATUS.md` 均改为以提交版范围为入口；工作包和任务表不再把未实施下游列为当前目标。

### 边界

- 本次仅是文档/架构口径收敛，未删除任何器件、模型、失败、配置、结果或交付文件，未启动任何模拟器。C00 R04 的 33/36 E0/FAIL 保持不变。
...
- R03 失败证据已由提交 `77d611e` 推送并与 `origin/main` 同步；报告 `0527fdc5...a82766` 和 16 个原生产物逐字节保留，R03 源、配置和结果未修改、未重跑。
- 新建 `config/c00_active_load_inverter_r04.json`、R04 common/静态 checker/runner/独立 checker 和三个 Make 入口，注册 50/36/29 三门。机器状态为 `contract_implemented/E0`，电路执行、独立检查和下游权限均为 false。

### 唯一修正

- Xyce 瞬态 `.PRINT` 不再显式请求工具已隐式输出的 `TIME`；parser 遇到历史重复 `TIME` 时，只在两列数值逐行相等时折叠，其他重名或不等时明确失败。
- 原生数据门改为检查所有向量等长且有限、轴严格递增并覆盖注册起止端点。取消的只是错误的“原生瞬态点数 >=601”断言；36 案例每例 601 行的 5 ns 登记插值网格不变。
- runner 只在 36/36 PASS 时写入成功边界，FAIL 时写入失败边界；无异常但登记验收项失败时使用 `AcceptanceFailure` 并列出失败检查。不改 2-TFT 拓扑、portable IGZO 候选、18/36 案例、锚点、提取、阈值或四串行进程预算。

### 验证与下一门

- R04 parser 只读解析 R03 Xyce 失败 `.PRN`，将两个 TIME 列验证后折叠为 115 个物理点；109 个向量等长、全部有限，时间轴严格递增并覆盖 0--3 us。该检查启动 0 个模拟器进程、写入 0 个数值产物。
- 首次纯内存开发自检曾把静态 checker 的 49 个 `add_check` 调用位点误当成 50 个运行时检查而触发断言；源码中工具路线位点对 ngspice/Xyce 执行两次，因此注册数仍为 50。修正开发自检计数后 PASS；未修改合同数、未写入报告、未启动进程。
- R04 相关 JSON/Python AST 检查通过，`make check` 返回 767/767 PASS，`make report-check` 返回 12 章、5 附录、15 个既有占位和 30 图 PASS，`git diff --check` PASS。本实现提交推送前不运行 50 项 R04 静态 checker；静态 PASS 报告未另行登记推送前不运行四进程 runner。
...
在提交 `010a51f` 的 R11 36/32/25 完整工具/parser 预检上建立 `M01_OPEN_SOURCE_DEVICE_DC_R01`。新增配置、公共网表/解析/指标模块、40 项静态 checker、30 项未来 runner、24 项未来独立 checker 和三个 Make 目标。合同冻结同一 IGZO 行为候选与 247 行目标，以选择清单顺序为执行顺序、按 `row_uid` 连接预测表；每路线一个包含 247 个独立器件的 ASCII 单点 DC 网表，ngspice/Xyce 各一个串行进程，总预算 2。

### 输出、失败与边界

未来排他输出固定为两份原始 247 行表、30 行曲线/分组指标、247 行路线差异、两张 PNG、原始日志/网表/命令记录、runner 报告和独立报告。任一路线或解析失败保留并停止，不放宽阈值。runner 只有在 40/40 静态 PASS 已登记为 `contract_ready/E3` 且 next scope 已切换后才可启动，并将两条 argv 分别与独立构造的冻结命令比较；独立 checker 同样要求已登记的 `formal_run_passed/E2`。失败报告会哈希绑定当时已存在的部分产物。

内存网表生成自测 PASS，字节数为 36452/36365，没有写正式网表或启动模拟器。首次 `make check` 因新增项目检查器错误要求预测表与选择清单同序而 1 项 FAIL；失败报告 SHA-256 `dcf6c2ff89fd6f2d31dfc140c55cbe7d04f77e662370e35698acb926653599a0` 已归档。修正仅改为既有 UID 映射，随后 722/722 PASS，目标字节、物理和门槛不变。提交前 5 个 Python 文件通过 `py_compile`，18 个正式输出路径全部缺失，候选与 R11 三份报告哈希不变；`make check` 为 722/722 PASS，`make report-check` 为 12 章/5 附录/15 个既有占位/26 张图片 PASS，`git diff --check` 通过。

### 下一门

当前是 E0 实施态；40 项静态合同、runner 和独立检查均未运行，18 个未来路径全部不存在。提交并推送后才唯一运行静态合同；静态 40/40 PASS 状态另行提交前禁止执行 ngspice/Xyce 器件路线，C00 和全部下游继续关闭。
