...
- `C00_ACTIVE_LOAD_INVERTER_R01` 静态合同唯一返回 46/48、E0/FAIL，`nor` 子串误命中 `NORM`；R02 唯一静态 50/50 PASS、E3，runner 因 tracked 提交自引用未运行。R03 静态 50/50 E3 后，提交 `d0f6ef7` 上唯一四进程 runner 返回 24/36 E0/FAIL，报告 `0527fdc5...a82766`；16 个原生产物由 `77d611e` 保留，R03 不重跑。`C00_ACTIVE_LOAD_INVERTER_R04` 只修 Xyce 隐式/重复 TIME、原生轴物理覆盖和 PASS/FAIL 边界；保持拓扑、候选模型、18/36 案例、锚点、提取、阈值、50/36/29 门和四进程预算不变。实现提交 `fa04d29` 推送同步后，静态 checker 唯一返回 50/50 PASS、E3；报告 `1ec06611...fd56` 记录 0 模拟器进程、0 电路网表和 `NOT_RUN_BY_CONTRACT_CHECK`。该 E3 只开放报告哈希登记提交推送后的 R04 runner，不是锚点或电路性能通过；runner 与独立 checker 尚未运行，C01+、版图、PEX 和 HZO 仍关闭。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。M01 R03 42/30/24 E3/E2/E3 后只在冻结教学域 `DONE_WITH_LIMITATION`；R02 分歧、根因最小点和全部旧失败不改写。C00 R04 静态合同已唯一返回 50/50 E3；下一门只允许在报告哈希登记提交推送后唯一运行 R04 四进程 runner。不得重跑旧门、放宽锚点或阈值，也不得提前进入 C01+、KLayout、PEX 或 HZO。
