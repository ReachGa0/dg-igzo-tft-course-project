...
- `M01_OPEN_SOURCE_DEVICE_DC_R03` 已完成 42/30/24 E3/E2/E3 链：新候选唯一语义变化为显式 `min(max())` clamp；独立 checker 零进程精确重算 247+247 行、30 指标、247 差异、14 哈希和两图尺寸，复现最大绝对/对数差 `4.37069e-19 A/cm`/`2.75335e-14 decade`。提交 `5d2134c` 后已按冻结四项验收单独记录 `M01_TEACHING_MODEL_ONLY_PASS`，M01 仅在教学域 `DONE_WITH_LIMITATION`；C00 只开放静态合同规划，电路执行仍关闭。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。M01 R03 42/30/24 E3/E2/E3 后只在冻结教学域 `DONE_WITH_LIMITATION`；R02 分歧、根因最小点和全部旧失败不改写。下一门只建立 C00 静态合同；不得放宽阈值、改变 split、用 holdout 调参或提前执行电路、KLayout、PEX、HZO。
