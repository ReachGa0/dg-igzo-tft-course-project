...
- `C00_ACTIVE_LOAD_INVERTER_R01` 静态合同唯一返回 46/48、E0/FAIL，`nor` 子串误命中 `NORM`；0 进程、0 网表，R01 不重跑。`C00_ACTIVE_LOAD_INVERTER_R02` 绑定 R01 并只改为 ASCII 标识符整词比较，在实现提交 `216c6a7` 后唯一返回 50/50 PASS、E3；报告 SHA-256 `e820af3b6a80095a907ddbdc7ddab5461937cd99ff1bc442e03a6ffd07bd1a99`，仍为 0 进程、0 网表。执行前审计发现 R02 runner 的 `HEAD == origin == machine.static_pass_commit` 与 tracked 提交登记形成不可满足自引用，故 runner 未运行；阻塞报告 `9c810623...6ea51` 保留 0 网表/进程/数值输出。R02 冻结，R03 只可修正 committed-state 绑定，拓扑、18/36 案例、锚点、提取、阈值和 50/36/29 门不变。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。M01 R03 42/30/24 E3/E2/E3 后只在冻结教学域 `DONE_WITH_LIMITATION`；R02 分歧、根因最小点和全部旧失败不改写。下一门是新 C00 R03 committed-state 修正合同；不得重跑 R02、放宽阈值、改变 split、用 holdout 调参或提前执行电路、KLayout、PEX、HZO。
