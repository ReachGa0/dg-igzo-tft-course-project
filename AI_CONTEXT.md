...
- S00 数据审计及 T01-A/B/C/D-A 已完成，G0 为 `TEACHING_BASELINE_ONLY`；T01-D-A 已关闭低漏压正栅压目标点的网格警告，T01-D-B/C、M00/C00/L00/V01 仍未实现。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. 用户已授权并完成 T01-D-A。下一步只能以通过的界面网格口径进入 T01-D-B，多 VGS 运行冻结的 Id-Vd 点；随后 T01-D-C 补齐状态/提取并关闭完整 T01。未经新指令不得启动 T02、SPICE、KLayout 或大批量扫描。
