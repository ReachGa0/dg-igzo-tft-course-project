...
- `M01_OPEN_SOURCE_DEVICE_DC_R03` 当前为 independently verified E3 态：新候选只做五处登记替换（唯一语义变化为显式 `min(max())` clamp），绑定不可变 R02 40/30/24 与根因 40/30/22 链。42/42 静态 checker、30/30 runner 和 24/24 独立 checker 已按门唯一运行；独立 checker 零进程再生两网表、解析 raw/PRN、精确重算 247+247 行、30 指标、247 差异、14 哈希和两图尺寸，复现最大绝对/对数差 `4.37069e-19 A/cm`/`2.75335e-14 decade`。下一门是先提交本 E3，再单独判断 M01 教学边界收口和 C00 开放；当前仍不自动关闭 M01/C00。
- HZO 是可选扩展，不能阻塞基础闭环。
...
- M01 正式开源器件 DC R01/R02：R01 唯一静态检查固定为 39/40 E0/FAIL；R02 40/30/24 三门全部通过。三门绑定 R11 完整预检、247 行/13 曲线、同一 IGZO 候选、精确两进程预算、UID 连接、原始表/指标/差异/图和失败保留。R02 器件级执行与落盘完整性 E3，但路线不一致且该历史结论不可改写。根因 R01 为 38/40 E0/FAIL；根因 R02 40/30/22 已完成并独立复核，结论仅限最小探针。R03 42/30/24 已以 E3/E2/E3 完成，独立重算复现机器精度级路线一致；本 E3 提交并另行记录收口决定前，M01/C00 不关闭。
