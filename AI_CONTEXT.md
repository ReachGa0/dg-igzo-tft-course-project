...
- `C00_ACTIVE_LOAD_INVERTER_R01` 静态合同在实现提交 `4097b9d` 推送后唯一返回 46/48、E0/FAIL。唯一主失败为禁止词 `nor` 的子串匹配误命中注册 DC 节点 `NORM`，最终结果门连带失败；报告 SHA-256 `53f408e870e5f84993dd635cf95086c9b4db263ea5b8bead6cc2f5ed44a08493`，记录 0 进程、0 网表。R01 不重跑；失败提交后新建 R02，只修正 ASCII 标识符整词匹配，不改变拓扑、扫描、锚点、提取或阈值。
- HZO 是可选扩展，不能阻塞基础闭环。
