...
- `C00_ACTIVE_LOAD_INVERTER_R01` 当前为 `contract_implemented/E0`：2-TFT 端口映射、18 个 DC/36 个瞬态计划案例、锚点、提取、失败保留、四个串行 ngspice/Xyce 进程和 48/36/29 三门代码已冻结。T02 entry evidence 真实登记为 E2；C00 静态合同、runner 和独立 checker 均未运行，正式网表、VTC、延迟、功耗及下游证据不存在。先提交推送实现，再唯一运行 48 项零进程静态门。
- HZO 是可选扩展，不能阻塞基础闭环。
