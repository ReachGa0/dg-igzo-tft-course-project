...
- S00 数据审计及 T01-A/B/C/D-A/D-B/D-C 已完成，G0 为 `TEACHING_BASELINE_ONLY`；冻结教学模型的完整 T01 数值门已通过并打开 T02，T02、M00/C00/L00/V01 仍未实现。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. 用户已授权并完成 T01-D-C。下一步只能进入 T02 的最小双栅移动电荷输入合同和回归基线；先证明关闭顶栅耦合返回 T01，再扩展双栅偏压族。未经新指令不得启动 T03、SPICE、KLayout 或大批量扫描。
