...
- S00、T01-A/B/C/D-A/D-B/D-C 和 T02-A/B/C 已完成；T03-P4-L 已完整关闭，T03-P1-BIAS 也已完成 E3 复核，但 P1 仍缺少 CAP-RATIO 子阶段。G0 仍为 `TEACHING_BASELINE_ONLY`。冻结教学模型的完整 T01/T02 数值门已关闭；P2/P3/P5、M00/C00/L00/V01 仍未实现。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. T02-A/B/C、T03-P4-L 和 T03-P1-BIAS 已通过。下一步只能冻结独立的 `T03-P1-CAP-RATIO` 至少五点单变量合同，并先明确它与 P4 几何/介质变量的归属；合同通过后只运行这一子阶段。不得同时铺开 P2/P3/P5，也不得提前启动 SPICE、KLayout 或 HZO。
