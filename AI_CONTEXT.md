...
- M01 Xyce build/tool R07：配置、公共树哈希模块、39 项静态 checker、47 项 runner、25 项独立 checker 和 Make 入口已实现，静态合同尚未运行，当前 `contract_planned/E0`。R07 哈希绑定 R06 配置/checker/36/37 报告和两份项目检查失败；独立 checker 可登记 runner 路径，但不得导入 runner、导入/调用 `subprocess` 或执行进程。源码与成功依赖哈希不变，R05 partial 及 R06 Xyce/build/output 根禁止复用，全部新 R07 根为空。下一步提交 R07 实施后只运行静态合同一次。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 两级数值门和 M01 revision-3/开源恢复/R05 静态合同已完成，历史失败继续保留。M00 R01/R02、M01 Xyce build/tool R01/R02/R03/R04/R05/R06 和既有静态合同均不得重跑；不得放宽阈值、改变 split 或用 holdout 调参。R07 实施需先提交推送，再唯一运行 39 项静态检查；该 PASS 状态再次提交前不得运行 build/tool，runner PASS 前不得运行独立检查。M01 数值执行、电路、KLayout、PEX 和 HZO 继续关闭。
