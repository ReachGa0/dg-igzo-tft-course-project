...
- 正式 R01 静态合同固定为 39/40、E0/FAIL且不重跑。`M01_OPEN_SOURCE_DEVICE_DC_R02` 只把 R11 不存在的顶层进程字段读取替换为实际 25/25 summary 与 independence PASS；物理/数据/阈值/路线/进程和边界未改。R02 静态合同唯一 40/40 PASS、E3 后，runner 唯一 30/30 PASS、E2，恰好启动一个 ngspice 和一个 GPL Xyce 器件级 DC 进程，完整输出已落盘。ngspice 近数值底部、Xyce 跟随候选，最大路线绝对/对数差为 `4.68252e-4 A/cm`/`16.6705 decade`；合同明确这些是 diagnostic-only，不是路线一致、方程身份、物理参数或校准。独立 checker 尚未运行。
- HZO 是可选扩展，不能阻塞基础闭环。
...
- M01 正式开源器件 DC R01/R02：R01 唯一静态检查固定为 39/40 E0/FAIL；R02 静态合同唯一 40/40 PASS、E3，runner 唯一 30/30 PASS、E2。40/30/24 三门绑定 R11 完整预检、247 行/13 曲线、同一 IGZO 候选、精确两进程预算、UID 连接、原始表/指标/差异/图和失败保留。runner 只证明完整有限的器件级双路线执行；当前路线不一致，独立 checker 尚未运行。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。R02 静态合同 40/40 E3、runner 30/30 E2 已完成；路线差异仍是 diagnostic-only，不能写成路线一致或物理校准。先提交 runner 状态，再唯一运行独立 checker；不得放宽阈值、改变 split、用 holdout 调参或打开电路、KLayout、PEX、HZO。
