...
- `C00_ACTIVE_LOAD_INVERTER_R01` 静态合同唯一返回 46/48、E0/FAIL，`nor` 子串误命中 `NORM`；R02 唯一静态 50/50 PASS、E3，runner 因 tracked 提交自引用未运行。R03 静态 50/50 E3 后，提交 `d0f6ef7` 上唯一四进程 runner 返回 24/36 E0/FAIL，报告 `0527fdc5...a82766`；16 个原生产物由 `77d611e` 保留，R03 不重跑。`C00_ACTIVE_LOAD_INVERTER_R04` 只修 Xyce 隐式/重复 TIME、原生轴物理覆盖和 PASS/FAIL 边界；实现提交 `fa04d29` 上静态 checker 唯一 50/50 E3，登记提交 `017b295` 上 runner 唯一返回 33/36 E0/FAIL。四个开源进程全部返回 0，原生轴、完整表、108 行指标、差异表、两图和 26 个哈希通过；但冻结锚点两路线均只有约 0.0912 V VOH、约 0.884 最大增益、0 个单位增益交点和约 0.022--0.057 V 瞬态输出，无输出 crossing。报告 `18ea1e1a...a8f4` 的正确失败边界已接受。这是电路验收失败，不是工具/收敛/parser/路线一致失败；R04 不重跑，独立 checker 与 C01+、版图、PEX、HZO 关闭。
- HZO 是可选扩展，不能阻塞基础闭环。
...
- M01 开源替代路线只能使用可审计的纯源码 GPL Xyce 构建；XyceNF 专有模型和未授权 AIM-Spice 不进入正式证据链。
...
- T03-P5-TEMPERATURE 合同：E3 静态证据；23/23 PASS。冻结 250/300/350 K 和 `V_t=8.617333262e-5*T`，只修改既有 SG 电流的 channel `V_t` 参数；迁移率固定 35.5 cm2/(V*s)，DOS、能带、介电、接触、陷阱、几何和偏压不变。合同检查时 3 器件、123 DC、93 点、3 状态和 18 VTK 尚未运行，该合同记录本身不能写成 P5 或完整 T03 通过。
...
- M01 Xyce build/tool R07：静态合同在提交 `d421277` 后唯一返回 39/39 PASS、E3；随后 build/tool runner 在提交 `7d5337a` 后唯一返回 42/47、E0/FAIL，当前 `preflight_failed_self_test/E0`。M4/Bison/Flex、串行纯源码 Xyce 7.10.0、版本/许可证和 B-source 命令成功；Xyce 实际输出 `bsource_self_test.prn`，R07 误注册 `.csv` 并用 CSV parser，故观察值为空，parser-only 门未执行。报告 SHA-256 为 `7e2c17794b013c9928e3b707c674d3da4413c98aa2ab4ec25d2d145856d0a6e6`，所有 R07 logs/manifest/`.prn`/完整安装树保留，独立 checker 未运行。R08 只在新命名空间修正 `.prn` output/parser，但其静态 checker 已唯一中止于 30/36 注册缺陷；R09 静态 checker 又唯一返回 34/36，两个断言失败已归档；R10 静态合同唯一返回 36/36 PASS、E3，runner 唯一在三个工具命令后因 ASCII/中文绝对路径写入失败，状态为 `preflight_failed_runner/E0`。失败报告 SHA-256 `90e47fc91d58330faa7c53107d19579d51990eb8f17ba05a38d66b931bb40e16`，部分运行树 SHA-256 `5a3d1ac4ff62848fb7132db9211a6281a477b43900e87ddfe6047f6da9fef85e`；parser-only 与独立检查未运行。R11 静态合同唯一 36/36 PASS、E3 后 runner 唯一 32/32 PASS、E2，报告 SHA-256 `cfe369d5df97217499f207701836447f784f5dbc201ad33b01a0b1765841d552`，4 个工具/parser 进程均返回 0，1.25 V 自测和相对 ASCII include parser-only syntax 通过；独立 checker 唯一 25/25 PASS、E3，报告 SHA-256 `792a4f6d4be65746fc08ff0c00205acc2f7b64bc5ad2d3f0c370d27c41d2d615`，零进程。正式两路线器件 DC 仍未运行。
- M01 正式开源器件 DC R01/R02：R01 39/40 E0/FAIL、R02 40/30/24 路线分歧及根因 R01/R02 历史全部保留。R03 42/30/24 E3/E2/E3 满足同目标、模型边界、两路输出/差异和工具预检四项验收，M01 因而只在冻结教学域受限关闭。该决定不接受 AIM-Spice、不声称方程身份/物理参数/校准；只允许下一步建立 C00 静态合同。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。M01 R03 42/30/24 E3/E2/E3 后只在冻结教学域 `DONE_WITH_LIMITATION`；R02 分歧、根因最小点和全部旧失败不改写。C00 R04 runner 已唯一返回 33/36 E0/FAIL；不得重跑、运行其 PASS-only 独立 checker、放宽锚点/阈值或提前进入 C01+、KLayout、PEX、HZO。新恢复路线必须另建合同并单独评审。
