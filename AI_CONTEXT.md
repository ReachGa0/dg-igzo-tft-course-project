...
- M01 正式开源器件 DC R01/R02：R01 唯一静态检查固定为 39/40 E0/FAIL；R02 40/30/24 三门全部通过。三门绑定 R11 完整预检、247 行/13 曲线、同一 IGZO 候选、精确两进程预算、UID 连接、原始表/指标/差异/图和失败保留。当前结论是器件级执行与落盘完整性 E3，但路线不一致且 M01/C00 不关闭。根因 R01 为 38/40 E0/FAIL；R02 已在新命名空间完成 E0 schema-only 实现，只修正三份实际诊断 CSV 的读取和状态提示，静态合同尚未执行。先提交 R02 实施态，再执行 40 项静态合同。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。M01 R02 40/30/24 完成但确认路线分歧；E3 不能写成路线一致或物理校准。根因 R01 38/40 E0/FAIL 已冻结；根因 R02 40/30/22 已以 E3/E2/E3 完成，结论仅限预注册最小点。下一门为新可移植完整器件候选和独立 247 行静态合同；不得放宽阈值、改变 split、用 holdout 调参或打开正式敏感性、电路、KLayout、PEX、HZO。
