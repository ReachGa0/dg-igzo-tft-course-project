...
- S00、T01-A/B/C/D-A/D-B/D-C、T02-A 极限回归和 T02-B 最小正向顶栅族已完成，G0 为 `TEACHING_BASELINE_ONLY`；T02-B 只打开 T02-C 双向曲线族，完整 T02、M00/C00/L00/V01 仍未实现。
- HZO 是可选扩展，不能阻塞基础闭环。
...
8. T02-A/B 已通过。下一步只能先冻结 T02-C 的双向顶/底栅偏压、负偏压/回程路径、Delta VTH/gm 提取和状态点合同；合同通过后再运行双向族。T02-C 通过前不得启动 T03、SPICE、KLayout 或大批量扫描。
