...
- 正式 R01 静态合同固定为 39/40、E0/FAIL且不重跑。`M01_OPEN_SOURCE_DEVICE_DC_R02` 只把 R11 不存在的顶层进程字段读取替换为实际 25/25 summary 与 independence PASS；物理/数据/阈值/路线/进程和边界未改。R02 静态合同已唯一 40/40 PASS、E3，报告 SHA-256 `0154abfbe5175b91d7622416804561c5bb50bdeeef0792e426d0299a37564d2c`，记录零 build/simulator process、零器件网表和零数值输出；两路线 runner/独立 checker 未运行。
- HZO 是可选扩展，不能阻塞基础闭环。
...
- M01 正式开源器件 DC R01/R02：R01 唯一静态检查固定为 39/40 E0/FAIL；R02 schema-only 静态合同唯一 40/40 PASS、E3。40/30/24 三门绑定 R11 完整预检、247 行/13 曲线、同一 IGZO 候选、精确两进程预算、UID 连接、原始表/指标/差异/图和失败保留。R02 仅生成静态报告，器件网表、数值表、指标、差异和图仍不存在；提交静态 PASS 后才允许唯一两路线 runner。
...
8. T02-A/B/C、T03-P1/P2/P3/P4/P5、M00 R02 数值门与 R11 工具/parser 预检已完成，历史失败继续保留且不得重跑。R02 schema-only 静态合同已唯一 40/40 PASS、E3；先提交该状态，再唯一运行 ngspice/Xyce 两路线。runner PASS 提交前不得运行独立 checker，不得放宽阈值、改变 split、用 holdout 调参或打开电路、KLayout、PEX、HZO。
