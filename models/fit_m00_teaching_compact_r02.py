#!/usr/bin/env python3
"""Run the single preregistered M00 R02 teaching compact-model fit."""
        ...
        rows.append(
            {
                ...
                / (upper - lower),
                "provenance": "deterministic M00 R02 teaching-surrogate fit; not a physical parameter",
            }
        )
    return rows
...
def generate_fit_figure(
    ...
    figure.suptitle("M00 R02 IGZO teaching surrogate fit (T=train, H=holdout)", fontsize=11)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def generate_ngspice_candidate(path: Path, parameters: dict[str, float]) -> None:
    content = f"""* IGZO-only M00 behavioral translation candidate R02
...
.subckt IGZO_DG_BEHAVIORAL_R02 D TG BG S params: WUM=60 LUM=10
...
BIDS D S I={{sgn(V(D,S))*WUM*1e-4*(pow(10,M00_LOGBETA)*(10/LUM)*(pow(M00_Q(M00_ETA*(V(TG,S)+V(BG,S))-M00_VTHDG-M00_KVTHL*ln(LUM/10),M00_SOFT),M00_GAMMA)-pow(M00_Q(M00_ETA*(V(TG,S)+V(BG,S))-M00_VTHDG-M00_KVTHL*ln(LUM/10)-M00_KD*abs(V(D,S)),M00_SOFT),M00_GAMMA))*(1+M00_LAMBDA*abs(V(D,S)))+pow(10,M00_LOGGMIN)*abs(V(D,S)))}}
.ends IGZO_DG_BEHAVIORAL_R02
"""
    path.parent.mkdir(parents=True, exist_ok=True)
...
def generate_aimspice_candidate(path: Path, parameters: dict[str, float]) -> None:
    content = f"""* IGZO-only AIM-Spice LEVEL=15 candidate R02
...
.model IGZO_N_L15_R02 NMOS LEVEL=15 VTO={parameters['vth_dual_v']:.16g} TOX=10n EPSI=6.8 MUBAND=3.55e-3 RD=0 RS=0 LAMBDA={parameters['lambda_per_v']:.16g} VFB=0 ALPHASAT=0.6 KASAT=0.006 DEF0=0.6 DELTA=5 EL=0.35 EMU=0.06 GAMMA=0.4 IOL=1e-14 GMIN=1e23 SIGMA0=1e-14 V0=0.12 VAA=7.5e3 VDSL=7 VGSL=7 VMIN=0.3 TNOM=27
.subckt IGZO_DG_LEVEL15_R02 D TG BG S params: W=60u L=10u
EGEFF GEFF S VALUE={{{parameters['eta_dg']:.16g}*(V(TG,S)+V(BG,S))}}
MCORE D GEFF S S IGZO_N_L15_R02 W={{W}} L={{L}}
.ends IGZO_DG_LEVEL15_R02
"""
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("x", encoding="ascii") as stream:
        stream.write(content)
...
def run_formal_fit(config_path: Path) -> dict[str, Any]:
    ...
    existing_outputs = [
        ...
        if key != "contract_report" and path.exists()
    ]
    if existing_outputs:
        relative = ", ".join(str(path.relative_to(ROOT)) for path in existing_outputs)
        raise FileExistsError(f"refusing to overwrite R02 formal outputs: {relative}")
    ...
    snapshot = {
        ...
        "independent_checker": artifact_entry(ROOT / "scripts" / "check_m00_compact_model_fit_r02.py"),
    ...
    add_check(checks, "reporting:separate_metrics_provenance_and_validity", len(curve_metrics) == 13 and len(params_rows) == 10 and validity["validity_domain"] == config["validity_domain"] and aggregate.keys() == {"train", "holdout"}, "curve metrics, parameter provenance and local domain persisted")
    ...
    if base_pass:
        ...
        mapping = {
            "mapping_id": "IGZO_M00_TO_AIMSPICE_LEVEL15_R02",
            ...
            "fixed_teaching_inputs": {
                ...
                "fixed_length_geometry_exponent": 1.0,
            },
            "not_directly_mapped": [
                "log_beta",
                "vth_single_v",
    ...
    report = {
        ...
        "prohibited_claims": config["evidence_boundary"]["prohibited_claims"],
        "next_gate": "Run the R02 independent persisted-evidence checker only if this report passes; M01 and downstream remain closed.",
    }
...
def self_test() -> int:
    ...
    values = np.asarray([-3.0, 0.2, 0.15, 0.5, 0.026, 2.0, 1.0, 0.1, -30.0, 0.0])
    base = {
    ...
    length_points = [
        {**base, "l_um": length, "vds_v": 0.01} for length in (10.0, 8.0)
    ]
    length_output = evaluate_kernel(values, length_points, config)
    ...
    passed = (
        ...
        and np.isclose(length_output[1] / length_output[0], 1.25, rtol=1.0e-10)
        and scipy_probe.success
        and np.allclose(scipy_probe.x, [1.0, 2.0])
    )
    print(f"M00_COMPACT_MODEL_R02_SYNTHETIC_SELF_TEST_{'PASS' if passed else 'FAIL'}")
...
def main() -> int:
    ...
    try:
        ...
        print(f"M00_COMPACT_MODEL_R02_FIT_ERROR {type(error).__name__}: {error}", file=sys.stderr)
        return 1
    print(
        f"M00_COMPACT_MODEL_R02_FIT_{report['status']} "
    ...
    for failure in report["failures"]:
        print(f"M00_COMPACT_MODEL_R02_FIT_GATE_FAIL {failure['name']}: {failure['detail']}", file=sys.stderr)
