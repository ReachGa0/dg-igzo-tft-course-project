...
S00 来源、单位和冲突审计及 T01-A/B/C/D-A/D-B 已完成。T01-D-B 用 `interface_4x` 运行冻结的 4 条单栅 `Id-Vd` 采样曲线，并用 `interface_8x` 复核 VGS=0.5/1.0 V；6 条曲线、65 次 DC、30 个正式点全部收敛，最细两档最大电流差为 0.01639%。G0 决策仍为 `TEACHING_BASELINE_ONLY`：T01-D-C 状态图与受限参数提取、实验标定、电路和版图尚未完成。所有 `TODO/E0` 内容都只是计划，不得在汇报中写成已完成结果。
...
| 完整 T01 漂移扩散电流模型 | E0 | T01-D-C 仍待完成；已有离散 Id-Vd 曲线族，但全工作区状态和受限 VTH/SS/迁移率提取边界尚未关闭 |
...
2. 完成 T01-D-C 的状态图集合、受限参数提取和完整 T01 门。
...
当前已通过 T01-D-B 的受限离散 Id-Vd 阶段门，不把它扩大为完整模型性能：
...
- 该 PASS 只允许对冻结离散偏压点作教学模型数值结论；连续输出特性、真实饱和机理、物理 Ion/Ioff、实验精度和参数提取继续阻塞到 T01-D-C 或条件完整原始数据。
- 项目结构检查通过。

连续或实验校准的 `Id-Vd`、全偏压 `Id-Vg` 网格指标、VTH/SS/迁移率验证和实验对比仍不在本次 PASS 范围内。
