...
S00 来源、单位和冲突审计已完成；T01-A 输入合同和 T01-B 单栅低偏压漂移扩散烟雾均已通过。G0 决策仍为 `TEACHING_BASELINE_ONLY`：T01 只能做 E2 教学参数案例，完整 `Id-Vg`/`Id-Vd`、实验标定、电路和版图尚未完成。所有 `TODO/E0` 内容都只是计划，不得在汇报中写成已完成结果。
...
| 完整 T01 漂移扩散电流模型 | E0 | T01-C/D 仍待完成；尚未得到完整 Id-Vg/Id-Vd 或参数提取 |
...
2. 完成 T01-C 单栅 VGS continuation，再完成 T01-D 的 Id-Vd、状态图和完整网格指标。
...
当前先验收 T01-B 的受限数值门，不把它扩大为完整模型性能：
...
- 单极性逻辑拓扑和器件数固定。
- 旧范围电路结果不再作为当前证据。
- 两档网格的零偏压和 VGS=0 V、VDS<=0.01 V 求解收敛，端口电流守恒和低偏压网格比较通过。
- 项目结构检查通过。

完整 `Id-Vg`、完整 `Id-Vd`、VTH/迁移率提取和实验对比仍不在本次 PASS 范围内。
