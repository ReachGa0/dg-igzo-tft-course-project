...
S00 来源、单位和冲突审计及 T01-A/B/C 已完成。T01-C 已得到 VDS=0.01 V 下的教学参数低漏压 `Id-Vg` 续算，但高正栅压绝对电流的粗细网格差最高 27.5%。G0 决策仍为 `TEACHING_BASELINE_ONLY`：完整 `Id-Vd`、网格收敛、参数提取、实验标定、电路和版图尚未完成。所有 `TODO/E0` 内容都只是计划，不得在汇报中写成已完成结果。
...
| 完整 T01 漂移扩散电流模型 | E0 | T01-D 仍待完成；尚未得到网格收敛的完整 Id-Vd、VTH/SS/迁移率提取 |
...
2. 完成 T01-D 的累积层网格加密、Id-Vd、状态图集合和完整网格指标。
...
当前已通过 T01-C 的受限数值门，不把它扩大为完整模型性能：
...
- 单极性逻辑拓扑和器件数固定。
- 旧范围电路结果不再作为当前证据。
- 两档网格的 30 次 DC 求解和 16 个 VGS 点全部收敛，端口电流守恒，漏端电流随 VGS 单调增加。
- 16 份节点状态和选定偏压 VTK 均已保存，独立结果检查通过。
- 高正栅压绝对电流网格差明确标为警告；当前定量绝对电流、物理 Ion/Ioff 和参数提取继续阻塞到 T01-D。
- 项目结构检查通过。

完整 `Id-Vd`、网格收敛的 `Id-Vg`、VTH/SS/迁移率提取和实验对比仍不在本次 PASS 范围内。
