...
S00 来源、单位和冲突审计已完成，T01-A 单栅漂移扩散输入合同已冻结但尚未运行仿真。G0 决策为 `TEACHING_BASELINE_ONLY`：T01 只能做 E2 教学参数案例，尚未完成新的器件电流、电路或版图实现。所有 `TODO/E0` 内容都只是计划，不得在汇报中写成已完成结果。
...
| T01-A 单栅漂移扩散输入合同 | E3（合同）/E0（仿真） | 单栅、单位、物理范围、接触、网格和偏压阶梯已冻结；`simulation=NOT_RUN` |
