...
`M01_R06_XYCE_RECOVERY_CONTRACT_FAILED_E0_NEXT_R07`：T01/T02/T03 数值门已关闭，历史失败保留。M00 R01 保持 21/24、E0/FAIL；R02 runner 24/24 E2、独立检查 20/20 E3，只在冻结 IGZO 教学数值域内关闭 M00。M01 revision-3 合同 32/32 E3，R01 工具/来源预检唯一运行 11/13、E0/FAIL，开源恢复合同 30/30 E3。Xyce build/tool R01 执行 14/29、独立 9/20 E0/FAIL；R02/R03/R04 静态合同分别唯一检查 22/25、21/25、25/26 E0/FAIL。R05 静态合同 27/27 E3 后，runner 唯一运行 19/29 E0/FAIL。R06 静态合同因 checker 的 runner 路径字面量矛盾唯一返回 36/37、E0/FAIL；没有构建、模拟器、网表或数值输出，下一步建立 R07。
...
| M01 | 双仿真器对照 | PREFLIGHT E0/FAIL；R06 静态合同 36/37；Xyce 尚未生成或运行 | revision-3 32/32、开源恢复 30/30；R01 执行 14/29、独立 9/20，R02/R03/R04 静态合同 22/25、21/25、25/26 均 E0/FAIL 且保留。R05 合同 27/27 E3、runner 19/29 E0/FAIL；R06 唯一失败是 checker 路径字面量矛盾，0 个构建/模拟器进程，无网表、自测或正式器件 DC |
...
M00 R01/R02 和 M01 R01/R02/R03/R04/R05/R06 均不重跑，不放宽门槛、不改变 split 或用 holdout 选参数。M01 revision-3、开源恢复、Xyce build/tool R01 合同和 R05 合同分别为 32/32、30/30、25/25、27/27 E3；R01 执行/独立检查 14/29、9/20，R02/R03/R04 静态合同 22/25、21/25、25/26，R05 runner 19/29，R06 静态合同 36/37，失败均保留。下一步提交 R06 失败后建立 R07，只修正 runner 路径字面量断言并使用全新 revision 根；R07 静态 PASS 状态提交前不运行 build/tool。正式 M01 器件 DC、电路、版图、PEX 和 HZO 继续关闭。
