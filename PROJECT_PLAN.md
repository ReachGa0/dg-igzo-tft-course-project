...
`T03_P2_BULK_TRAPS_EQUATION_SMOKE_E3_PARTIAL_P2`：T01/T02 冻结教学模型数值门、T03-P4-L 和数值 P1 已关闭，P2-DIT 界面子阶段也已关闭。bulk-trap 30/30 静态合同、3 器件/21 次 DC 方程冒烟和独立 16 项检查已通过，正式 NTA/NGA transfer 敏感性尚未运行，因此完整 P2 仍为 partial。G0 仍为 `TEACHING_BASELINE_ONLY`；P3/P5、紧凑模型和定量电路预测也尚未完成。
...
| T03 | 五组器件参数 | PARTIAL，P1 DONE；P4-L DONE；P2-DIT 界面子阶段 DONE；P2 bulk equation smoke DONE，正式扫描与 P3/P5 TODO | 方程冒烟只提供未标定教学模型的 E2/E3 数值证据，完整 P2/T03 尚未关闭 |
...
数值 P1/P4、P2-DIT 界面子阶段和 bulk-trap 三案例方程冒烟已完成。下一件事先建立正式隔离 NTA/NGA transfer-sensitivity 合同，明确正式点、提取方法、失败保留和输出边界；合同与独立检查通过前不铺开 P3/P5，也不启动 SPICE 或版图。
