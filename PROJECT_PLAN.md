...
`T02_A_LIMIT_REGRESSION_E2`：T01-A/B/C/D-A/D-B/D-C 与 T02-A 顶栅极限回归已通过；T02-A 以 16 项合同检查冻结对称 30 nm Al2O3 顶栈，14 次 DC 全部收敛，移除整个顶栈的 7 个点返回 T01，启用顶栈全零偏压平衡态通过。G0 仍为 `TEACHING_BASELINE_ONLY`，提取量只是数值代理，非零双栅电流、实验拟合和定量电路预测继续被阻止。
...
| T02 | 双栅电流与阈值耦合 | T02-A DONE，T02-B/C TODO | 依赖 T01；非零双栅偏压族待实现 |
...
T02-A 的输入合同和极限回归已完成。当前下一件事是 T02-B：只运行一个最小非零顶栅偏压族，固定 VDS、VBG、几何、缺陷、接触和温度，只验证电流方向、守恒、顶栅结构和内部状态。通过 T02-B 后才能扩展双向曲线族与 T03；收到主 IGZO 原始数据和双栅边界后，才升级为定量标定路线。
