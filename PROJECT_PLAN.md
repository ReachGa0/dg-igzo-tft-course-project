...
`T03_P2_BULK_TRAPS_CONTRACT_E3_PARTIAL_P2`：T01/T02 冻结教学模型数值门、T03-P4-L 和数值 P1 已关闭，P2-DIT 界面子阶段也已关闭。bulk-trap 合同已以 29/29 静态检查冻结 E1 NTA/NGA 点、能量分布、准静态占据、96 点积分、共同偏压和最小冒烟门，但没有运行 DEVSIM，因此完整 P2 仍为 partial。G0 仍为 `TEACHING_BASELINE_ONLY`；P3/P5、紧凑模型和定量电路预测也尚未完成。
...
| T03 | 五组器件参数 | PARTIAL，P1 DONE；P4-L DONE；P2-DIT 界面子阶段 DONE；P2 bulk contract PASS，方程冒烟/正式扫描与 P3/P5 TODO | bulk 合同只有 E3 静态证据且未运行仿真，完整 P2/T03 尚未关闭 |
...
数值 P1/P4 与 P2-DIT 界面子阶段已完成，bulk-trap 静态合同也已通过。下一件事只运行合同规定的三个案例方程冒烟：零控制、NTA 参考、NGA 参考；冒烟与独立落盘检查通过后才允许另立正式三点扫描。此前不铺开 P3/P5，也不启动 SPICE 或版图。
