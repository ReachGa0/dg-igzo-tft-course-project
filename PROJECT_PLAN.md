...
`T03_P3_CONTACT_V2_CONTRACT_E3_RUN_NEXT`：T01/T02 冻结教学模型数值门、T03-P4-L、数值 P1/P2 已关闭。P3 V1 完成 12 器件/243 次收敛 DC/156 点但因零漏压相对守恒适用域错误保持 E0/FAIL；V2 的 34/34 E3 静态恢复合同保持三点、模型、偏压、提取、阈值和预算不变，只分离 `VDS>0` 相对门与 `VDS=0` 绝对门并使用独立路径，明确没有运行 V2 DEVSIM。G0 仍为 `TEACHING_BASELINE_ONLY`；P3 V2 正式运行、P5、完整 T03、紧凑模型和定量电路预测尚未完成。
...
| T03 | 五组器件参数 | PARTIAL，P1/P2/P4 DONE；P3 V1 FAIL PRESERVED、V2 CONTRACT E3、V2 RUN TODO；P5 TODO | P3 V1 24/25 失败不是完成；V2 34/34 静态合同不是仿真；完整 T03 尚未关闭 |
...
P3 V1 失败证据已完整保留，V2 静态恢复合同已 34/34 PASS 且尚未运行 V2。当前下一件事只完整运行一次正式 T03-P3 V2 对称串联接触电阻代理敏感性；若运行器 PASS，再执行不导入运行器/DEVSIM 的独立持久化证据检查。不得改变 `0/0.5/4.5 kOhm*um` 点、总对称分摊口径、偏压/提取或数值阈值；P5、M00/M01、SPICE、电路、版图、PEX 和 HZO 继续关闭。
