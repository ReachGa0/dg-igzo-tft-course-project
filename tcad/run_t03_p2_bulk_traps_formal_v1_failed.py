...
def assess(
    ...
    add_check(
        checks,
        "eight_fresh_devices_and_328_dc_solves_converged",
        [run.get("case_id") for run in runs] == expected_case_ids
        and len(runs) == 8
        and [len(run.get("solver_records", [])) for run in runs] == [41] * 8
    ...
    add_check(
        checks,
        "exact_eight_curve_248_point_grid_completed",
    ...
    pairwise_valid = (
        int(zero_control.get("point_count", -1)) == 31
    ...
    add_check(
        ...
        and "P2, T03" in prohibited
        and "physical or experimentally validated" in prohibited,
