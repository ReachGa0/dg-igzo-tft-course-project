#!/usr/bin/env python3
"""Run the V2 formal isolated T03-P3 contact-resistance sensitivity."""
...
def assess(
    ...
    nonzero_vds_rows = [
        row for row in all_rows if float(row["external_vds_v"]) > 1.0e-12
    ]
    max_all_point_imbalance = max(
        float(row["relative_current_imbalance"]) for row in all_rows
    )
    max_nonzero_vds_imbalance = max(
        float(row["relative_current_imbalance"]) for row in nonzero_vds_rows
    )
    add_check(
        checks,
        "nonzero_vds_device_terminal_current_conservation",
        acceptance["relative_device_terminal_current_imbalance_gate_domain"]
        == "external VDS>0"
        and bool(nonzero_vds_rows)
        and max_nonzero_vds_imbalance
        <= float(acceptance["maximum_relative_device_terminal_current_imbalance"]),
        (
            f"rows={len(nonzero_vds_rows)} "
            f"maximum_nonzero_vds_relative_imbalance={max_nonzero_vds_imbalance:.6e} "
            f"maximum_all_point_relative_imbalance_nongating={max_all_point_imbalance:.6e}"
        ),
    )
    ...
    return {
        ...
        "summary_metrics": {
            ...
            "maximum_nonzero_vds_relative_terminal_current_imbalance": max_nonzero_vds_imbalance,
            "maximum_all_point_relative_terminal_current_imbalance_nongating": max_all_point_imbalance,
...
def archive_failed_run(
    ...
    report_stem = Path(config["outputs"]["report"]).stem
    archive_report = ROOT / "results" / "reports" / f"{report_stem}_{slug}.json"
    ...
    for name, path in candidates.items():
        ...
        try:
            ...
            continue
        destination = external_dir / f"{name}__{path.name}"
        if destination.exists():
            raise RuntimeError(
                f"refusing to overwrite failed-run external artifact: {destination}"
            )
