...
19. `T03-P5-TEMPERATURE` 已在 23/23 E3 静态合同后完成唯一一次正式运行：3 器件/123 次收敛 DC/93 点，runner 14/14 E2、独立检查 15/15 E3，关闭冻结边界内的五组数值 T03。
...
三条已按阶段门顺序执行。静态合同为 23/23 PASS、E3、`NOT_RUN_BY_CONTRACT_CHECK`；随后唯一一次正式运行完成 3 个新器件、123 次全部收敛 DC、93 个 transfer 点、3 状态和 18 VTK，runner 14/14 PASS、E2；最后不导入运行器或 DEVSIM 的独立落盘检查 15/15 PASS、E3。300 K 曲线、状态、VTH 和 gm 精确复现 T02-C，最大端口相对不平衡为 `4.37301e-10`。

合同只把既有 Scharfetter-Gummel 电子电流中的 `V_t` 替换为 `k_B*T`；教学迁移率仍为 35.5 cm2/(V*s)，DOS、能带、介电、接触、陷阱、几何、网格和偏压均不变。250/300/350 K 的 VTH 与 SS 数值代理增加，gm 与高栅电流代理下降，低栅电流代理增加；这些只是在冻结闭包内的数值响应，不是物理或校准 IGZO 温度模型。
...
4. T03-P4-L、数值 P1、完整数值 P2、数值 P3 和 P5 已完成，P2/P3 的历史失败均保留；
5. 五组数值 T03 只在冻结、未校准的二维教学模型边界内关闭，不代表实验或物理参数验证；
6. 下一门只允许建立 M00 教学紧凑模型输入与验证合同；合同通过并提交前不拟合、不运行 SPICE；
7. 与老师数据或条件完整的文献/实验数据定量对比仍待输入条件补齐。
