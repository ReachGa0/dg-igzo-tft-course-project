#!/usr/bin/env python3
"""Check the revision-2 Xyce build/preflight contract without invoking a simulator."""
...
CONFIG_PATH = ROOT / "config" / "m01_xyce_build_preflight_r02.json"
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r02.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r02.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r02.py"
...
def check_contract() -> dict[str, Any]:
    ...
    r02 = m01["xyce_build_preflight_r02"]
    checks: list[dict[str, str]] = []

    add_check(
        checks,
        "identity:revision_2_xyce_build_preflight",
        config.get("stage_id") == "M01"
        and config.get("preflight_id") == "M01_XYCE_BUILD_PREFLIGHT_R02"
    ...
    add_check(
        ...
        "old transcription retained; independent archive hash is the R02 input",
    )
    ...
    add_check(
        ...
        and all(token.lower() not in candidate_text.lower() for token in ("sno", "hzo", "ferroelectric", "tran", "noise", "circuit")),
    ...
    add_check(
        checks,
        "build:unique_external_r02_roots",
        ...
        "R02 roots are unique, external and absent before execution",
    )
    add_check(
        checks,
    ...
    add_check(
        checks,
        "outputs:exclusive_unique_r02_namespace",
        ...
        and all("r02" in value for key, value in config["outputs"].items() if key != "contract_report"),
        "R02 run artifacts are unique and append-only",
    )
    add_check(
        checks,
        "outputs:r01_failure_paths_preserved",
        all((ROOT / value).exists() for value in config["historical_paths_to_preserve"])
        and r01_report.is_file()
        and r01_check.is_file(),
        "historical R01 failure artifacts remain present",
    )
    ...
    add_check(
        ...
        "R02 evidence remains tool-only",
    )
    add_check(
        checks,
        "project:next_scope_is_r02",
        project["tcad_track"]["next_scope"].startswith("establish and commit M01 Xyce build/tool preflight revision-2")
        and m01["status"] == "preflight_failed_tool_provenance",
        project["tcad_track"]["next_scope"],
    )
    add_check(
        checks,
        "experiment:r02_is_planned_and_r01_failed",
        r02["status"] == "preflight_planned"
        and r02["revision"] == 2
        and r02["current_evidence"] == "E0"
        and r02["formal_run_completed"] is False
        and r02["device_netlist_invoked"] is False
        and m01["xyce_build_preflight"]["status"] == "preflight_failed_build"
        and m01["xyce_build_preflight"]["preflight_run_ordinal"] == 1,
        "R02 is unexecuted while R01 failure remains machine-recorded",
    )
    ...
    add_check(
        checks,
        "runner:r02_wrapper_and_no_formal_execution",
        ...
        "R02 wraps the reviewed runner and injects only registered BLAS/LAPACK options",
    )
    add_check(
        checks,
        "checker:r02_independent_standard_library",
        ...
        "R02 independent checker has no simulator subprocess path",
    )
    add_check(
        checks,
        "make:r02_targets_registered",
        "m01-xyce-build-preflight-r02-contract-check:" in makefile_source
    ...
    add_check(
        checks,
        "gate:formal_m01_requires_r02_independent_pass",
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R02 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    with REPORT_PATH.open("x", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print(f"M01_XYCE_BUILD_PREFLIGHT_R02_CONTRACT_{result['status']} checks={len(checks) - len(failures)}/{len(checks)} report={REPORT_PATH}")
    return result
