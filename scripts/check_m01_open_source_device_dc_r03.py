#!/usr/bin/env python3
"""Independently recompute persisted M01 R03 two-route device DC evidence."""
...
from m01_open_source_device_dc_r03_common import (
    DIFFERENCE_FIELDS,
    METRIC_FIELDS,
    RAW_FIELDS,
    compute_metrics,
...
RUN_REPORT_PATH = ROOT / "results" / "reports" / "m01_open_source_cross_check_r03.json"
CHECK_REPORT_PATH = ROOT / "results" / "reports" / "m01_open_source_cross_check_r03_check.json"
...
def check() -> int:
    ...
    machine = experiment_map["M01"]["open_source_device_dc_r03"]
    ...
    expected_xy = [
        ...
        outputs["xyce_log"],
        "-o",
        "results/compact/m01_open_source_cross_check_r03/xyce_values",
    ...
    add_check(
        ...
        and candidate_path.read_text(encoding="ascii").count(".subckt IGZO_DG_BEHAVIORAL_R03_PORTABLE") == 1
    ...
    print(
        f"M01_OPEN_SOURCE_DEVICE_DC_R03_CHECK_{status} checks={passed}/{EXPECTED_CHECK_COUNT} "
