#!/usr/bin/env python3
"""Build the R07 generator toolchain and Xyce without formal device DC."""
...
R07_STATIC_BOUNDARY_MARKER = "formal_device_dc_invoked=false"
...
def run_preflight() -> dict[str, Any]:
    ...
    if preexisting:
        raise RuntimeError(
            "Refusing to overwrite existing R07 outputs: "
    ...
    if existing_new_roots:
        raise RuntimeError(
            "Refusing to reuse R07 build/install roots: "
    ...
    with preflight_log.open("x", encoding="utf-8") as stream:
        stream.write("M01 Xyce build/tool preflight R07\n")
        ...
        stream.write("r06_xyce_or_outputs_reused=false\n")
    ...
    add_check(
        checks,
        "binding:r07_static_contract_passed",
    ...
    add_check(
        checks,
        "build:exclusive_new_r07_roots_absent",
...
"""
    ...
    add_check(checks, "build:xyce_configure", xyce_configure_ok, "Xyce 7.10 R07 configure")
...
"""
    ...
    add_check(
        ...
        "formal M01 requires persisted R07 preflight plus independent check",
    )
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(
            f"R07 preflight registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}"
        )
    ...
    report = {
        ...
        "summary": {
            ...
            "r06_xyce_or_outputs_reused": False,
        },
        ...
        "next_gate": "Run the R07 independent persisted-evidence check exactly once; formal M01 device DC and downstream stages remain closed until it passes."
        if not failures
        else "Retain this failed R07 preflight and stop M01; diagnose in a new revision without deleting evidence or weakening the registered gate.",
    }
    with report_path.open("x", encoding="utf-8") as stream:
        json.dump(report, stream, indent=2, ensure_ascii=True)
        stream.write("\n")
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R07_{report['status']} "
