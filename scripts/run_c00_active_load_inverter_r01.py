#!/usr/bin/env python3
"""Run the formal C00 R01 ngspice/Xyce inverter analyses exactly once."""
...
CONTRACT_CHECKER_PATH = ROOT / "scripts" / "check_c00_active_load_inverter_r01_contract.py"
...
def run() -> int:
    ...
    try:
        ...
        static_ok = (
            ...
            and static_report.get("summary") == {"passed": 48, "failed": 0, "total": 48}
        ...
        machine_ok = (
            ...
            and machine.get("contract_checks_passed") == 48
        ...
        scope_ok = all(token not in text.lower() for token in forbidden for text in texts.values())
        _set(checks, "netlists:no_forbidden_scope", scope_ok, f"forbidden={len(forbidden)}")
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R01_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={report_path}")
