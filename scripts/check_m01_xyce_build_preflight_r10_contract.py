#!/usr/bin/env python3
"""Check the R10 Xyce output/parser contract without running any process."""
...
from m01_xyce_r10_common import digest_tree, load_json, read_xyce_prn, sha256, tree_matches


ROOT = Path(__file__).resolve().parents[1]
CONFIG_PATH = ROOT / "config" / "m01_xyce_build_preflight_r10.json"
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r10.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r10.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r10.py"
COMMON_PATH = ROOT / "scripts" / "m01_xyce_r10_common.py"
...
def check_contract() -> dict[str, Any]:
    if REPORT_PATH.exists():
        raise RuntimeError(f"Refusing to overwrite existing R10 contract report: {REPORT_PATH}")
    ...
    machine = m01.get("xyce_build_preflight_r10", {})
    checks: list[dict[str, str]] = []

    add_check(
        checks,
        "identity:revision_10_xyce_parser_recovery",
        config.get("stage_id") == "M01"
        and config.get("preflight_id") == "M01_XYCE_BUILD_PREFLIGHT_R10"
        and config.get("revision") == 10
    ...
    add_check(
        ...
        "immutable R08 30/36 and R09 34/36 checker failures are preserved",
    )
    add_check(
        checks,
        "binding:r08_sources_immutable",
        r08["must_remain_unchanged"] is True
        and r09["must_remain_unchanged"] is True
        and all(
            (ROOT / path).is_file() and sha256(ROOT / path) == expected
            for path, expected in r08_source_bindings + r09_source_bindings
        ),
        f"artifacts={sum((ROOT / path).is_file() for path, _ in r08_source_bindings + r09_source_bindings)}/{len(r08_source_bindings + r09_source_bindings)}",
    )
    add_check(
        checks,
        "boundary:r08_rerun_prohibited",
        ...
        and r09_failure_report.get("summary", {}).get("build_processes_invoked") == 0
        and r09_failure_report.get("summary", {}).get("simulator_processes_invoked") == 0,
        "R08 and R09 remain closed after their static-contract failures",
    )
    ...
    add_check(
        ...
        and all(
            "r07" in value or "r08" in value or "r09" in value or "r10" in value
            for value in reuse.get("forbidden_reused_paths", [])
        )
        and "results/compact/m01_xyce_build_preflight_r10" in reuse.get("forbidden_reused_paths", [])
    ...
    add_check(
        checks,
        "paths:r10_namespace_disjoint_from_historical_failures",
        all("r10" in value for value in output_values)
        and not any(
            value in {
        ...
        "R10 reserves only its own output namespace",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        "runner parses fixed-column .prn and writes only R10 paths",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        and "run_m01_xyce_build_preflight_r10.py" in checker_source,
    ...
    add_check(
        ...
        and re.search(
            r"^(?:from\s+run_m01_xyce_build_preflight_r10\s+import|import\s+run_m01_xyce_build_preflight_r10\b)",
            checker_source,
            re.MULTILINE,
        )
        is None,
    ...
    add_check(
        ...
        is None,
        "R10 static contract checker is read-only and process-free",
    )
    add_check(
        checks,
        "source:r10_namespace_isolated",
        all("r08" not in source.lower() for source in (runner_source, checker_source, common_source))
        and "M01_XYCE_BUILD_PREFLIGHT_R10" in runner_source
        and "M01_XYCE_BUILD_PREFLIGHT_R10" in checker_source,
        "R10 execution sources do not silently reuse R08 names",
    )
    add_check(
        checks,
        "source:makefile_r10_targets",
        "m01-xyce-build-preflight-r10-contract-check:" in (ROOT / "Makefile").read_text(encoding="utf-8")
        and "m01-xyce-build-preflight-r10:" in (ROOT / "Makefile").read_text(encoding="utf-8")
        and "m01-xyce-build-preflight-r10-check:" in (ROOT / "Makefile").read_text(encoding="utf-8"),
        "contract, runner and independent-check targets are registered",
    )
    add_check(
        checks,
        "project:next_scope_r10_contract",
    ...
    add_check(
        checks,
        "project:evidence_boundary_r10",
        "m01_xyce_build_preflight_r10_contract_boundary" in project.get("tcad_track", {})
        and "not a built simulator" in project["tcad_track"]["m01_xyce_build_preflight_r10_contract_boundary"],
        "project boundary records R10 as tool/output contract only",
    )
    add_check(
        checks,
        "registry:static_report_is_only_future_artifact",
        machine.get("contract_report_path") == "results/reports/m01_xyce_build_preflight_contract_r10.json"
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R10 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R10_CONTRACT_{report['status']} "
