#!/usr/bin/env python3
"""Independently check persisted R06 Xyce build/preflight evidence."""
...
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_r06.json"
CHECK_REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_check_r06.json"
...
R06_INDEPENDENT_CHECK_COUNT = 25
...
def check_preflight() -> dict[str, Any]:
    if CHECK_REPORT_PATH.exists():
        raise RuntimeError(f"Refusing to overwrite existing R06 independent report: {CHECK_REPORT_PATH}")
    ...
    add_check(
        ...
        "R06 is generator build, Xyce build, scalar self-test and parser-only evidence",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        "R06 configures only generators and Xyce against reused dependency prefixes",
    )
    ...
    add_check(
        ...
        and report.get("next_gate", "").startswith(
            "Run the R06 independent persisted-evidence check exactly once"
        ),
    ...
    add_check(
        ...
        "R05 failure is immutable and R06 Xyce uses new roots",
    )

    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(
            f"R06 independent registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}"
        )
    ...
    result = {
        ...
        if not failures
        else "Stop M01 and retain the failed R06 independent check without altering the registered gate.",
    }
    with CHECK_REPORT_PATH.open("x", encoding="utf-8") as stream:
        json.dump(result, stream, indent=2, ensure_ascii=True)
        stream.write("\n")
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R06_CHECK_{result['status']} "
