#!/usr/bin/env python3
"""Run the R11 Xyce output/parser recovery without rebuilding any tool."""
...
def check_preflight() -> dict[str, Any]:
    ...
    if report_path.exists() or independent_report_path.exists() or run_directory.exists():
        raise RuntimeError("R11 refuses to overwrite an existing report, check report or run directory")
    ...
    r10_failure_report = (
        load_json(r10_failure_report_path) if r10_failure_report_path.is_file() else {}
    )
    r10_static_report = (
        load_json(r10_static_report_path) if r10_static_report_path.is_file() else {}
    )
    r10_artifact_paths = [
    ...
    r10_artifacts_ok = all(
        (ROOT / relative).is_file() and sha256(ROOT / relative) == expected
        for relative, expected in r10_artifact_paths
    )
    ...
    add_check(
        checks,
        "binding:historical_failure_reports",
        ...
        == 1.25,
        "immutable R07 runner failure and R10 path-encoding failure are bound",
    )
    add_check(
        checks,
        "binding:historical_artifacts_hash_bound",
        ...
        and r10_artifacts_ok
        and tree_matches(r10_partial_directory, r10["partial_run_tree"]),
        f"r07={sum((ROOT / item[0]).is_file() for item in r07_artifact_paths)}/{len(r07_artifact_paths)} "
        f"r10={sum((ROOT / item[0]).is_file() for item in r10_artifact_paths)}/{len(r10_artifact_paths)}",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        "R11 reuses only complete R07 install prefixes",
    )
    add_check(
        checks,
        "paths:r11_exclusive_namespace",
        len(future_paths) == len(set(future_paths))
        and all(not path.exists() for path in future_paths)
        and str(run_directory).endswith("m01_xyce_build_preflight_r11"),
    ...
    preflight_log.write_text(
        "M01 Xyce output/parser recovery preflight R11\n"
        "build_invoked=false\n"
        ...
        "r10_failure_preserved=true\n"
        "r10_runner_rerun=false\n"
        "parser_include_path_mode=repository_relative_ascii\n"
    ...
    self_test_text = """* R11 controlled Xyce B-source scalar self-test; not a project device
Vctrl nin 0 1.0
...
"""
    ...
    candidate_relative_path = Path(config["device_syntax_check"]["candidate_path"])
    candidate_include_path = candidate_relative_path.as_posix()
    candidate = ROOT / candidate_relative_path
    device_syntax_text = (
        "* R11 parser-only frozen IGZO candidate; no numerical solve\n"
        f'.include "{candidate_include_path}"\n'
        "VDS D S 0.1\nVTG TG S 0.5\nVBG BG S 0\n"
    ...
    syntax_scope_ok = (
        ...
        and candidate_include_path == config["device_syntax_check"]["include_path"]
        and candidate_include_path.isascii()
        and not candidate_relative_path.is_absolute()
        ...
        and str(ROOT) not in device_syntax_text
        and all(
    ...
    if observed_v is not None and abs(observed_v - expected_v) <= tolerance_v:
        device_syntax_netlist.write_text(
            device_syntax_text,
            encoding=config["device_syntax_check"]["netlist_encoding"],
        )
    add_check(
        checks,
    ...
    reuse_manifest = {
        ...
        "r10_failure_archive": {
            "bound_commit": r10["bound_commit"],
            "static_contract_report": {
                "path": r10["static_contract_report_path"],
                "sha256": sha256(r10_static_report_path),
            },
            "runner_failure_report": {
                "path": r10["runner_failure_report_path"],
                "sha256": sha256(r10_failure_report_path),
            },
            "partial_run_directory": r10["partial_run_directory"],
            "partial_run_tree": digest_tree(r10_partial_directory),
            "runner_rerun": False,
        },
        "generator_prefix": {
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R11 preflight registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R11_{report['status']} "
