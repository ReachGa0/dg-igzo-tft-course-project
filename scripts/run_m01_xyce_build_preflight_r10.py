#!/usr/bin/env python3
"""Run the R10 Xyce output/parser recovery without rebuilding any tool."""
...
def check_preflight() -> dict[str, Any]:
    ...
    if report_path.exists() or independent_report_path.exists() or run_directory.exists():
        raise RuntimeError("R10 refuses to overwrite an existing report, check report or run directory")
    ...
    add_check(
        ...
        "R10 reuses only complete R07 install prefixes",
    )
    add_check(
        checks,
        "paths:r10_exclusive_namespace",
        len(future_paths) == len(set(future_paths))
        and all(not path.exists() for path in future_paths)
        and str(run_directory).endswith("m01_xyce_build_preflight_r10"),
    ...
    preflight_log.write_text(
        "M01 Xyce output/parser recovery preflight R10\n"
        "build_invoked=false\n"
    ...
    self_test_text = """* R10 controlled Xyce B-source scalar self-test; not a project device
Vctrl nin 0 1.0
...
"""
    ...
    device_syntax_text = (
        "* R10 parser-only frozen IGZO candidate; no numerical solve\n"
        f'.include "{candidate}"\n'
        "VDS D S 0.1\nVTG TG S 0.5\nVBG BG S 0\n"
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R10 preflight registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R10_{report['status']} "
