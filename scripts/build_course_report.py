"""Build the chart-complete, self-contained course report from frozen evidence."""
...
OUT = ROOT / "report" / "final" / "课程报告.html"
...
def page(number: int, title: str, body: str, image_name: str | None = None, alt: str = "") -> str:
    ...
    extras = "".join(img(name, caption) for name, caption in EXTRA_FIGURES.get(number, []))
    return f'<section class="page"><div class="page-no">{number:02d}</div><h2>{title}</h2>{body}{figure}{extras}</section>'


EXTRA_FIGURES = {
    5: [("tcad_t01_dc_extraction.png", "T01 单栅受限提取"), ("tcad_t01_dc_state_maps.png", "T01 单栅二维内部状态")],
    6: [("tcad_t02_b_minimal_bias.png", "T02-B 最小正向顶栅族"), ("tcad_t02_c_state_maps.png", "T02-C 双栅二维状态")],
    7: [("tcad_t03_p1_bias_state_maps.png", "T03-P1 偏压敏感性的二维状态"), ("tcad_t03_p1_cap_ratio_sensitivity.png", "T03-P1 有效电容分配比敏感性"), ("tcad_t03_p1_cap_ratio_state_maps.png", "T03-P1 电容分配比二维状态")],
    8: [("tcad_t03_p2_dit_formal_states.png", "T03-P2 DIT 正式敏感性的二维状态")],
    9: [("tcad_t03_p2_bulk_traps_formal_v3_states.png", "T03-P2 NTA/NGA 正式 V3 二维状态")],
    10: [("tcad_t03_p3_contact_v2_states.png", "T03-P3 V2 接触电阻二维状态")],
    11: [("tcad_t03_p4_l_sensitivity.png", "T03-P4 沟道长度敏感性"), ("tcad_t03_p4_l_state_maps.png", "T03-P4 沟道长度二维状态"), ("tcad_t03_p5_temperature_states.png", "T03-P5 温度敏感性的二维状态")],
    13: [("m00_compact_model_residuals_r02.png", "M00 R02 教学紧凑模型残差")],
    14: [("m01_open_source_route_difference_r03.png", "M01 R03 两开源路线差异")],
}
...
pages = [
    ...
    page(19, "复现、交付与人工核对", "<p>提交包包含配置、脚本、原始/处理结果、完整审计报告、精简报告、PPTX 和 Git 哈希。总检查命令为 <code>make check</code>，报告结构检查为 <code>make report-check</code>。正式 runner 均有唯一运行约束，不因复现便利而重跑。</p><p>提交前人工核对完整报告封面的姓名、学号/班级和指导教师，并确认 PDF/PPT 字体和页面排版。</p>"),
    page(20, "最终结论", "<p>项目以可复现的二维 IGZO 教学模型完成了器件与模型主线，并以完整双路线证据得到 C00 有源负载锚点不合格的负结论。这个结论比未经验证的后续逻辑或版图占位更可靠。</p><div class=boundary><strong>最终边界：</strong>本项目不主张实验校准、真实器件物理参数、原生 Level 61、可流片 PDK、合格逻辑单元或 HZO 结果。任何 C00 恢复必须在新合同中重新冻结设计变量，不能回写 R04。</div><p class=note>提交版本：9af6e4c。完整报告保留全部审计细节；本精简报告用于教师连续阅读。</p>"),
]
...
html = html.replace("双栅 IGZO TFT 课程项目精简报告", "双栅 IGZO TFT 课程报告")
html = html.replace("本精简报告", "本课程报告")
html = html.replace(
    "</style>",
    """.page{min-height:0;padding:8px 0 32px;break-after:auto;page-break-after:auto;break-inside:auto;page-break-inside:auto}.page:last-child{break-after:auto;page-break-after:auto}h2{break-after:avoid;page-break-after:avoid}table,.boundary,pre{break-inside:avoid;page-break-inside:avoid}img{max-height:470px}@media(max-width:720px){.page{min-height:0}}@media print{.page{min-height:0;padding:5mm 0 8mm}.page-no{position:static;text-align:right;margin-bottom:2mm}figure{margin:5mm 0}img{max-height:122mm}}</style>""",
)
