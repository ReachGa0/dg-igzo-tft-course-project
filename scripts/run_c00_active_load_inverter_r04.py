#!/usr/bin/env python3
"""Run the formal C00 R04 ngspice/Xyce inverter analyses exactly once."""
...
CONTRACT_CHECKER_PATH = ROOT / "scripts" / "check_c00_active_load_inverter_r04_contract.py"
...
CHECK_NAMES = [
    ...
    "git:head_origin_synced_and_static_snapshot_published",
    ...
    "raw:four_native_axes_cover_registered_domains",
...
def run() -> int:
    ...
    native_axis_summaries: dict[str, dict[str, Any]] = {}
    ...
    try:
        ...
        static_git_snapshot = static_report.get("git_state", {})
        static_head = static_git_snapshot.get("head")
        static_origin = static_git_snapshot.get("origin_main")
        git_ok = (
            _is_commit_id(head)
            and head == origin
            and _is_commit_id(static_head)
            and static_head == static_origin
            and static_git_snapshot.get("synchronized") is True
            and static_head != head
            and machine.get("contract_report_sha256") == sha256(static_report_path)
        )
        _set(
            checks,
            "git:head_origin_synced_and_static_snapshot_published",
            git_ok,
            f"head={head} origin={origin} static_snapshot={static_head}",
        )
        if not git_ok:
            raise CircuitRunFailure("static PASS report is not registered in a synchronized committed state")
        ...
        vector_sets = {
            "ngspice_dc": ng_dc_vectors,
            "ngspice_transient": ng_tran_vectors,
            "xyce_dc": xy_dc_vectors,
            "xyce_transient": xy_tran_vectors,
        }
        native_axis_summaries = {
            "ngspice_dc": native_axis_summary(
                ng_dc_vectors,
                ("v(norm)", "norm"),
                float(sweep["dc_normalized_input_start"]),
                float(sweep["dc_normalized_input_stop"]),
            ),
            "xyce_dc": native_axis_summary(
                xy_dc_vectors,
                ("v(norm)", "norm"),
                float(sweep["dc_normalized_input_start"]),
                float(sweep["dc_normalized_input_stop"]),
            ),
            "ngspice_transient": native_axis_summary(
                ng_tran_vectors,
                ("time",),
                0.0,
                float(sweep["transient_stop_s"]),
            ),
            "xyce_transient": native_axis_summary(
                xy_tran_vectors,
                ("time",),
                0.0,
                float(sweep["transient_stop_s"]),
        ...
        native_points = {
            ...
            for name, summary in native_axis_summaries.items()
        }
        raw_ok = set(native_axis_summaries) == set(vector_sets) and all(
            summary["valid"] for summary in native_axis_summaries.values()
        )
        _set(
            checks,
            "raw:four_native_axes_cover_registered_domains",
            raw_ok,
            json.dumps(native_axis_summaries, sort_keys=True),
        )
        if not raw_ok:
            raise CircuitRunFailure("native axis coverage or cardinality failed")
    ...
    if status == "FAIL" and failure_category is None:
        failed_names = [item["name"] for item in ordered_checks if item["status"] == "FAIL"]
        failure_category = "AcceptanceFailure"
        failure_detail = "registered checks failed: " + ", ".join(failed_names)
    report = {
        "schema_version": "1.0",
        ...
        "git_state": {
            "head": head,
            "origin_main": origin,
            "synchronized": head == origin and _is_commit_id(head),
        },
        "summary": {
            "passed": passed,
            "failed": failed,
        ...
        "native_axis_summaries": native_axis_summaries,
        ...
        "evidence_boundary": config["evidence_boundary"][
            "future_runner_allowed_claim" if status == "PASS" else "runner_failure_allowed_claim"
        ],
        "next_gate": (
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R04_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={report_path}")
