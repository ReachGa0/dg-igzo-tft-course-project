#!/usr/bin/env python3
"""Independently validate persisted M00 R02 compact-model fit evidence."""
...
def check_fit(config_path: Path) -> dict[str, Any]:
    ...
    add_check(
        ...
        and fit_report.get("formal_fit_revision") == 2
    ...
    runner_path = ROOT / "models" / "fit_m00_teaching_compact_r02.py"
    ...
    add_check(
        ...
        == sha256(ROOT / config["dataset_contract"]["registry"])
        and snapshot.get("formal_run_revision") == 2
        and snapshot.get("formal_run_ordinal") == 1
        and snapshot.get("command") == "make m00-compact-model-r02-fit"
    ...
    add_check(
        ...
        and config.get("reference_kernel", {}).get("kernel_id")
        == "IGZO_DG_TEACHING_KERNEL_R02"
        and config.get("optimization_contract", {}).get(
        ...
        == 1
        and config.get("optimization_contract", {}).get(
            "r01_holdout_metrics_or_fitted_parameters_used"
        )
        is False
        and "length_exponent" not in {
            item["name"] for item in config["parameter_contract"]
        }
    ...
    add_check(
        ...
        and ".subckt igzo_dg_behavioral_r02" in ngspice_text.lower()
        and "(10/lum)" in ngspice_text.lower()
        and "m00_pl" not in ngspice_text.lower()
        and "level=15" in aimspice_text.lower()
        and ".model igzo_n_l15_r02" in aimspice_text.lower()
        and ".subckt igzo_dg_level15_r02" in aimspice_text.lower()
        and "tox=10n" in aimspice_text.lower()
        and "physical al2o3 thickness is 30 nm" in aimspice_text.lower()
        and mapping.get("mapping_id") == "IGZO_M00_TO_AIMSPICE_LEVEL15_R02"
        and mapping.get("fixed_teaching_inputs", {}).get(
            "fixed_length_geometry_exponent"
        )
        == 1.0
    ...
    add_check(
        ...
        checker_imports.isdisjoint(
            {
                "numpy", "scipy", "devsim", "subprocess",
                "fit_m00_teaching_compact", "fit_m00_teaching_compact_r02",
            }
        )
    ...
    report = {
        ...
        "m00_completion": {
            "status": "PASS" if not failures else "FAIL",
            "complete_m00_r02_reference_kernel_fit": not failures,
        ...
        "next_gate": "After documentation and commit, establish the M01 simulator cross-check contract; no simulator or circuit evidence is created by this R02 check.",
    }
    return report
...
def main() -> int:
    ...
    print(
        f"M00_COMPACT_MODEL_R02_FIT_CHECK_{report['status']} "
        f"checks={sum(item.get('status') == 'PASS' for item in report.get('checks', []))}/"
        f"{len(report.get('checks', []))} report={report_path}"
    )
    for failure in report.get("failures", []):
        print(
            f"M00_COMPACT_MODEL_R02_FIT_CHECK_ERROR {failure['name']}: {failure['detail']}",
            file=sys.stderr,
        )
