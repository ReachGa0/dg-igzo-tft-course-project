#!/usr/bin/env python3
"""Run the R08 Xyce output/parser recovery without rebuilding any tool."""
...
def check_preflight() -> dict[str, Any]:
    ...
    if report_path.exists() or independent_report_path.exists() or run_directory.exists():
        raise RuntimeError("R08 refuses to overwrite an existing report, check report or run directory")
    ...
    add_check(
        ...
        and r07["bound_commit"] == "9a7375e0b7b5f865e2e0cae1c5fd45a7f1a9e7e9"
    ...
    add_check(
        ...
        "R08 reuses only complete R07 install prefixes",
    )
    add_check(
        checks,
        "paths:r08_exclusive_namespace",
        len(future_paths) == len(set(future_paths))
        and all(not path.exists() for path in future_paths)
        and str(run_directory).endswith("m01_xyce_build_preflight_r08"),
    ...
    preflight_log.write_text(
        "M01 Xyce output/parser recovery preflight R08\n"
        "build_invoked=false\n"
    ...
    self_test_text = """* R08 controlled Xyce B-source scalar self-test; not a project device
Vctrl nin 0 1.0
...
"""
    ...
    device_syntax_text = (
        "* R08 parser-only frozen IGZO candidate; no numerical solve\n"
        f'.include "{candidate}"\n'
        "VDS D S 0.1\nVTG TG S 0.5\nVBG BG S 0\n"
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R08 preflight registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R08_{report['status']} "
