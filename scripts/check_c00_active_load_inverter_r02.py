#!/usr/bin/env python3
"""Independently verify persisted C00 R02 evidence without a simulator process."""
...
REPORT_PATH = ROOT / "results" / "reports" / "c00_active_load_inverter_r02.json"
CHECK_PATH = ROOT / "results" / "reports" / "c00_active_load_inverter_r02_check.json"
...
def generate_netlist(config: dict[str, Any], route: str, analysis: str) -> str:
    ...
    sweep = config["sweep_contract"]
    lines = [f"* C00 R02 {route} IGZO-only active-load inverter {'DC' if analysis == 'dc' else 'transient'}", config["netlist_contract"]["candidate_include"], ".option numdgt=17"]
    if analysis == "dc":
...
def check() -> int:
    ...
    machine = c00["active_load_inverter_r02"]
    ...
    add_check(checks, "independence:no_subprocess_common_or_runner_import", not {"subprocess", "c00_active_load_inverter_r02_common", "run_c00_active_load_inverter_r02"} & imported_modules, "standard-library independent implementation")
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R02_CHECK_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={CHECK_PATH}")
