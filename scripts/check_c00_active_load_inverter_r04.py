#!/usr/bin/env python3
"""Independently verify persisted C00 R04 evidence without a simulator process."""
...
REPORT_PATH = ROOT / "results" / "reports" / "c00_active_load_inverter_r04.json"
CHECK_PATH = ROOT / "results" / "reports" / "c00_active_load_inverter_r04_check.json"
...
def generate_netlist(config: dict[str, Any], route: str, analysis: str) -> str:
    ...
    sweep = config["sweep_contract"]
    lines = [f"* C00 R04 {route} IGZO-only active-load inverter {'DC' if analysis == 'dc' else 'transient'}", config["netlist_contract"]["candidate_include"], ".option numdgt=17"]
    if analysis == "dc":
    ...
    for index, line in enumerate(lines):
        ...
        for name in names:
            ...
            if occurrences[name] > 1:
                duplicates[name].append(key)
        if any(name != "time" for name in duplicates):
            raise ValueError("Xyce duplicate non-TIME header")
        ...
        if columns:
            for duplicate in duplicates.get("time", []):
...
def axis_summary(
    ...
    axis = next((vectors[name] for name in axis_names if name in vectors), None)
    if axis is None:
        raise ValueError("registered axis missing")
    lengths = {len(values) for values in vectors.values()}
    tolerance = max(1e-15, abs(stop) * 1e-10)
    summary = {
        "point_count": len(axis),
        "vector_count": len(vectors),
        "all_vectors_same_length": len(lengths) == 1,
        "all_values_finite": all(
            math.isfinite(value) for values in vectors.values() for value in values
        ),
        "axis_strictly_increasing": len(axis) >= 2
        and all(right > left for left, right in zip(axis, axis[1:])),
        "covers_registered_start": bool(axis) and axis[0] <= start + tolerance,
        "covers_registered_stop": bool(axis) and axis[-1] >= stop - tolerance,
    }
    summary["valid"] = all(
        summary[key]
        for key in (
            "all_vectors_same_length",
            "all_values_finite",
            "axis_strictly_increasing",
            "covers_registered_start",
            "covers_registered_stop",
        )
    )
    return summary
...
def check() -> int:
    ...
    machine = c00["active_load_inverter_r04"]
    ...
    add_check(checks, "independence:no_subprocess_common_or_runner_import", not {"subprocess", "c00_active_load_inverter_r04_common", "run_c00_active_load_inverter_r04"} & imported_modules, "standard-library independent implementation")
    ...
    sweep = config["sweep_contract"]
    native_summaries = {
        "ngspice_dc": axis_summary(vectors["ngspice_dc"], ("v(norm)", "norm"), sweep["dc_normalized_input_start"], sweep["dc_normalized_input_stop"]),
        "xyce_dc": axis_summary(vectors["xyce_dc"], ("v(norm)", "norm"), sweep["dc_normalized_input_start"], sweep["dc_normalized_input_stop"]),
        "ngspice_transient": axis_summary(vectors["ngspice_transient"], ("time",), 0.0, sweep["transient_stop_s"]),
        "xyce_transient": axis_summary(vectors["xyce_transient"], ("time",), 0.0, sweep["transient_stop_s"]),
    }
    native = {key: value["point_count"] for key, value in native_summaries.items()}
    add_check(checks, "raw:four_independent_parsers_complete", native == report.get("native_points") and native_summaries == report.get("native_axis_summaries") and all(value["valid"] for value in native_summaries.values()), json.dumps(native_summaries, sort_keys=True))
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R04_CHECK_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={CHECK_PATH}")
