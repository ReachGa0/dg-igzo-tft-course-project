...
REQUIRED_FILES = [
    ...
    "results/reports/m01_xyce_build_preflight_r11.json",
    "results/reports/m01_xyce_build_preflight_check_r11.json",
    "results/compact/m01_xyce_build_preflight_r11/preflight.log",
    "results/compact/m01_xyce_build_preflight_r11/reuse_manifest.json",
    "results/compact/m01_xyce_build_preflight_r11/execution_manifest.json",
    "results/compact/m01_xyce_build_preflight_r11/bsource_self_test.cir",
    "results/compact/m01_xyce_build_preflight_r11/bsource_self_test.log",
    "results/compact/m01_xyce_build_preflight_r11/bsource_self_test.prn",
    "results/compact/m01_xyce_build_preflight_r11/xyce_version.log",
    "results/compact/m01_xyce_build_preflight_r11/xyce_license.log",
    "results/compact/m01_xyce_build_preflight_r11/xyce_bsource_self_test.log",
    "results/compact/m01_xyce_build_preflight_r11/device_syntax.cir",
    "results/compact/m01_xyce_build_preflight_r11/device_syntax.log",
    "results/compact/m01_xyce_build_preflight_r11/xyce_device_syntax.log",
    "config/m01_open_source_device_dc_r01.json",
    "scripts/m01_open_source_device_dc_r01_common.py",
    "scripts/check_m01_open_source_device_dc_r01_contract.py",
    ...
    "scripts/run_m01_route_divergence_root_cause_r01.py",
    "scripts/check_m01_route_divergence_root_cause_r01.py",
    "results/reports/project_check_m01_route_divergence_r01_implementation_binding_failed.json",
    "results/reports/project_check_m01_route_divergence_r01_implementation_source_scan_failed.json",
...
def main() -> int:
    ...
    try:
        ...
        r03_runner_scope_active = current_next_scope.startswith(
            ...
            "execute the committed C00 active-load inverter revision-1 four-process runner"
        ) or current_next_scope.startswith(
            ...
            "run the 50-check static contract for C00 active-load inverter revision-3"
        ) or current_next_scope.startswith(
        ...
        r08_scope_active = current_next_scope.startswith(
            ...
            "run independent persisted-evidence checker for M01 Xyce build/tool preflight revision-11"
        ) or current_next_scope.startswith(
            "establish and commit a formal M01 two-route device-only DC execution contract"
        ) or current_next_scope.startswith(
    ...
    try:
        ...
        add_check(
            ...
            and (c00_experiment.get("status"), c00_experiment.get("current_evidence"))
            in {
                ("contract_planning_open", "E0"),
    ...
    try:
        ...
        r08_next_scope_valid = (
            ...
            r08_failed_state and r08_scope_active
        )
    ...
    try:
        ...
        r09_next_scope_valid = (
            ...
            r09_failed_state and r08_scope_active
        )
        add_check(
            checks,
    ...
    try:
        ...
        r10_next_scope_valid = (
            ...
            r10_failed_state and r08_scope_active
        )
        add_check(
            checks,
    ...
    try:
        ...
        m01_xyce_r11_runner_report_path = ROOT / r11_outputs["preflight_report"]
        r11_runner_report = (
            json.loads(m01_xyce_r11_runner_report_path.read_text(encoding="utf-8"))
            if m01_xyce_r11_runner_report_path.is_file()
            else {}
        )
        r11_runner_summary = r11_runner_report.get("summary", {})
        r11_runner_output_paths = [
            ROOT / value
            for key, value in r11_outputs.items()
            if key
            not in {
                "contract_report",
                "run_directory",
        ...
        r11_runner_pass_state = (
            r11_static_pass
            and r11_machine.get("status") == "preflight_passed"
            and r11_machine.get("revision") == 11
            and r11_machine.get("current_evidence") == "E2"
            and r11_machine.get("contract_check_completed") is True
            and r11_machine.get("contract_status") == "PASS"
            and r11_machine.get("contract_checks_passed") == 36
            and r11_machine.get("contract_checks_failed") == 0
            and r11_machine.get("preflight_run_completed") is True
            and r11_machine.get("preflight_run_ordinal") == 1
            and r11_machine.get("preflight_status") == "PASS"
            and r11_machine.get("runner_checks_passed") == 32
            and r11_machine.get("runner_checks_failed") == 0
            and r11_machine.get("process_invocations") == 4
            and r11_machine.get("device_syntax_only_invoked") is True
            and r11_machine.get("formal_device_dc_invoked") is False
            and r11_machine.get("observed_self_test_value_v") == 1.25
            and r11_machine.get("result_paths")
            == [
                "results/reports/m01_xyce_build_preflight_contract_r11.json",
                "results/reports/m01_xyce_build_preflight_r11.json",
            ...
            and r11_machine.get("artifact_hashes", {}).get("preflight_report_sha256")
            == sha256(m01_xyce_r11_runner_report_path)
            and r11_runner_report.get("status") == "PASS"
            and r11_runner_report.get("preflight_status") == "PASS"
            and r11_runner_report.get("evidence_level") == "E2"
            and len(r11_runner_report.get("checks", [])) == 32
            and all(item.get("status") == "PASS" for item in r11_runner_report.get("checks", []))
            and r11_runner_report.get("config", {}).get("sha256")
            == sha256(m01_xyce_r11_config_path)
            and r11_runner_report.get("runner", {}).get("sha256")
            == sha256(m01_xyce_r11_runner_path)
            and r11_runner_summary.get("process_invocations") == 4
            and r11_runner_summary.get("build_processes_invoked") == 0
            and r11_runner_summary.get("generator_build_invoked") is False
            and r11_runner_summary.get("dependency_rebuild_invoked") is False
            and r11_runner_summary.get("ngspice_invoked") is False
            and r11_runner_summary.get("aimspice_invoked") is False
            and r11_runner_summary.get("controlled_bsource_self_test_invoked") is True
            and r11_runner_summary.get("device_syntax_only_invoked") is True
            and r11_runner_summary.get("formal_device_dc_invoked") is False
            and r11_runner_summary.get("formal_m01_outputs_created") is False
            and r11_runner_summary.get("r10_failure_preserved") is True
            and r11_runner_summary.get("r10_runner_rerun") is False
            and r11_runner_summary.get("parser_include_path_mode") == "repository_relative_ascii"
            and {item.get("name") for item in r11_runner_report.get("commands", [])}
            == {"xyce_version", "xyce_license", "xyce_bsource_self_test", "xyce_device_syntax"}
            and len(r11_runner_report.get("commands", [])) == 4
            and all(item.get("returncode") == 0 for item in r11_runner_report.get("commands", []))
            and all(path.is_file() for path in r11_runner_output_paths)
            and (ROOT / r11_outputs["run_directory"]).is_dir()
            and not (ROOT / r11_outputs["device_syntax_output"]).exists()
            and not (ROOT / r11_outputs["independent_check_report"]).exists()
        )
        m01_xyce_r11_independent_report_path = (
            ROOT / r11_outputs["independent_check_report"]
        )
        r11_independent_report = (
            json.loads(
                m01_xyce_r11_independent_report_path.read_text(encoding="utf-8")
            )
            if m01_xyce_r11_independent_report_path.is_file()
            else {}
        )
        r11_verified_result_paths = [
            "results/reports/m01_xyce_build_preflight_contract_r11.json",
            "results/reports/m01_xyce_build_preflight_r11.json",
            "results/compact/m01_xyce_build_preflight_r11/preflight.log",
            "results/compact/m01_xyce_build_preflight_r11/reuse_manifest.json",
        ...
        r11_verified_state = (
            r11_static_pass
            and r11_machine.get("status") == "preflight_verified"
            and r11_machine.get("revision") == 11
            and r11_machine.get("current_evidence") == "E3"
            and r11_machine.get("contract_check_completed") is True
            and r11_machine.get("contract_status") == "PASS"
            ...
            and r11_machine.get("runner_checks_failed") == 0
            and r11_machine.get("process_invocations") == 4
            and r11_machine.get("independent_check_run") is True
            and r11_machine.get("independent_check_status") == "PASS"
            and r11_machine.get("independent_checks_passed") == 25
            and r11_machine.get("independent_checks_failed") == 0
            and r11_machine.get("independent_check_processes_invoked") == 0
            and r11_machine.get("device_syntax_only_invoked") is True
            and r11_machine.get("formal_device_dc_invoked") is False
            and r11_machine.get("observed_self_test_value_v") == 1.25
            and r11_machine.get("result_paths") == r11_verified_result_paths
            and r11_machine.get("artifact_hashes", {}).get(
                "preflight_report_sha256"
            )
            == sha256(m01_xyce_r11_runner_report_path)
            and r11_machine.get("artifact_hashes", {}).get(
                "independent_check_report_sha256"
            )
            == sha256(m01_xyce_r11_independent_report_path)
            and r11_runner_report.get("status") == "PASS"
            and r11_runner_report.get("preflight_status") == "PASS"
            and r11_runner_report.get("evidence_level") == "E2"
            and len(r11_runner_report.get("checks", [])) == 32
            and all(
                item.get("status") == "PASS"
                for item in r11_runner_report.get("checks", [])
            )
            and r11_runner_report.get("config", {}).get("sha256")
            == sha256(m01_xyce_r11_config_path)
            and r11_runner_report.get("runner", {}).get("sha256")
            ...
            and all(
                ...
                for item in r11_runner_report.get("commands", [])
            )
            and r11_independent_report.get("status") == "PASS"
            and r11_independent_report.get("evidence_level") == "E3"
            and r11_independent_report.get("preflight_id")
            == "M01_XYCE_BUILD_PREFLIGHT_R11"
            and len(r11_independent_report.get("checks", [])) == 25
            and all(
                item.get("status") == "PASS"
                for item in r11_independent_report.get("checks", [])
            )
            and r11_independent_report.get("summary", {}).get("check_count") == 25
            and r11_independent_report.get("summary", {}).get("passed") == 25
            and r11_independent_report.get("summary", {}).get("failed") == 0
            and r11_independent_report.get("config", {}).get("sha256")
            == sha256(m01_xyce_r11_config_path)
            and r11_independent_report.get("checker", {}).get("sha256")
            == sha256(m01_xyce_r11_checker_path)
            and r11_independent_report.get("runner_report", {}).get("sha256")
            == sha256(m01_xyce_r11_runner_report_path)
            and r11_independent_report.get("observed_self_test_value_v") == 1.25
            and r11_independent_report.get("next_gate")
            == "Formal M01 remains closed; this report is tool/parser evidence only."
            and all(path.is_file() for path in r11_runner_output_paths)
            and (ROOT / r11_outputs["run_directory"]).is_dir()
            and not (ROOT / r11_outputs["device_syntax_output"]).exists()
            and m01_xyce_r11_independent_report_path.is_file()
        )
        ...
        add_check(
            ...
            and (
                r11_machine_planned
                or r11_ready_state
                or r11_runner_pass_state
                or r11_verified_state
            )
            and (not r11_contract_exists if r11_machine_planned else r11_contract_exists)
            and (
                (r11_machine_planned or r11_ready_state)
                and all(not path.exists() for path in r11_future_paths)
                or r11_runner_pass_state
                and all(path.exists() for path in r11_runner_output_paths)
                and (ROOT / r11_outputs["run_directory"]).is_dir()
                and not (ROOT / r11_outputs["device_syntax_output"]).exists()
                and not (ROOT / r11_outputs["independent_check_report"]).exists()
                or r11_verified_state
                ...
                and (ROOT / r11_outputs["run_directory"]).is_dir()
                and not (ROOT / r11_outputs["device_syntax_output"]).exists()
                and (ROOT / r11_outputs["independent_check_report"]).is_file()
            )
            ...
            and r11_machine.get("simulator_processes_invoked")
            == (4 if r11_runner_pass_state or r11_verified_state else 0)
            ...
            f"runner={r11_runner_pass_state} verified={r11_verified_state} "
    ...
    m01_device_dc_config_path = ROOT / "config" / "m01_open_source_device_dc_r01.json"
    m01_device_dc_common_path = ROOT / "scripts" / "m01_open_source_device_dc_r01_common.py"
    m01_device_dc_contract_checker_path = (
        ROOT / "scripts" / "check_m01_open_source_device_dc_r01_contract.py"
    )
    m01_device_dc_runner_path = ROOT / "scripts" / "run_m01_open_source_device_dc_r01.py"
    m01_device_dc_independent_path = (
        ROOT / "scripts" / "check_m01_open_source_device_dc_r01.py"
    )
    try:
        device_dc_config = json.loads(m01_device_dc_config_path.read_text(encoding="utf-8"))
        device_dc_machine = experiment_map["M01"].get("open_source_device_dc_r01", {})
        device_dc_outputs = device_dc_config["outputs"]
        device_dc_contract_report_path = ROOT / device_dc_outputs["contract_report"]
        device_dc_run_report_path = ROOT / device_dc_outputs["run_report"]
        device_dc_independent_report_path = ROOT / device_dc_outputs["independent_check_report"]
        device_dc_run_directory = ROOT / device_dc_outputs["run_directory"]
        device_dc_contract_report = (
            json.loads(device_dc_contract_report_path.read_text(encoding="utf-8"))
            if device_dc_contract_report_path.is_file()
            else {}
        )
        device_dc_run_report = (
            json.loads(device_dc_run_report_path.read_text(encoding="utf-8"))
            if device_dc_run_report_path.is_file()
            else {}
        )
        device_dc_independent_report = (
            json.loads(device_dc_independent_report_path.read_text(encoding="utf-8"))
            if device_dc_independent_report_path.is_file()
            else {}
        )
        device_dc_source_paths = [
            "config/m01_open_source_device_dc_r01.json",
            "scripts/m01_open_source_device_dc_r01_common.py",
            "scripts/check_m01_open_source_device_dc_r01_contract.py",
            "scripts/run_m01_open_source_device_dc_r01.py",
        ...
        device_dc_all_output_paths = [ROOT / value for value in device_dc_outputs.values()]
        device_dc_run_output_paths = [
            ROOT / device_dc_outputs[key] for key in device_dc_run_artifact_keys
        ]
        device_dc_contract_pass = (
            device_dc_contract_report.get("status") == "PASS"
            and device_dc_contract_report.get("evidence_level") == "E3"
            and device_dc_contract_report.get("simulation_status")
            == "NOT_RUN_BY_CONTRACT_CHECK"
            and device_dc_contract_report.get("summary", {}).get("passed") == 40
            and device_dc_contract_report.get("summary", {}).get("failed") == 0
            ...
            is False
            and device_dc_contract_report.get("config", {}).get("sha256")
            == sha256(m01_device_dc_config_path)
            and device_dc_contract_report.get("contract_checker", {}).get("sha256")
            == sha256(m01_device_dc_contract_checker_path)
            and device_dc_contract_report.get("runner", {}).get("sha256")
            == sha256(m01_device_dc_runner_path)
            and device_dc_contract_report.get("independent_checker", {}).get("sha256")
            == sha256(m01_device_dc_independent_path)
            and len(device_dc_contract_report.get("checks", [])) == 40
            and all(
                item.get("status") == "PASS"
        ...
        device_dc_run_pass = (
            device_dc_contract_pass
            and device_dc_run_report.get("status") == "PASS"
            and device_dc_run_report.get("evidence_level") == "E2"
            and device_dc_run_report.get("simulation_status")
            == "FORMAL_DEVICE_DC_COMPLETE"
            and device_dc_run_report.get("summary", {}).get("passed") == 30
            and device_dc_run_report.get("summary", {}).get("failed") == 0
            and device_dc_run_report.get("summary", {}).get("total") == 30
            and device_dc_run_report.get("summary", {}).get("process_invocations") == 2
            and device_dc_run_report.get("summary", {}).get("ngspice_invoked") is True
            and device_dc_run_report.get("summary", {}).get("xyce_invoked") is True
            and device_dc_run_report.get("summary", {}).get("aimspice_invoked") is False
            and device_dc_run_report.get("summary", {}).get("tcad_invoked") is False
            and device_dc_run_report.get("summary", {}).get(
                "circuit_or_downstream_invoked"
            )
            is False
            and len(device_dc_run_report.get("checks", [])) == 30
            and all(
                item.get("status") == "PASS"
                for item in device_dc_run_report.get("checks", [])
            )
            and device_dc_run_report.get("config", {}).get("sha256")
            == sha256(m01_device_dc_config_path)
            and device_dc_run_report.get("runner", {}).get("sha256")
            == sha256(m01_device_dc_runner_path)
        )
        device_dc_verified = (
            device_dc_run_pass
            and device_dc_independent_report.get("status") == "PASS"
            and device_dc_independent_report.get("evidence_level") == "E3"
            and device_dc_independent_report.get("summary", {}).get("passed") == 24
            and device_dc_independent_report.get("summary", {}).get("failed") == 0
            and device_dc_independent_report.get("summary", {}).get("total") == 24
            and device_dc_independent_report.get("processes_invoked") == 0
            and len(device_dc_independent_report.get("checks", [])) == 24
            and all(
                item.get("status") == "PASS"
                for item in device_dc_independent_report.get("checks", [])
            )
            and device_dc_independent_report.get("config", {}).get("sha256")
            == sha256(m01_device_dc_config_path)
            and device_dc_independent_report.get("runner", {}).get("sha256")
            == sha256(m01_device_dc_runner_path)
            and device_dc_independent_report.get("runner_report", {}).get("sha256")
            == sha256(m01_device_dc_run_report_path)
        )
        device_dc_implemented_state = (
            device_dc_machine.get("status") == "contract_implemented"
            and device_dc_machine.get("current_evidence") == "E0"
            and device_dc_machine.get("contract_check_completed") is False
            and device_dc_machine.get("contract_status") == "NOT_RUN"
            ...
            and device_dc_machine.get("independent_check_status") == "NOT_RUN"
        ...
        device_dc_ready_state = (
            ...
            and device_dc_machine.get("current_evidence") == "E3"
            and device_dc_machine.get("contract_check_completed") is True
            and device_dc_machine.get("contract_status") == "PASS"
            and device_dc_machine.get("contract_checks_passed") == 40
            and device_dc_machine.get("contract_checks_failed") == 0
            and device_dc_machine.get("formal_run_completed") is False
            and device_dc_machine.get("runner_processes_invoked") == 0
            and device_dc_machine.get("result_paths")
            == device_dc_source_paths + [device_dc_outputs["contract_report"]]
        ...
        device_dc_runner_state = (
            device_dc_run_pass
            and device_dc_machine.get("status") == "formal_run_passed"
            and device_dc_machine.get("current_evidence") == "E2"
            and device_dc_machine.get("contract_check_completed") is True
            and device_dc_machine.get("contract_status") == "PASS"
            and device_dc_machine.get("formal_run_completed") is True
            and device_dc_machine.get("formal_run_status") == "PASS"
            and device_dc_machine.get("runner_checks_passed") == 30
            and device_dc_machine.get("runner_checks_failed") == 0
            and device_dc_machine.get("runner_processes_invoked") == 2
            and device_dc_machine.get("ngspice_invoked") is True
            and device_dc_machine.get("xyce_invoked") is True
            and device_dc_machine.get("independent_check_completed") is False
            and device_dc_run_directory.is_dir()
            and all(path.is_file() for path in device_dc_run_output_paths)
            and not device_dc_independent_report_path.exists()
        )
        device_dc_verified_state = (
            device_dc_verified
            and device_dc_machine.get("status") == "verified"
            and device_dc_machine.get("current_evidence") == "E3"
            and device_dc_machine.get("formal_run_completed") is True
            and device_dc_machine.get("formal_run_status") == "PASS"
            and device_dc_machine.get("independent_check_completed") is True
            and device_dc_machine.get("independent_check_status") == "PASS"
            and device_dc_machine.get("independent_checks_passed") == 24
            and device_dc_machine.get("independent_checks_failed") == 0
            and device_dc_machine.get("independent_processes_invoked") == 0
            and device_dc_run_directory.is_dir()
            and all(path.is_file() for path in device_dc_run_output_paths)
            and device_dc_independent_report_path.is_file()
        )
        device_dc_common_source = m01_device_dc_common_path.read_text(encoding="ascii")
        device_dc_contract_source = m01_device_dc_contract_checker_path.read_text(
            encoding="ascii"
        )
        device_dc_runner_source = m01_device_dc_runner_path.read_text(encoding="ascii")
        device_dc_independent_source = m01_device_dc_independent_path.read_text(
            encoding="ascii"
        )
        target = device_dc_config["target_contract"]
        ...
        candidate = ROOT / device_dc_config["device_contract"]["candidate_path"]
        preflight = device_dc_config["committed_preflight_binding"]
        preflight_artifacts = [
            (preflight["r11_config_path"], preflight["r11_config_sha256"]),
            (preflight["r11_contract_report_path"], preflight["r11_contract_report_sha256"]),
            (preflight["r11_runner_report_path"], preflight["r11_runner_report_sha256"]),
            (
                preflight["r11_independent_report_path"],
                preflight["r11_independent_report_sha256"],
            ),
        ]
        ng_tool = Path(device_dc_config["routes"]["ngspice"]["tool_path"])
        xyce_tool = Path(device_dc_config["routes"]["xyce"]["tool_path"])
        device_dc_next_scope = config.get("tcad_track", {}).get("next_scope", "")
        device_dc_next_scope_valid = (
            device_dc_implemented_state
        ...
        add_check(
            ...
            device_dc_config.get("contract_id") == "M01_OPEN_SOURCE_DEVICE_DC_R01"
            and device_dc_config.get("revision") == 1
            and device_dc_config.get("status") == "contract_planned"
            and device_dc_config.get("simulation_status")
            == "NOT_RUN_BY_CONTRACT_CHECK"
            and device_dc_config.get("scope", {}).get("active_material_scope")
            == "IGZO only"
            and device_dc_config.get("scope", {}).get("upstream_device_model")
            == "2D frozen teaching TCAD"
            and device_dc_config.get("scope", {}).get("execution_scope")
            == "compact-model device-only DC cross-check"
            and device_dc_config.get("registered_checks")
            ...
            and device_dc_machine.get("revision") == 1
            and device_dc_machine.get("target_row_count") == 247
            and device_dc_machine.get("target_curve_count") == 13
            and device_dc_machine.get("route_count") == 2
            and device_dc_machine.get("generated_device_netlist_budget") == 2
            and device_dc_machine.get("runner_process_budget") == 2
            and device_dc_machine.get("aimspice_invoked") is False
            and device_dc_machine.get("tcad_invoked") is False
            and device_dc_machine.get("circuit_or_downstream_permitted") is False
            and device_dc_machine.get("r11_preflight_bound_commit")
            == "010a51f30e87602e7935bd55e4ee35c38014e9ff"
            and device_dc_machine.get("r11_static_runner_independent_passed") is True
            and device_dc_machine.get("implementation_project_check_failure", {}).get(
                "sha256"
            )
            == "dcf6c2ff89fd6f2d31dfc140c55cbe7d04f77e662370e35698acb926653599a0"
            and device_dc_machine.get("implementation_project_check_failure", {}).get(
                "preserved"
            )
            is True
            and all((ROOT / path).is_file() for path in device_dc_source_paths)
            and "EXPECTED_CHECK_COUNT = 40" in device_dc_contract_source
            and "EXPECTED_CHECK_COUNT = 30" in device_dc_runner_source
            and "EXPECTED_CHECK_COUNT = 24" in device_dc_independent_source
            and "def generate_device_netlist" in device_dc_common_source
            and "def parse_ngspice_ascii_raw" in device_dc_common_source
            and "def parse_xyce_prn" in device_dc_common_source
            and "import subprocess" in device_dc_runner_source
            and re.search(
                r"^(?:import|from)\s+subprocess\b",
                device_dc_contract_source,
                re.MULTILINE,
            )
            is None
            and re.search(
                r"^(?:import|from)\s+subprocess\b",
                device_dc_independent_source,
                re.MULTILINE,
            )
            is None
            and all(
                (ROOT / path).is_file() and sha256(ROOT / path) == expected
                for path, expected in preflight_artifacts
            )
            and preflight.get("r11_static_checks_passed") == 36
            and preflight.get("r11_runner_checks_passed") == 32
            and preflight.get("r11_independent_checks_passed") == 25
            and preflight.get("r11_independent_processes") == 0
            ...
            == device_dc_config["device_contract"]["candidate_sha256"]
            and device_dc_config["device_contract"][
                "same_candidate_bytes_for_both_routes"
            ]
            is True
            and device_dc_config["device_contract"]["equation_identity_claimed"]
            is False
            and ng_tool.is_file()
            and sha256(ng_tool)
            == device_dc_config["routes"]["ngspice"]["tool_sha256"]
            and ng_tool.stat().st_size
            == device_dc_config["routes"]["ngspice"]["tool_bytes"]
            and xyce_tool.is_file()
            and sha256(xyce_tool)
            == device_dc_config["routes"]["xyce"]["tool_sha256"]
            and xyce_tool.stat().st_size
            == device_dc_config["routes"]["xyce"]["tool_bytes"]
            and device_dc_config.get("resource_budget", {}).get("route_processes") == 2
            and device_dc_config.get("resource_budget", {}).get("parallel_route_execution")
            is False
            and device_dc_config.get("failure_retention", {}).get(
    ...
    try:
        ...
        r03_contract_report_path = ROOT / r03_outputs["contract_report"]
        r03_contract_report = (
            json.loads(r03_contract_report_path.read_text(encoding="utf-8"))
        ...
        r03_contract_pass = (
            r03_contract_report.get("status") == "PASS"
            and r03_contract_report.get("evidence_level") == "E3"
            and r03_contract_report.get("simulation_status")
            == "NOT_RUN_BY_CONTRACT_CHECK"
            and r03_contract_report.get("summary", {}).get("passed") == 42
            and r03_contract_report.get("summary", {}).get("failed") == 0
            and r03_contract_report.get("summary", {}).get("total") == 42
            and r03_contract_report.get("summary", {}).get(
                "simulator_processes_invoked"
            )
            == 0
            and r03_contract_report.get("summary", {}).get(
            ...
            is False
            and r03_contract_report.get("future_outputs_absent_before_report")
            is True
            and r03_contract_report.get("config", {}).get("sha256")
            == sha256(m01_device_dc_r03_config_path)
            and r03_contract_report.get("contract_checker", {}).get("sha256")
            == sha256(m01_device_dc_r03_contract_checker_path)
            and r03_contract_report.get("common", {}).get("sha256")
            == sha256(m01_device_dc_r03_common_path)
            and r03_contract_report.get("runner", {}).get("sha256")
            == sha256(m01_device_dc_r03_runner_path)
            and r03_contract_report.get("independent_checker", {}).get("sha256")
            == sha256(m01_device_dc_r03_independent_path)
            and len(r03_contract_report.get("checks", [])) == 42
            and all(
                item.get("status") == "PASS"
        ...
        r03_ready_state = (
            ...
            and r03_machine.get("runner_processes_invoked") == 0
            and r03_machine.get("independent_check_completed") is False
            and r03_machine.get("future_outputs_absent") is False
            and r03_machine.get("contract_report_sha256")
            == sha256(r03_contract_report_path)
            and r03_static_failure.get("sha256")
            == sha256(r03_static_failure_path)
            and r03_static_failure.get("preserved") is True
            and r03_machine.get("result_paths")
            == r03_source_paths
            + [r03_static_failure["path"], r03_outputs["contract_report"]]
            and all(
                not path.exists()
                for path in r03_output_paths
        ...
        add_check(
            ...
            is True
            and (
                r03_machine.get("next_gate") == r03_config.get("next_gate")
                or (
                    r03_ready_state
    ...
    c00_config_path = ROOT / "config" / "c00_active_load_inverter_r01.json"
    c00_common_path = ROOT / "scripts" / "c00_active_load_inverter_r01_common.py"
    c00_contract_path = ROOT / "scripts" / "check_c00_active_load_inverter_r01_contract.py"
    c00_runner_path = ROOT / "scripts" / "run_c00_active_load_inverter_r01.py"
    c00_independent_path = ROOT / "scripts" / "check_c00_active_load_inverter_r01.py"
    try:
        c00_config = json.loads(c00_config_path.read_text(encoding="utf-8"))
        c00_experiment = experiment_map["C00"]
        c00_machine = c00_experiment.get("active_load_inverter_r01", {})
        c00_outputs = c00_config["outputs"]
        c00_contract_report_path = ROOT / c00_outputs["contract_report"]
        c00_run_report_path = ROOT / c00_outputs["run_report"]
        c00_independent_report_path = ROOT / c00_outputs["independent_report"]
        c00_run_artifact_keys = [
            key
            for key in c00_outputs
            if key
            not in {
                "run_directory",
                "contract_report",
        ...
        c00_run_artifact_paths = [ROOT / c00_outputs[key] for key in c00_run_artifact_keys]
        c00_contract_report = (
            json.loads(c00_contract_report_path.read_text(encoding="utf-8"))
            if c00_contract_report_path.is_file()
            else {}
        )
        c00_run_report = (
            json.loads(c00_run_report_path.read_text(encoding="utf-8"))
            if c00_run_report_path.is_file()
            else {}
        )
        c00_independent_report = (
            json.loads(c00_independent_report_path.read_text(encoding="utf-8"))
            if c00_independent_report_path.is_file()
            else {}
        )
        c00_sources = [
            "config/c00_active_load_inverter_r01.json",
            "scripts/c00_active_load_inverter_r01_common.py",
            "scripts/check_c00_active_load_inverter_r01_contract.py",
            "scripts/run_c00_active_load_inverter_r01.py",
            "scripts/check_c00_active_load_inverter_r01.py",
        ]
        c00_source_hashes_ok = (
            c00_machine.get("config_sha256") == sha256(c00_config_path)
            and c00_machine.get("common_sha256") == sha256(c00_common_path)
            and c00_machine.get("contract_checker_sha256") == sha256(c00_contract_path)
            and c00_machine.get("runner_sha256") == sha256(c00_runner_path)
            and c00_machine.get("independent_checker_sha256")
            == sha256(c00_independent_path)
        )
        c00_implemented_state = (
            c00_experiment.get("status") == "contract_implemented"
            and c00_experiment.get("current_evidence") == "E0"
            and c00_machine.get("status") == "contract_implemented"
            and c00_machine.get("current_evidence") == "E0"
            and c00_machine.get("contract_check_completed") is False
            and c00_machine.get("formal_run_completed") is False
            and c00_machine.get("independent_check_completed") is False
            ...
            and not c00_contract_report_path.exists()
        ...
        c00_ready_state = (
            ...
            and c00_machine.get("status") == "contract_ready"
            and c00_machine.get("contract_check_completed") is True
            and c00_machine.get("contract_checks_passed") == 48
            and c00_machine.get("contract_checks_failed") == 0
            and c00_machine.get("circuit_execution_permitted") is True
            and c00_contract_report.get("status") == "PASS"
            and c00_contract_report.get("summary")
        ...
        c00_runner_state = (
            ...
            and c00_machine.get("status") == "formal_run_passed"
            and c00_machine.get("formal_run_completed") is True
            and c00_machine.get("runner_checks_passed") == 36
            and c00_machine.get("runner_checks_failed") == 0
            and c00_machine.get("simulator_processes_invoked") == 4
            and c00_machine.get("independent_check_permitted") is True
            and c00_machine.get("independent_check_completed") is False
            and c00_run_report.get("status") == "PASS"
            and c00_run_report.get("summary", {}).get("passed") == 36
            and c00_machine.get("runner_report_sha256") == sha256(c00_run_report_path)
            and all(path.is_file() for path in c00_run_artifact_paths)
            and not c00_independent_report_path.exists()
        ) if c00_run_report_path.is_file() else False
        c00_verified_state = (
            c00_experiment.get("status") == "verified"
            and c00_experiment.get("current_evidence") == "E3"
            and c00_machine.get("status") == "verified"
            and c00_machine.get("independent_check_completed") is True
            and c00_machine.get("independent_checks_passed") == 29
            and c00_machine.get("independent_checks_failed") == 0
            and c00_machine.get("independent_processes_invoked") == 0
            and c00_independent_report.get("status") == "PASS"
            and c00_independent_report.get("summary")
            == {"passed": 29, "failed": 0, "total": 29}
            and c00_machine.get("independent_report_sha256")
            == sha256(c00_independent_report_path)
        ) if c00_independent_report_path.is_file() else False
        c00_next_scope = config.get("tcad_track", {}).get("next_scope", "")
        c00_scope_ok = (
            c00_implemented_state
        ...
        add_check(
            ...
            c00_config.get("contract_id") == "C00_ACTIVE_LOAD_INVERTER_R01"
            and c00_config.get("revision") == 1
            and c00_config.get("scope", {}).get("active_material_scope")
            == "IGZO only"
            and c00_config.get("topology_contract", {}).get("device_count_per_case")
            == 2
            and c00_config.get("registered_checks")
            == {"static_contract": 48, "runner": 36, "independent": 29}
            and c00_config.get("sweep_contract", {}).get("dc_case_count") == 18
            and c00_config.get("sweep_contract", {}).get("transient_case_count")
            == 36
            and c00_config.get("resource_budget", {}).get(
                "max_serial_simulator_processes"
            )
            == 4
            and c00_config.get("resource_budget", {}).get(
                "parallel_simulator_processes"
            )
            == 0
            and c00_machine.get("expected_contract_check_count") == 48
            and c00_machine.get("expected_runner_check_count") == 36
            and c00_machine.get("expected_independent_check_count") == 29
            and c00_machine.get("result_paths")
            == c00_sources
            + ([c00_outputs["contract_report"]] if not c00_implemented_state else [])
            + (
                [c00_outputs[key] for key in c00_run_artifact_keys]
                + [c00_outputs["run_report"]]
                if c00_runner_state or c00_verified_state
                else []
            )
            + ([c00_outputs["independent_report"]] if c00_verified_state else [])
            and c00_source_hashes_ok
            and all((ROOT / path).is_file() for path in c00_sources)
            and "EXPECTED_CHECK_COUNT = 48"
            in c00_contract_path.read_text(encoding="ascii")
            and "EXPECTED_CHECK_COUNT = 36"
            in c00_runner_path.read_text(encoding="ascii")
            and "EXPECTED_CHECK_COUNT = 29"
            in c00_independent_path.read_text(encoding="ascii")
            and re.search(
                r"^(?:import|from)\s+subprocess\b",
                c00_contract_path.read_text(encoding="ascii"),
                re.MULTILINE,
            )
            is None
            and re.search(
                r"^(?:import|from)\s+subprocess\b",
                c00_independent_path.read_text(encoding="ascii"),
                re.MULTILINE,
            )
            is None
            and "import subprocess" in c00_runner_path.read_text(encoding="ascii")
            and "c00-active-load-inverter-r01-contract-check:" in makefile_source
            and "c00-active-load-inverter-r01:" in makefile_source
    ...
    c00_r03_config_path = ROOT / "config" / "c00_active_load_inverter_r03.json"
    c00_r03_common_path = ROOT / "scripts" / "c00_active_load_inverter_r03_common.py"
    c00_r03_contract_path = ROOT / "scripts" / "check_c00_active_load_inverter_r03_contract.py"
    c00_r03_runner_path = ROOT / "scripts" / "run_c00_active_load_inverter_r03.py"
    c00_r03_independent_path = ROOT / "scripts" / "check_c00_active_load_inverter_r03.py"
    try:
        c00_r03_config = json.loads(c00_r03_config_path.read_text(encoding="utf-8"))
        c00_experiment = experiment_map["C00"]
        c00_r03_machine = c00_experiment.get("active_load_inverter_r03", {})
        c00_r02_machine = c00_experiment.get("active_load_inverter_r02", {})
        c00_r03_outputs = c00_r03_config["outputs"]
        c00_r03_contract_report_path = ROOT / c00_r03_outputs["contract_report"]
        c00_r03_run_report_path = ROOT / c00_r03_outputs["run_report"]
        c00_r03_independent_report_path = ROOT / c00_r03_outputs["independent_report"]
        c00_r03_contract_report = (
            json.loads(c00_r03_contract_report_path.read_text(encoding="utf-8"))
            if c00_r03_contract_report_path.is_file()
            else {}
        )
        c00_r03_run_report = (
            json.loads(c00_r03_run_report_path.read_text(encoding="utf-8"))
            if c00_r03_run_report_path.is_file()
            else {}
        )
        c00_r03_independent_report = (
            json.loads(c00_r03_independent_report_path.read_text(encoding="utf-8"))
            if c00_r03_independent_report_path.is_file()
            else {}
        )
        c00_r03_sources = [
            "config/c00_active_load_inverter_r03.json",
            "scripts/c00_active_load_inverter_r03_common.py",
            "scripts/check_c00_active_load_inverter_r03_contract.py",
            "scripts/run_c00_active_load_inverter_r03.py",
            "scripts/check_c00_active_load_inverter_r03.py",
        ]
        c00_r03_artifact_keys = [
            key
            for key in c00_r03_outputs
            if key
            not in {
                "run_directory",
                "contract_report",
        ...
        c00_r03_bound_pairs = (
            ...
            ("runner_gate_blocker_path", "runner_gate_blocker_sha256"),
            (
                "registration_project_check_failure_path",
                "registration_project_check_failure_sha256",
            ),
        )
        c00_r02_static_report = json.loads(
            (ROOT / c00_r03_binding["static_report_path"]).read_text(encoding="utf-8")
        )
        c00_r02_blocker = json.loads(
            (ROOT / c00_r03_binding["runner_gate_blocker_path"]).read_text(
                encoding="utf-8"
            )
        )
        c00_r02_project_failure = json.loads(
            (
                ROOT / c00_r03_binding["registration_project_check_failure_path"]
            ).read_text(encoding="utf-8")
        )
        c00_r03_source_hashes_ok = (
            c00_r03_machine.get("config_sha256") == sha256(c00_r03_config_path)
            and c00_r03_machine.get("common_sha256") == sha256(c00_r03_common_path)
            and c00_r03_machine.get("contract_checker_sha256")
            == sha256(c00_r03_contract_path)
            and c00_r03_machine.get("runner_sha256") == sha256(c00_r03_runner_path)
            and c00_r03_machine.get("independent_checker_sha256")
            == sha256(c00_r03_independent_path)
        )
        c00_r03_binding_ok = (
            c00_r03_binding.get("implementation_commit")
            == "216c6a7cda148888eac8102a99ff371f0fa2c6be"
            and c00_r03_binding.get("preservation_commit")
            == c00_r03_machine.get("r02_preservation_commit")
            == "747f3843240377d374c8111a09f0d7663014a98a"
            and all(
                (ROOT / c00_r03_binding[path_key]).is_file()
                and sha256(ROOT / c00_r03_binding[path_key])
                == c00_r03_binding[hash_key]
                for path_key, hash_key in c00_r03_bound_pairs
            )
            and c00_r03_binding.get("static_report_sha256")
            == c00_r03_machine.get("r02_static_report_sha256")
            == c00_r02_machine.get("contract_report_sha256")
            and c00_r02_static_report.get("status") == "PASS"
            and c00_r02_static_report.get("summary")
            == {"passed": 50, "failed": 0, "total": 50}
            and c00_r03_binding.get("runner_gate_blocker_sha256")
            == c00_r03_machine.get("r02_runner_gate_blocker_sha256")
            and c00_r02_blocker.get("status") == "BLOCKED"
            and c00_r02_blocker.get("failure_category")
            == "STATIC_PASS_COMMIT_SELF_REFERENCE_UNSATISFIABLE"
            and c00_r02_blocker.get("runner_invoked") is False
            and c00_r02_project_failure.get("status") == "FAIL"
            and c00_r02_project_failure.get("checks") == 765
            and c00_r02_project_failure.get("failures") == 1
            and c00_r02_machine.get("status")
            == "static_contract_passed_runner_gate_blocked"
            and c00_r02_machine.get("formal_run_completed") is False
        )
        c00_r03_base_paths = c00_r03_sources + [
            c00_r03_binding["static_report_path"],
            c00_r03_binding["runner_gate_blocker_path"],
            c00_r03_binding["registration_project_check_failure_path"],
        ]
        c00_r03_implemented_state = (
            c00_experiment.get("status") == "contract_implemented"
            and c00_experiment.get("current_evidence") == "E0"
            and c00_r03_machine.get("status") == "contract_implemented"
            and c00_r03_machine.get("current_evidence") == "E0"
            and c00_r03_machine.get("contract_check_completed") is False
            and c00_r03_machine.get("formal_run_completed") is False
            and c00_r03_machine.get("independent_check_completed") is False
            and c00_r03_machine.get("simulator_processes_invoked") == 0
            and c00_r03_machine.get("circuit_netlists_created") == 0
            and c00_r03_machine.get("circuit_execution_permitted") is False
            and c00_r03_machine.get("downstream_permitted") is False
            and not c00_r03_contract_report_path.exists()
        ...
        c00_r03_ready_state = (
            c00_experiment.get("status") == "contract_ready"
            and c00_experiment.get("current_evidence") == "E3"
            and c00_r03_machine.get("status") == "contract_ready"
            and c00_r03_machine.get("current_evidence") == "E3"
            ...
            and c00_r03_machine.get("circuit_execution_permitted") is True
            and c00_r03_machine.get("formal_run_completed") is False
            and c00_r03_contract_report.get("status") == "PASS"
            and c00_r03_contract_report.get("evidence_level") == "E3"
            and c00_r03_contract_report.get("summary")
            == {"passed": 50, "failed": 0, "total": 50}
            and c00_r03_contract_report.get("git_state", {}).get("head")
            == c00_r03_contract_report.get("git_state", {}).get("origin_main")
        ...
        c00_r03_failed_state = (
            (
                ...
                or (
                    c00_experiment.get("active_load_inverter_r04", {}).get("revision") == 4
                    and (c00_experiment.get("status"), c00_experiment.get("current_evidence"))
                    in {
                        ("contract_implemented", "E0"),
                        ("contract_ready", "E3"),
        ...
        c00_r03_runner_state = (
            c00_experiment.get("status") == "formal_run_passed"
            and c00_experiment.get("current_evidence") == "E2"
            and c00_r03_machine.get("status") == "formal_run_passed"
            and c00_r03_machine.get("current_evidence") == "E2"
            and c00_r03_machine.get("formal_run_completed") is True
            and c00_r03_machine.get("formal_run_status") == "PASS"
            and c00_r03_machine.get("runner_checks_passed") == 36
            and c00_r03_machine.get("runner_checks_failed") == 0
            and c00_r03_machine.get("simulator_processes_invoked") == 4
            and c00_r03_machine.get("independent_check_permitted") is True
            and c00_r03_machine.get("independent_check_completed") is False
            and c00_r03_run_report.get("status") == "PASS"
            and c00_r03_run_report.get("summary", {}).get("passed") == 36
            and c00_r03_run_report.get("summary", {}).get("process_invocations") == 4
            and c00_r03_run_report.get("git_state", {}).get("head")
            == c00_r03_run_report.get("git_state", {}).get("origin_main")
            and c00_r03_run_report.get("git_state", {}).get("synchronized") is True
            and c00_r03_machine.get("runner_report_sha256")
            == sha256(c00_r03_run_report_path)
            and all(path.is_file() for path in c00_r03_artifact_paths)
            and not c00_r03_independent_report_path.exists()
        ) if c00_r03_run_report_path.is_file() else False
        c00_r03_verified_state = (
            c00_experiment.get("status") == "verified"
            and c00_experiment.get("current_evidence") == "E3"
            and c00_r03_machine.get("status") == "verified"
            and c00_r03_machine.get("current_evidence") == "E3"
            and c00_r03_machine.get("independent_check_completed") is True
            and c00_r03_machine.get("independent_check_status") == "PASS"
            and c00_r03_machine.get("independent_checks_passed") == 29
            and c00_r03_machine.get("independent_checks_failed") == 0
            and c00_r03_machine.get("independent_processes_invoked") == 0
            and c00_r03_independent_report.get("status") == "PASS"
            and c00_r03_independent_report.get("summary")
            == {"passed": 29, "failed": 0, "total": 29}
            and c00_r03_machine.get("independent_report_sha256")
            == sha256(c00_r03_independent_report_path)
        ) if c00_r03_independent_report_path.is_file() else False
        c00_r03_next_scope = config.get("tcad_track", {}).get("next_scope", "")
        c00_r03_scope_ok = (
            c00_r03_implemented_state
        ...
        if c00_r03_implemented_state:
            ...
            c00_r03_expected_paths = (
                ...
                + [
                    c00_r03_outputs["run_report"],
                    c00_r03_outputs["independent_report"],
                ]
            )
        else:
            c00_r03_expected_paths = []
        c00_r03_contract_source = c00_r03_contract_path.read_text(encoding="ascii")
        c00_r03_runner_source = c00_r03_runner_path.read_text(encoding="ascii")
        c00_r03_independent_source = c00_r03_independent_path.read_text(encoding="ascii")
        c00_r03_correction = c00_r03_config.get("correction_contract", {})
        add_check(
            checks,
            "c00_active_load_inverter_r03:committed_snapshot_versioned_contract_chain",
            c00_r03_config.get("contract_id") == "C00_ACTIVE_LOAD_INVERTER_R03"
            and c00_r03_config.get("revision") == 3
            and c00_r03_config.get("registered_checks")
            == {"static_contract": 50, "runner": 36, "independent": 29}
            and c00_r03_config.get("scope", {}).get("active_material_scope")
            == "IGZO only"
            and c00_r03_config.get("topology_contract", {}).get("device_count_per_case")
            == 2
            and c00_r03_config.get("sweep_contract", {}).get("dc_case_count") == 18
            and c00_r03_config.get("sweep_contract", {}).get("transient_case_count")
            == 36
            and c00_r03_config.get("resource_budget", {}).get(
                "max_serial_simulator_processes"
            )
            == 4
            and c00_r03_config.get("resource_budget", {}).get(
            ...
            == 0
            and c00_r03_correction.get("static_report_records_head_origin_snapshot")
            is True
            and c00_r03_correction.get("runner_requires_registered_static_report_hash")
            is True
            and c00_r03_correction.get("independent_requires_registered_runner_report_hash")
            is True
            and c00_r03_correction.get("tracked_value_must_equal_containing_commit")
            is False
            and c00_r03_machine.get("expected_contract_check_count") == 50
            and c00_r03_machine.get("expected_runner_check_count") == 36
            and c00_r03_machine.get("expected_independent_check_count") == 29
            and c00_r03_machine.get("result_paths") == c00_r03_expected_paths
            and c00_r03_source_hashes_ok
            and c00_r03_binding_ok
            and all((ROOT / path).is_file() for path in c00_r03_sources)
            and "EXPECTED_CHECK_COUNT = 50" in c00_r03_contract_source
            and "EXPECTED_CHECK_COUNT = 36" in c00_r03_runner_source
            and "EXPECTED_CHECK_COUNT = 29" in c00_r03_independent_source
            and "static_git_snapshot" in c00_r03_runner_source
            and "runner_git_snapshot" in c00_r03_independent_source
            and "static_pass_commit" not in c00_r03_runner_source
            and "runner_pass_commit" not in c00_r03_independent_source
            and re.search(
                r"^(?:import|from)\s+subprocess\b",
                c00_r03_contract_source,
                re.MULTILINE,
            )
            is None
            and re.search(
                r"^(?:import|from)\s+subprocess\b",
                c00_r03_independent_source,
                re.MULTILINE,
            )
            is None
            and "import subprocess" in c00_r03_runner_source
            and "c00-active-load-inverter-r03-contract-check:" in makefile_source
            and "c00-active-load-inverter-r03:" in makefile_source
            and "c00-active-load-inverter-r03-check:" in makefile_source
            and (
                c00_r03_implemented_state
            ...
            and c00_r03_scope_ok,
            f"implemented={c00_r03_implemented_state} ready={c00_r03_ready_state} "
            f"runner={c00_r03_runner_state} verified={c00_r03_verified_state} "
            f"source_bindings={c00_r03_source_hashes_ok} r02_binding={c00_r03_binding_ok}",
        )
    except Exception as error:  # noqa: BLE001
        add_check(
            checks,
    ...
    c00_r04_config_path = ROOT / "config" / "c00_active_load_inverter_r04.json"
    c00_r04_common_path = ROOT / "scripts" / "c00_active_load_inverter_r04_common.py"
    c00_r04_contract_path = ROOT / "scripts" / "check_c00_active_load_inverter_r04_contract.py"
    c00_r04_runner_path = ROOT / "scripts" / "run_c00_active_load_inverter_r04.py"
    c00_r04_independent_path = ROOT / "scripts" / "check_c00_active_load_inverter_r04.py"
    try:
        c00_r04_config = json.loads(c00_r04_config_path.read_text(encoding="utf-8"))
        c00_experiment = experiment_map["C00"]
        ...
        c00_r04_independent_report_path = ROOT / c00_r04_outputs["independent_report"]
        c00_r04_contract_report = (
            json.loads(c00_r04_contract_report_path.read_text(encoding="utf-8"))
            if c00_r04_contract_report_path.is_file()
            else {}
        )
        c00_r04_run_report = (
            json.loads(c00_r04_run_report_path.read_text(encoding="utf-8"))
            if c00_r04_run_report_path.is_file()
            else {}
        )
        c00_r04_independent_report = (
            json.loads(c00_r04_independent_report_path.read_text(encoding="utf-8"))
            if c00_r04_independent_report_path.is_file()
            else {}
        )
        c00_r04_sources = [
            "config/c00_active_load_inverter_r04.json",
            "scripts/c00_active_load_inverter_r04_common.py",
            "scripts/check_c00_active_load_inverter_r04_contract.py",
            "scripts/run_c00_active_load_inverter_r04.py",
            "scripts/check_c00_active_load_inverter_r04.py",
        ]
        c00_r04_artifact_keys = [
            key
            for key in c00_r04_outputs
            if key not in {"run_directory", "contract_report", "run_report", "independent_report"}
        ]
        c00_r04_artifact_paths = [ROOT / c00_r04_outputs[key] for key in c00_r04_artifact_keys]
        c00_r04_binding = c00_r04_config["r03_formal_failure_binding"]
        c00_r04_bound_pairs = (
            ("config_path", "config_sha256"),
            ("common_path", "common_sha256"),
            ("contract_checker_path", "contract_checker_sha256"),
        ...
        c00_r04_binding_ok = (
            c00_r04_binding.get("failure_preservation_commit")
            == c00_r04_machine.get("r03_failure_preservation_commit")
            == "77d611ebc8519b3f43e0ae951592286b60df8eaa"
            and all(
                (ROOT / c00_r04_binding[path_key]).is_file()
                and sha256(ROOT / c00_r04_binding[path_key]) == c00_r04_binding[hash_key]
                for path_key, hash_key in c00_r04_bound_pairs
            )
            and len(c00_r04_binding.get("native_artifacts", {})) == 16
            and all(
                (ROOT / item["path"]).is_file()
                and sha256(ROOT / item["path"]) == item["sha256"]
                for item in c00_r04_binding["native_artifacts"].values()
            )
            and c00_r04_binding.get("runner_report_sha256")
            == c00_r04_machine.get("r03_runner_report_sha256")
            == c00_r03_machine.get("runner_report_sha256")
            and c00_r04_binding.get("status") == "FORMAL_RUN_FAILED"
            and c00_r04_binding.get("checks_passed") == 24
            and c00_r04_binding.get("checks_failed") == 12
            and c00_r04_binding.get("simulator_processes_invoked") == 4
            and c00_r04_binding.get("must_remain_unchanged") is True
        )
        c00_r04_source_hashes_ok = (
            c00_r04_machine.get("config_sha256") == sha256(c00_r04_config_path)
            and c00_r04_machine.get("common_sha256") == sha256(c00_r04_common_path)
            and c00_r04_machine.get("contract_checker_sha256") == sha256(c00_r04_contract_path)
            and c00_r04_machine.get("runner_sha256") == sha256(c00_r04_runner_path)
            and c00_r04_machine.get("independent_checker_sha256")
            == sha256(c00_r04_independent_path)
        )
        c00_r04_bound_paths = [
            c00_r04_binding[path_key] for path_key, _ in c00_r04_bound_pairs
        ] + [item["path"] for item in c00_r04_binding["native_artifacts"].values()]
        c00_r04_base_paths = c00_r04_sources + c00_r04_bound_paths
        c00_r04_implemented_state = (
            c00_experiment.get("status") == "contract_implemented"
            and c00_experiment.get("current_evidence") == "E0"
            and c00_r04_machine.get("status") == "contract_implemented"
            and c00_r04_machine.get("current_evidence") == "E0"
            and c00_r04_machine.get("contract_check_completed") is False
            and c00_r04_machine.get("formal_run_completed") is False
            and c00_r04_machine.get("independent_check_completed") is False
            and c00_r04_machine.get("simulator_processes_invoked") == 0
            and c00_r04_machine.get("circuit_netlists_created") == 0
            and c00_r04_machine.get("circuit_execution_permitted") is False
            and c00_r04_machine.get("downstream_permitted") is False
            and not c00_r04_contract_report_path.exists()
            and not (ROOT / c00_r04_outputs["run_directory"]).exists()
            and not c00_r04_run_report_path.exists()
            and not c00_r04_independent_report_path.exists()
            and all(not path.exists() for path in c00_r04_artifact_paths)
        )
        c00_r04_ready_state = (
            c00_experiment.get("status") == "contract_ready"
            and c00_experiment.get("current_evidence") == "E3"
            and c00_r04_machine.get("status") == "contract_ready"
            and c00_r04_machine.get("current_evidence") == "E3"
            and c00_r04_machine.get("contract_check_completed") is True
            and c00_r04_machine.get("contract_check_status") == "PASS"
            and c00_r04_machine.get("contract_checks_passed") == 50
            and c00_r04_machine.get("contract_checks_failed") == 0
            and c00_r04_machine.get("circuit_execution_permitted") is True
            and c00_r04_machine.get("formal_run_completed") is False
            and c00_r04_contract_report.get("status") == "PASS"
            and c00_r04_contract_report.get("summary")
            == {"passed": 50, "failed": 0, "total": 50}
            ...
            and not c00_r04_run_report_path.exists()
            and not c00_r04_independent_report_path.exists()
        ) if c00_r04_contract_report_path.is_file() else False
        c00_r04_runner_common_state = (
            c00_r04_machine.get("formal_run_completed") is True
            and c00_r04_machine.get("runner_report_sha256") == sha256(c00_r04_run_report_path)
            and c00_r04_run_report.get("summary", {}).get("total") == 36
            and c00_r04_run_report.get("summary", {}).get("process_invocations") == 4
            and c00_r04_run_report.get("summary", {}).get("ngspice_processes") == 2
            and c00_r04_run_report.get("summary", {}).get("xyce_processes") == 2
            and c00_r04_run_report.get("summary", {}).get("aimspice_processes") == 0
            and c00_r04_run_report.get("summary", {}).get("tcad_processes") == 0
            and c00_r04_run_report.get("summary", {}).get("layout_or_pex_processes") == 0
            and c00_r04_machine.get("simulator_processes_invoked") == 4
            and c00_r04_machine.get("runner_artifact_hashes")
            == c00_r04_run_report.get("artifacts")
            and all(
                (ROOT / c00_r04_outputs[key.removesuffix("_sha256")]).is_file()
                and sha256(ROOT / c00_r04_outputs[key.removesuffix("_sha256")]) == value
                for key, value in c00_r04_run_report.get("artifacts", {}).items()
            )
            and not c00_r04_independent_report_path.exists()
        ) if c00_r04_run_report_path.is_file() else False
        c00_r04_failed_state = (
            c00_experiment.get("status") == "formal_run_failed"
            and c00_experiment.get("current_evidence") == "E0"
            and c00_r04_machine.get("status") == "formal_run_failed"
            and c00_r04_machine.get("current_evidence") == "E0"
            and c00_r04_machine.get("formal_run_status") == "FAIL"
            and c00_r04_machine.get("independent_check_permitted") is False
            and c00_r04_machine.get("downstream_permitted") is False
            and c00_r04_run_report.get("status") == "FAIL"
            and c00_r04_run_report.get("evidence_level") == "E0"
        ...
        c00_r04_runner_state = (
            c00_experiment.get("status") == "formal_run_passed"
            and c00_experiment.get("current_evidence") == "E2"
            and c00_r04_machine.get("status") == "formal_run_passed"
            and c00_r04_machine.get("current_evidence") == "E2"
            and c00_r04_machine.get("formal_run_status") == "PASS"
            and c00_r04_machine.get("independent_check_permitted") is True
            and c00_r04_run_report.get("status") == "PASS"
            and c00_r04_run_report.get("summary", {}).get("passed") == 36
            and c00_r04_run_report.get("evidence_boundary")
            == c00_r04_config["evidence_boundary"]["future_runner_allowed_claim"]
            and c00_r04_runner_common_state
        )
        c00_r04_verified_state = (
            c00_experiment.get("status") == "verified"
            and c00_experiment.get("current_evidence") == "E3"
            and c00_r04_machine.get("status") == "verified"
            and c00_r04_machine.get("current_evidence") == "E3"
            and c00_r04_machine.get("independent_check_completed") is True
            and c00_r04_machine.get("independent_check_status") == "PASS"
            and c00_r04_machine.get("independent_checks_passed") == 29
            and c00_r04_machine.get("independent_checks_failed") == 0
            and c00_r04_machine.get("independent_processes_invoked") == 0
            and c00_r04_independent_report.get("status") == "PASS"
            and c00_r04_independent_report.get("summary")
            == {"passed": 29, "failed": 0, "total": 29}
            and c00_r04_machine.get("independent_report_sha256")
            == sha256(c00_r04_independent_report_path)
        ) if c00_r04_independent_report_path.is_file() else False
        c00_r04_next_scope = config.get("tcad_track", {}).get("next_scope", "")
        c00_r04_scope_ok = (
            c00_r04_implemented_state
            and c00_r04_next_scope.startswith(
                "run the 50-check static contract for C00 active-load inverter revision-4"
            )
        ) or (
            c00_r04_ready_state
        ...
        elif c00_r04_verified_state:
            c00_r04_expected_paths = (
                c00_r04_base_paths
                + [c00_r04_outputs["contract_report"]]
                + [c00_r04_outputs[key] for key in c00_r04_artifact_keys]
                + [c00_r04_outputs["run_report"], c00_r04_outputs["independent_report"]]
            )
        else:
            c00_r04_expected_paths = []
        c00_r04_contract_source = c00_r04_contract_path.read_text(encoding="ascii")
        c00_r04_runner_source = c00_r04_runner_path.read_text(encoding="ascii")
        c00_r04_independent_source = c00_r04_independent_path.read_text(encoding="ascii")
        c00_r04_correction = c00_r04_config["correction_contract"]
        add_check(
            checks,
            "c00_active_load_inverter_r04:output_axis_versioned_contract_chain",
            c00_r04_config.get("contract_id") == "C00_ACTIVE_LOAD_INVERTER_R04"
            and c00_r04_config.get("revision") == 4
            and c00_r04_config.get("registered_checks")
            == {"static_contract": 50, "runner": 36, "independent": 29}
            and c00_r04_config.get("scope", {}).get("active_material_scope") == "IGZO only"
            and c00_r04_config.get("topology_contract", {}).get("device_count_per_case") == 2
            and c00_r04_config.get("sweep_contract", {}).get("dc_case_count") == 18
            and c00_r04_config.get("sweep_contract", {}).get("transient_case_count") == 36
            and c00_r04_config.get("resource_budget", {}).get("max_serial_simulator_processes") == 4
            and c00_r04_config.get("resource_budget", {}).get("parallel_simulator_processes") == 0
            and c00_r04_correction.get("r03_must_not_rerun") is True
            and c00_r04_correction.get("native_transient_minimum_point_count") is None
            and c00_r04_correction.get("registered_interpolation_rows_per_case") == 601
            and c00_r04_correction.get("acceptance_thresholds_unchanged") is True
            and c00_r04_machine.get("expected_contract_check_count") == 50
            and c00_r04_machine.get("expected_runner_check_count") == 36
            and c00_r04_machine.get("expected_independent_check_count") == 29
            and c00_r04_machine.get("result_paths") == c00_r04_expected_paths
            and c00_r04_source_hashes_ok
            and c00_r04_binding_ok
            and all((ROOT / path).is_file() for path in c00_r04_sources)
            and "EXPECTED_CHECK_COUNT = 50" in c00_r04_contract_source
            and "EXPECTED_CHECK_COUNT = 36" in c00_r04_runner_source
            and "EXPECTED_CHECK_COUNT = 29" in c00_r04_independent_source
            and "native_axis_summary" in c00_r04_runner_source
            and "native raw point count failed" not in c00_r04_runner_source
            and "runner_failure_allowed_claim" in c00_r04_runner_source
            and re.search(r"^(?:import|from)\s+subprocess\b", c00_r04_contract_source, re.MULTILINE) is None
            and re.search(r"^(?:import|from)\s+subprocess\b", c00_r04_independent_source, re.MULTILINE) is None
            and "import subprocess" in c00_r04_runner_source
            and "c00-active-load-inverter-r04-contract-check:" in makefile_source
            and "c00-active-load-inverter-r04:" in makefile_source
            and "c00-active-load-inverter-r04-check:" in makefile_source
            and (
                c00_r04_implemented_state
                or c00_r04_ready_state
                or c00_r04_failed_state
                or c00_r04_runner_state
                or c00_r04_verified_state
            )
            and c00_r04_scope_ok,
    ...
    m01_rca_r01_runner_path = ROOT / "scripts" / "run_m01_route_divergence_root_cause_r01.py"
    m01_rca_r01_independent_path = ROOT / "scripts" / "check_m01_route_divergence_root_cause_r01.py"
    try:
        rca_config = json.loads(m01_rca_r01_config_path.read_text(encoding="utf-8"))
        rca_machine = experiment_map["M01"].get("route_divergence_root_cause_r01", {})
        rca_outputs = rca_config["outputs"]
        rca_contract_report_path = ROOT / rca_outputs["contract_report"]
        rca_run_report_path = ROOT / rca_outputs["run_report"]
        rca_independent_report_path = ROOT / rca_outputs["independent_report"]
        rca_contract_report = (
            json.loads(rca_contract_report_path.read_text(encoding="utf-8"))
            if rca_contract_report_path.is_file()
            else {}
        )
        rca_run_report = (
            json.loads(rca_run_report_path.read_text(encoding="utf-8"))
            if rca_run_report_path.is_file()
            else {}
        )
        rca_independent_report = (
            json.loads(rca_independent_report_path.read_text(encoding="utf-8"))
            if rca_independent_report_path.is_file()
            else {}
        )
        rca_source_paths = [
            "config/m01_route_divergence_root_cause_r01.json",
        ...
        rca_all_output_paths = [ROOT / value for value in rca_outputs.values()]
        rca_run_keys = [
            "ngspice_netlist",
            "xyce_netlist",
            "ngspice_log",
        ...
        rca_run_paths = [ROOT / rca_outputs[key] for key in rca_run_keys]
        source_hashes = rca_config["source_hashes"]
        source_hashes_ok = (
            source_hashes["common_sha256"] == sha256(m01_rca_r01_common_path)
            and source_hashes["contract_checker_sha256"] == sha256(m01_rca_r01_contract_path)
            and source_hashes["runner_sha256"] == sha256(m01_rca_r01_runner_path)
            and source_hashes["independent_checker_sha256"] == sha256(m01_rca_r01_independent_path)
        )
        binding = rca_config["r02_immutable_binding"]
        binding_pairs = [
            ...
            (binding["ngspice_log_path"], binding["ngspice_log_sha256"]),
        ...
        rca_r02_binding_ok = (
            ...
            and all(
                (ROOT / path).is_file() and sha256(ROOT / path) == expected
                for path, expected in binding_pairs
            )
        )
        implementation_failure = rca_config["implementation_project_check_failure"]
        implementation_failure_path = ROOT / implementation_failure["path"]
        implementation_failure_report = json.loads(
            implementation_failure_path.read_text(encoding="utf-8")
        )
        implementation_failure_ok = (
            implementation_failure_path.is_file()
            and sha256(implementation_failure_path) == implementation_failure["sha256"]
            and implementation_failure_report.get("status") == "FAIL"
            and implementation_failure_report.get("checks") == 753
            and implementation_failure_report.get("failures") == 1
            and implementation_failure.get("simulator_processes_invoked") == 0
            and implementation_failure.get("preserved") is True
        )
        source_scan_failure = rca_config["implementation_source_scan_failure"]
        source_scan_failure_path = ROOT / source_scan_failure["path"]
        source_scan_failure_report = json.loads(
            source_scan_failure_path.read_text(encoding="utf-8")
        )
        source_scan_failure_ok = (
            source_scan_failure_path.is_file()
            and sha256(source_scan_failure_path) == source_scan_failure["sha256"]
            and source_scan_failure_report.get("status") == "FAIL"
            and source_scan_failure_report.get("checks") == 754
            and source_scan_failure_report.get("failures") == 1
            and source_scan_failure.get("simulator_processes_invoked") == 0
            and source_scan_failure.get("preserved") is True
        )
        rca_contract_pass = (
            rca_contract_report.get("status") == "PASS"
            and rca_contract_report.get("evidence_level") == "E3"
            and rca_contract_report.get("simulation_status") == "NOT_RUN_BY_CONTRACT_CHECK"
            and rca_contract_report.get("summary", {}).get("passed") == 40
            and rca_contract_report.get("summary", {}).get("failed") == 0
        ...
        rca_run_pass = (
            ...
            and rca_run_report.get("evidence_level") == "E2"
            and rca_run_report.get("summary", {}).get("passed") == 30
            and rca_run_report.get("summary", {}).get("failed") == 0
            and rca_run_report.get("summary", {}).get("process_invocations") == 2
            and rca_run_report.get("summary", {}).get("formal_247_row_device_dc_invoked") is False
            and rca_run_report.get("diagnosis", {}).get("supported") is True
            and rca_run_report.get("diagnosis", {}).get("full_r02_route_agreement_established") is False
            and len(rca_run_report.get("checks", [])) == 30
            and all(item.get("status") == "PASS" for item in rca_run_report.get("checks", []))
        )
        rca_verified = (
            rca_run_pass
            and rca_independent_report.get("status") == "PASS"
            and rca_independent_report.get("evidence_level") == "E3"
            and rca_independent_report.get("summary", {}).get("passed") == 22
            and rca_independent_report.get("summary", {}).get("failed") == 0
            and rca_independent_report.get("processes_invoked") == 0
            and rca_independent_report.get("diagnosis", {}).get("independently_supported") is True
            and rca_independent_report.get("diagnosis", {}).get("full_r02_route_agreement_established") is False
        )
        rca_implemented_state = (
            rca_machine.get("status") == "contract_implemented"
            and rca_machine.get("current_evidence") == "E0"
            and rca_machine.get("contract_check_completed") is False
            and rca_machine.get("contract_status") == "NOT_RUN"
            ...
            and rca_machine.get("result_paths") == rca_source_paths
            and all(not path.exists() for path in rca_all_output_paths)
        )
        rca_ready_state = (
            rca_contract_pass
            and rca_machine.get("status") == "contract_ready"
            and rca_machine.get("current_evidence") == "E3"
            and rca_machine.get("contract_check_completed") is True
            and rca_machine.get("contract_status") == "PASS"
            and rca_machine.get("contract_checks_passed") == 40
            and rca_machine.get("contract_checks_failed") == 0
            and rca_machine.get("runner_completed") is False
            and rca_machine.get("processes_invoked") == 0
        ...
        rca_runner_state = (
            ...
            and rca_machine.get("current_evidence") == "E2"
            and rca_machine.get("runner_completed") is True
            and rca_machine.get("runner_status") == "PASS"
            and rca_machine.get("runner_checks_passed") == 30
            and rca_machine.get("runner_checks_failed") == 0
            and rca_machine.get("processes_invoked") == 2
            and rca_machine.get("root_cause_confirmed") is True
            and rca_machine.get("branch_current_extraction_eliminated") is True
            and rca_machine.get("portable_probe_validated") is True
            and rca_machine.get("independent_check_completed") is False
            and all(path.is_file() for path in rca_run_paths)
            and not rca_independent_report_path.exists()
        )
        rca_verified_state = (
            rca_verified
            and rca_machine.get("status") == "verified"
            and rca_machine.get("current_evidence") == "E3"
            and rca_machine.get("independent_check_completed") is True
            and rca_machine.get("independent_check_status") == "PASS"
            and rca_machine.get("independent_checks_passed") == 22
            and rca_machine.get("independent_checks_failed") == 0
            and rca_machine.get("independent_processes_invoked") == 0
            and all(path.is_file() for path in rca_run_paths)
            and rca_independent_report_path.is_file()
        )
        rca_next_scope = config.get("tcad_track", {}).get("next_scope", "")
        rca_next_scope_valid = (
            rca_implemented_state
            and rca_next_scope.startswith("establish and commit a versioned M01 route-divergence root-cause contract")
        ) or (
            rca_ready_state
            and rca_next_scope.startswith("execute the committed M01 route-divergence root-cause revision-1 probe")
        ) or (
            rca_runner_state
        ...
        independent_source = m01_rca_r01_independent_path.read_text(encoding="ascii")
        makefile_text = (ROOT / "Makefile").read_text(encoding="utf-8")
        add_check(
            checks,
            "m01_route_divergence_r01:contract_runner_independent_chain",
            rca_config.get("contract_id") == "M01_ROUTE_DIVERGENCE_ROOT_CAUSE_R01"
            and rca_config.get("revision") == 1
            and rca_config.get("status") == "contract_planned"
            and rca_config.get("scope", {}).get("active_material_scope") == "IGZO only"
            and rca_config.get("scope", {}).get("formal_247_row_device_dc_permitted") is False
            and rca_config.get("hypothesis", {}).get("id") == "THREE_ARGUMENT_LIMIT_SEMANTICS_MISMATCH"
            and rca_config.get("hypothesis", {}).get("candidate_file_modification_permitted") is False
            and rca_config.get("observed_r02_diagnostics", {}).get("root_cause_confirmed_before_probe") is False
            and rca_config.get("probe_contract", {}).get("expression_inputs") == [-75.0, 0.25, 75.0]
            and len(rca_config.get("probe_contract", {}).get("candidate_probe_points", [])) == 3
            and rca_config.get("probe_contract", {}).get("observable_count_per_route") == 13
            and rca_config.get("registered_checks") == {"static_contract": 40, "runner": 30, "independent": 22}
            and rca_config.get("resource_budget", {}).get("route_processes") == 2
            and rca_config.get("resource_budget", {}).get("parallel_route_execution") is False
            and rca_config.get("no_execution_rules", {}).get("r02_reexecution_permitted") is False
            and rca_config.get("no_execution_rules", {}).get("formal_247_row_execution_permitted") is False
            and rca_config.get("no_execution_rules", {}).get("circuit_or_downstream_permitted") is False
            and source_hashes_ok
            and rca_r02_binding_ok
            and implementation_failure_ok
            and source_scan_failure_ok
            and re.search(r"^(?:import|from)\s+subprocess\b", contract_source, re.MULTILINE) is None
            and "FORMAL_ROOT_CAUSE_PROBE" in runner_source
            and re.search(r"^(?:import|from)\s+subprocess\b", independent_source, re.MULTILINE) is None
            and re.search(
                r"^(?:import|from)\s+run_m01_route_divergence_root_cause_r01\b",
                independent_source,
                re.MULTILINE,
            ) is None
            and all(
