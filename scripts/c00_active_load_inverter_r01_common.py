#!/usr/bin/env python3
"""Deterministic C00 R01 netlist, parser, metric, and plotting helpers."""
...
def generate_dc_netlist(config: dict[str, Any], route: str) -> str:
    ...
    lines = [
        f"* C00 R01 {route} IGZO-only active-load inverter DC",
...
def generate_transient_netlist(config: dict[str, Any], route: str) -> str:
    ...
    lines = [
        f"* C00 R01 {route} IGZO-only active-load inverter transient",
        config["netlist_contract"]["candidate_include"],
        ".option numdgt=17",
    ]
    vectors = ["TIME"]
