#!/usr/bin/env python3
"""Independently check persisted R10 Xyce output/parser evidence."""
...
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_r10.json"
CHECK_REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_check_r10.json"
...
def check_preflight() -> dict[str, Any]:
    if CHECK_REPORT_PATH.exists():
        raise RuntimeError(f"R10 refuses to overwrite {CHECK_REPORT_PATH}")
    ...
    add_check(
        ...
        "R10 reuses complete R07 installs and executes only tool/parser commands",
    )
    add_check(
        checks,
        ...
        "R10 report binds current config, runner and preflight id",
    )
    add_check(
        checks,
        "binding:r07_failure",
    ...
    add_check(
        ...
        "R10 no-build boundary persisted",
    )
    add_check(
        checks,
    ...
    checker_source = (ROOT / "scripts" / "check_m01_xyce_build_preflight_r10.py").read_text(encoding="utf-8")
    common_source = (ROOT / "scripts" / "m01_xyce_r10_common.py").read_text(encoding="utf-8")
    ...
    add_check(
        ...
        and "bsource_self_test_output" in (ROOT / "scripts" / "run_m01_xyce_build_preflight_r10.py").read_text(encoding="utf-8"),
        "R10 binds Xyce fixed-column .prn parsing",
    )
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R10 independent registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R10_CHECK_{result['status']} "
