#!/usr/bin/env python3
"""Check the C00 R03 inverter contract without creating a netlist or process."""
...
from c00_active_load_inverter_r03_common import (
    anchor_ids,
    dc_cases,
    generate_dc_netlist,
...
INDEPENDENT_PATH = ROOT / "scripts" / "check_c00_active_load_inverter_r03.py"
...
def check_contract() -> int:
    ...
    machine = c00.get("active_load_inverter_r03", {})
    r02_machine = c00.get("active_load_inverter_r02", {})
    ...
    add_check(
        checks,
        "identity:c00_active_load_inverter_r03",
        config.get("project_id") == "DG-IGZO-TFT-PDK"
        and config.get("stage_id") == "C00"
        and config.get("contract_id") == "C00_ACTIVE_LOAD_INVERTER_R03"
    ...
    binding = config["r02_static_and_blocker_binding"]
    bound_files = (
    ...
    r02_report = load_json(ROOT / binding["static_report_path"])
    r02_blocker = load_json(ROOT / binding["runner_gate_blocker_path"])
    r02_project_failure = load_json(
        ROOT / binding["registration_project_check_failure_path"]
    )
    add_check(
        checks,
        "binding:r02_immutable_static_pass_blocker_and_failure",
        binding["implementation_commit"]
        == "216c6a7cda148888eac8102a99ff371f0fa2c6be"
        and binding["preservation_commit"]
        ...
        and binding["status"] == "STATIC_PASS_RUNNER_BLOCKED"
        and binding["evidence_level"] == r02_report.get("evidence_level") == "E3"
        and r02_report.get("status") == "PASS"
        and r02_report.get("summary") == {"passed": 50, "failed": 0, "total": 50}
        and binding["checks_passed"] == r02_machine.get("contract_checks_passed") == 50
        and binding["checks_failed"] == r02_machine.get("contract_checks_failed") == 0
        and r02_machine.get("status")
        == "static_contract_passed_runner_gate_blocked"
        and binding["runner_gate_failure_category"]
        == r02_blocker.get("failure_category")
        == "STATIC_PASS_COMMIT_SELF_REFERENCE_UNSATISFIABLE"
        and r02_blocker.get("audited_runner", {}).get("sha256")
        == binding["runner_sha256"]
        and r02_blocker.get("runner_invoked") is False
        and r02_project_failure.get("status") == "FAIL"
        and r02_project_failure.get("checks") == 765
        and r02_project_failure.get("failures") == 1
        and binding["runner_invoked"] is False
        and binding["simulator_processes_invoked"]
        == r02_report.get("simulator_processes_invoked")
        == r02_blocker.get("simulator_processes_invoked")
        == r02_machine.get("simulator_processes_invoked")
        == 0
        and binding["circuit_netlists_created"]
        == r02_report.get("circuit_netlists_created")
        == r02_blocker.get("circuit_netlists_created")
        == r02_machine.get("circuit_netlists_created")
        == 0
        and binding["must_remain_unchanged"] is True,
        f"static={binding['static_report_sha256']} blocker={binding['runner_gate_blocker_sha256']} 50/50",
    )
    correction = config["correction_contract"]
    r02_topology = dict(r02_config["topology_contract"])
    r03_topology = dict(config["topology_contract"])
    r02_topology.pop("cell_name")
    r03_topology.pop("cell_name")
    unchanged_route_keys = {
        key: value
        for key, value in r02_config["routes"]["ngspice"].items()
    ...
    unchanged_xyce_keys = {
        key: value
        for key, value in r02_config["routes"]["xyce"].items()
        if key not in {"dc_argv", "transient_argv"}
    }
    current_xyce_keys = {
        key: value
    ...
    add_check(
        checks,
        "correction:committed_state_binding_only_inputs_and_thresholds_unchanged",
        correction
        == {
            "only_semantic_correction": "Replace self-referential equality to a tracked containing-commit field with synchronized Git snapshots plus registered report hashes.",
            "registered_r02_defect": "R02 required HEAD == origin/main == machine.static_pass_commit while static_pass_commit was tracked in the same state commit.",
            "static_checker_requires_head_origin_sync": True,
            "static_report_records_head_origin_snapshot": True,
            "runner_requires_current_head_origin_sync": True,
            "runner_requires_registered_static_report_hash": True,
            "runner_requires_synced_static_report_snapshot": True,
            "independent_requires_current_head_origin_sync": True,
            "independent_requires_registered_runner_report_hash": True,
            "independent_requires_synced_runner_report_snapshot": True,
            "tracked_value_must_equal_containing_commit": False,
        ...
        and config["entry_gate"] == r02_config["entry_gate"]
        and config["scope"] == r02_config["scope"]
        and config["upstream_model"] == r02_config["upstream_model"]
        and r03_topology == r02_topology
        and config["topology_contract"]["cell_name"] == "DG_IGZO_ACTIVE_LOAD_INV_R03"
        and config["sweep_contract"] == r02_config["sweep_contract"]
        and config["extraction_contract"] == r02_config["extraction_contract"]
        and config["acceptance_contract"] == r02_config["acceptance_contract"]
        and config["netlist_contract"] == r02_config["netlist_contract"]
        and config["failure_retention"] == r02_config["failure_retention"]
        and config["resource_budget"] == r02_config["resource_budget"]
        and unchanged_route_keys == current_route_keys
        and unchanged_xyce_keys == current_xyce_keys
        and common_text.replace("R03", "R02").replace("r03", "r02")
        == (ROOT / binding["common_path"]).read_text(encoding="ascii")
        and "static_pass_commit" not in runner_text
        and "runner_pass_commit" not in independent_text
        and "static_git_snapshot" in runner_text
        and "runner_git_snapshot" in independent_text
        and is_commit_id(head)
        and head == origin,
        f"R02 circuit contract retained; synchronized_static_snapshot={head}",
    )
    scope = config["scope"]
    add_check(
        checks,
    ...
    add_check(checks, "source:independent_has_no_process_or_runner_import", not imports_module(independent_tree, "subprocess") and subprocess_calls(independent_tree) == 0 and not imports_module(independent_tree, "c00_active_load_inverter_r03_common") and not imports_module(independent_tree, "run_c00_active_load_inverter_r03"), "independent checker duplicates regeneration and extraction")
    ...
    add_check(checks, "outputs:unique_versioned_paths", len(output_values) == len(set(output_values)) and all("r03" in value or "c00_active_load_inverter_r03" in value for value in output_values), f"paths={len(output_values)}")
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R03_CONTRACT_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={report_path}")
