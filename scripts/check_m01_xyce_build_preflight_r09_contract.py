#!/usr/bin/env python3
"""Check the R09 Xyce output/parser contract without running any process."""
...
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r09.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r09.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r09.py"
COMMON_PATH = ROOT / "scripts" / "m01_xyce_r09_common.py"
...
def check_contract() -> dict[str, Any]:
    if REPORT_PATH.exists():
        raise RuntimeError(f"Refusing to overwrite existing R09 contract report: {REPORT_PATH}")
    ...
    machine = m01.get("xyce_build_preflight_r09", {})
    ...
    add_check(
        ...
        "immutable R08 30/36 checker registry failure is preserved",
    )
    add_check(
        checks,
        "binding:r08_sources_immutable",
        r08["must_remain_unchanged"] is True
        and all((ROOT / path).is_file() and sha256(ROOT / path) == expected for path, expected in r08_source_bindings),
        f"artifacts={sum((ROOT / path).is_file() for path, _ in r08_source_bindings)}/{len(r08_source_bindings)}",
    )
    add_check(
        checks,
        ...
        "R08 remains closed after its pre-report registry abort",
    )
    ...
    add_check(
        ...
        and all("r07" in value or "r09" in value for value in reuse.get("forbidden_reused_paths", []))
        and "results/compact/m01_xyce_build_preflight_r09" in reuse.get("forbidden_reused_paths", []),
    ...
    add_check(
        checks,
        "paths:r09_namespace_disjoint_from_r08_failure",
        all("r09" in value and "r08_registry_mismatch_failed" not in value for value in output_values)
        and not any(value in {r08["failure_report_path"], r08["failure_log_path"]} for value in output_values),
        "R09 reserves only its own output namespace",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        "runner parses fixed-column .prn and writes only R09 paths",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        and "run_m01_xyce_build_preflight_r09.py" in checker_source,
    ...
    add_check(
        ...
        and re.search(
            r"^(?:from\s+run_m01_xyce_build_preflight_r09\s+import|import\s+run_m01_xyce_build_preflight_r09\b)",
            checker_source,
            re.MULTILINE,
        )
        is None,
    ...
    add_check(
        ...
        is None,
        "R09 static contract checker is read-only and process-free",
    )
    add_check(
        checks,
        "source:r09_namespace_isolated",
        all("r08" not in source.lower() for source in (runner_source, checker_source, common_source))
        and "M01_XYCE_BUILD_PREFLIGHT_R09" in runner_source
        and "M01_XYCE_BUILD_PREFLIGHT_R09" in checker_source,
        "R09 execution sources do not silently reuse R08 names",
    )
    add_check(
        checks,
        "source:makefile_r09_targets",
        "m01-xyce-build-preflight-r09-contract-check:" in (ROOT / "Makefile").read_text(encoding="utf-8")
        and "m01-xyce-build-preflight-r09:" in (ROOT / "Makefile").read_text(encoding="utf-8")
        and "m01-xyce-build-preflight-r09-check:" in (ROOT / "Makefile").read_text(encoding="utf-8"),
        "contract, runner and independent-check targets are registered",
    )
    add_check(
        checks,
        "project:next_scope_r09_contract",
        project.get("tcad_track", {}).get("next_scope", "").startswith(
            "establish and commit M01 Xyce build/tool preflight revision-9 output/parser recovery contract"
        )
    ...
    add_check(
        checks,
        "project:evidence_boundary_r09",
        "m01_xyce_build_preflight_r09_contract_boundary" in project.get("tcad_track", {})
        and "not a built simulator" in project["tcad_track"]["m01_xyce_build_preflight_r09_contract_boundary"],
        "project boundary records R09 as tool/output contract only",
    )
    add_check(
        checks,
        "registry:static_report_is_only_future_artifact",
        machine.get("contract_report_path") == "results/reports/m01_xyce_build_preflight_contract_r09.json"
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R09 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R09_CONTRACT_{report['status']} "
