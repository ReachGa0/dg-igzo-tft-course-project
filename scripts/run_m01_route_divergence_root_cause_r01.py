#!/usr/bin/env python3
"""Run the bounded M01 route-divergence R01 root-cause probe."""
...
CONTRACT_REPORT_PATH = ROOT / "results" / "reports" / "m01_route_divergence_root_cause_contract_r01.json"
EXPECTED_CHECK_COUNT = 30
FORMAL_ROOT_CAUSE_PROBE = True
...
def run() -> int:
    ...
    try:
        ...
        machine = experiment_map["M01"]["route_divergence_root_cause_r01"]
        ...
        committed_static_pass = (
            ...
            and project.get("tcad_track", {}).get("next_scope", "").startswith(
                "execute the committed M01 route-divergence root-cause revision-1 probe"
            )
        )
        ...
        expected_xy_argv = [
            ...
            "results/compact/m01_route_divergence_root_cause_r01/xyce_probe",
    ...
    print(f"M01_ROUTE_DIVERGENCE_R01_RUNNER_{payload['status']} checks={passed}/{EXPECTED_CHECK_COUNT} report={report_path}")
