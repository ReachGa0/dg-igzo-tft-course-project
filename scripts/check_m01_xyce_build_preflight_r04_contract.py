#!/usr/bin/env python3
"""Check the revision-4 Xyce build/preflight contract without invoking a simulator."""
...
CONFIG_PATH = ROOT / "config" / "m01_xyce_build_preflight_r04.json"
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r04.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r04.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r04.py"
...
def check_contract() -> dict[str, Any]:
    ...
    r04 = m01["xyce_build_preflight_r04"]
    checks: list[dict[str, str]] = []

    add_check(
        checks,
        "identity:revision_4_xyce_build_preflight",
        config.get("stage_id") == "M01"
        and config.get("preflight_id") == "M01_XYCE_BUILD_PREFLIGHT_R04"
    ...
    add_check(
        ...
        "old transcription retained; independent archive hash is the R04 input",
    )
    ...
    add_check(
        checks,
        "build:unique_external_r04_roots",
        ...
        "R04 roots are unique, external and absent before execution",
    )
    add_check(
        checks,
    ...
    add_check(
        checks,
        "outputs:exclusive_unique_r04_namespace",
        ...
        and all("r04" in value for key, value in config["outputs"].items() if key != "contract_report"),
        "R04 run artifacts are unique and append-only",
    )
    ...
    add_check(
        ...
        "R04 evidence remains tool-only",
    )
    add_check(
        checks,
        "project:next_scope_is_r04",
        project["tcad_track"]["next_scope"].startswith("establish and commit M01 Xyce build/tool preflight revision-4")
    ...
    add_check(
        ...
        r04["status"] == "preflight_planned"
        and r04["revision"] == 4
        and r04["current_evidence"] == "E0"
        and r04["formal_run_completed"] is False
        and r04["device_netlist_invoked"] is False
        ...
        "R04 is unexecuted while R01/R03 failures remain machine-recorded",
    )
    ...
    add_check(
        checks,
        "runner:r04_wrapper_and_no_formal_execution",
        ...
        "R04 wraps the reviewed runner and injects only registered BLAS/LAPACK options",
    )
    add_check(
        checks,
        "checker:r04_independent_standard_library",
        ...
        "R04 independent checker has no simulator subprocess path",
    )
    add_check(
        checks,
        "make:r04_targets_registered",
        "m01-xyce-build-preflight-r04-contract-check:" in makefile_source
    ...
    add_check(
        checks,
        "gate:formal_m01_requires_r04_independent_pass",
        ...
        and "R03" in config["evidence_boundary"],
        config["next_gate"],
    )

    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R04 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    with REPORT_PATH.open("x", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print(f"M01_XYCE_BUILD_PREFLIGHT_R04_CONTRACT_{result['status']} checks={len(checks) - len(failures)}/{len(checks)} report={REPORT_PATH}")
    return result
