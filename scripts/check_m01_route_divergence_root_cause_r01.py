#!/usr/bin/env python3
"""Independently verify persisted M01 route-divergence R01 probe evidence."""
...
RUN_REPORT_PATH = ROOT / "results" / "reports" / "m01_route_divergence_root_cause_r01.json"
CHECK_REPORT_PATH = ROOT / "results" / "reports" / "m01_route_divergence_root_cause_r01_check.json"
...
def check() -> int:
    ...
    committed_runner_pass = (
        ...
        and project.get("tcad_track", {}).get("next_scope", "").startswith(
            "run the 22-check independent persisted-evidence checker for M01 route-divergence root-cause revision-1"
        )
    )
    add_check(
        checks,
    ...
    print(f"M01_ROUTE_DIVERGENCE_R01_CHECK_{payload['status']} checks={passed}/{EXPECTED_CHECK_COUNT} report={CHECK_REPORT_PATH}")
