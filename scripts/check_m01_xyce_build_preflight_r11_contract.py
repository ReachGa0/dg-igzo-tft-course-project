#!/usr/bin/env python3
"""Check the R11 Xyce output/parser contract without running any process."""
...
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r11.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r11.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r11.py"
COMMON_PATH = ROOT / "scripts" / "m01_xyce_r11_common.py"
...
def check_contract() -> dict[str, Any]:
    if REPORT_PATH.exists():
        raise RuntimeError(f"Refusing to overwrite existing R11 contract report: {REPORT_PATH}")
    ...
    machine = m01.get("xyce_build_preflight_r11", {})
    checks: list[dict[str, str]] = []

    add_check(
        checks,
        "identity:revision_11_xyce_parser_recovery",
    ...
    r08 = config["r08_failure_binding"]
    r08_failure_report_path = ROOT / r08["failure_report_path"]
    r08_failure_log_path = ROOT / r08["failure_log_path"]
    r08_failure_report = load_json(r08_failure_report_path)
    r08_source_bindings = [
        (r08["config_path"], r08["config_sha256"]),
        (r08["common_hash_helper_path"], r08["common_hash_helper_sha256"]),
        ...
        (r08["failure_report_path"], r08["failure_report_sha256"]),
        (r08["failure_log_path"], r08["failure_log_sha256"]),
    ]
    r09 = config["r09_failure_binding"]
    r09_failure_report_path = ROOT / r09["failure_report_path"]
    r09_failure_log_path = ROOT / r09["failure_log_path"]
    r09_failure_report = load_json(r09_failure_report_path)
    r09_source_bindings = [
        (r09["config_path"], r09["config_sha256"]),
        (r09["common_hash_helper_path"], r09["common_hash_helper_sha256"]),
        (r09["contract_checker_path"], r09["contract_checker_sha256"]),
        (r09["runner_path"], r09["runner_sha256"]),
        (r09["independent_checker_path"], r09["independent_checker_sha256"]),
        (r09["failure_report_path"], r09["failure_report_sha256"]),
        (r09["failure_log_path"], r09["failure_log_sha256"]),
    ]
    ...
    r10_failure_log_path = ROOT / r10["runner_failure_log_path"]
    ...
    add_check(
        ...
        r08["bound_commit"] == "95d656324fb5c4a301ed903961512a446b90669a"
        and r08_failure_report.get("status") == "FAIL"
        and r08_failure_report.get("contract_status") == "ABORTED_BEFORE_REPORT"
        and r08_failure_report.get("failure_category") == "contract_registry_mismatch"
        and r08_failure_report.get("registered_checks_before_abort") == 30
        and r08_failure_report.get("expected_checks") == 36
        and not r08_failure_report.get("r08_contract_report_created")
        and sha256(r08_failure_report_path) == r08["failure_report_sha256"]
        and sha256(r08_failure_log_path) == r08["failure_log_sha256"]
        and r09["bound_commit"] == "9285865"
        and r09_failure_report.get("status") == "FAIL"
        and r09_failure_report.get("contract_status") == "FAIL"
        and r09_failure_report.get("evidence_level") == "E0"
        and r09_failure_report.get("summary", {}).get("check_count") == 36
        and r09_failure_report.get("summary", {}).get("passed") == 34
        and r09_failure_report.get("summary", {}).get("failed") == 2
        and r09_failure_report.get("summary", {}).get("build_processes_invoked") == 0
        and r09_failure_report.get("summary", {}).get("simulator_processes_invoked") == 0
        and r09_failure_report.get("summary", {}).get("device_netlist_created") is False
        and r09_failure_report.get("summary", {}).get("numerical_outputs_created") is False
        and sha256(r09_failure_report_path) == r09["failure_report_sha256"]
        and sha256(r09_failure_log_path) == r09["failure_log_sha256"]
        ...
        and r10_summary.get("build_processes_invoked") == 0
        ...
        == 1.25,
        "immutable R08, R09 and R10 failures are preserved",
    )
    add_check(
        checks,
        "binding:historical_sources_immutable",
        r08["must_remain_unchanged"] is True
        and r09["must_remain_unchanged"] is True
        and r10["must_remain_unchanged"] is True
        and all(
            (ROOT / path).is_file() and sha256(ROOT / path) == expected
            for path, expected in r08_source_bindings + r09_source_bindings + r10_source_bindings
        )
        and tree_matches(r10_partial_directory, r10["partial_run_tree"]),
        f"artifacts={sum((ROOT / path).is_file() for path, _ in r08_source_bindings + r09_source_bindings + r10_source_bindings)}/{len(r08_source_bindings + r09_source_bindings + r10_source_bindings)}",
    )
    add_check(
        checks,
        "boundary:historical_reruns_prohibited",
        r08["runner_rerun"] is False
        and r08["independent_checker_run"] is False
        and r08_failure_report.get("r08_runner_run") is False
        and r08_failure_report.get("r08_independent_checker_run") is False
        and r08_failure_report.get("simulator_processes_invoked") == 0
        and r09["runner_rerun"] is False
        and r09["independent_checker_run"] is False
        and r09_failure_report.get("summary", {}).get("build_processes_invoked") == 0
        and r09_failure_report.get("summary", {}).get("simulator_processes_invoked") == 0
        and r10["runner_rerun"] is False
        and r10["independent_checker_run"] is False
        and r10_summary.get("process_invocations") == r10["completed_tool_processes"]
        and r10_summary.get("device_syntax_only_invoked") is False
        and r10_summary.get("formal_device_dc_invoked") is False,
        "R08, R09 and R10 remain closed after their preserved failures",
    )
    ...
    add_check(
        ...
        and all(
            ...
            or "r10" in value
            or "r11" in value
            for value in reuse.get("forbidden_reused_paths", [])
        )
        and "results/compact/m01_xyce_build_preflight_r11" in reuse.get("forbidden_reused_paths", [])
        and r08["failure_report_path"] in reuse.get("forbidden_reused_paths", [])
        and r08["failure_log_path"] in reuse.get("forbidden_reused_paths", [])
        and r09["failure_report_path"] in reuse.get("forbidden_reused_paths", [])
        and r09["failure_log_path"] in reuse.get("forbidden_reused_paths", [])
        and r10["partial_run_directory"] in reuse.get("forbidden_reused_paths", [])
        and r10["static_contract_report_path"] in reuse.get("forbidden_reused_paths", [])
        and r10["runner_failure_report_path"] in reuse.get("forbidden_reused_paths", [])
        and r10["runner_failure_log_path"] in reuse.get("forbidden_reused_paths", []),
    ...
    add_check(
        ...
        and candidate_contract.get("include_path_mode") == "repository_relative_ascii"
        and candidate_contract.get("include_path") == candidate_contract.get("candidate_path")
        and candidate_contract.get("include_path", "").isascii()
        and not Path(candidate_contract.get("include_path", "")).is_absolute()
        and candidate_contract.get("runner_working_directory") == "project_root"
        and candidate_contract.get("netlist_encoding") == "ascii"
        and candidate_contract.get("absolute_project_path_forbidden") is True,
        "IGZO candidate is parser-only and uses a repository-relative ASCII include",
    )
    ...
    add_check(
        checks,
        "paths:r11_namespace_disjoint_from_historical_failures",
        all("r11" in value for value in output_values)
        and not any(
            value in {
                r08["failure_report_path"],
                r08["failure_log_path"],
                r09["failure_report_path"],
                r09["failure_log_path"],
                r10["static_contract_report_path"],
                r10["runner_failure_report_path"],
                r10["runner_failure_log_path"],
                r10["partial_run_directory"],
            }
            for value in output_values
        ),
        "R11 reserves only its own output namespace",
    )
    add_check(
        checks,
        ...
        and config["failure_retention"]["retain_r10_failure"] is True
    ...
    add_check(
        ...
        and rules["r10_runner_must_not_be_rerun"] is True
    ...
    add_check(
        ...
        and "candidate_relative_path" in runner_source
        and "candidate_include_path = candidate_relative_path.as_posix()" in runner_source
        and "candidate_include_path.isascii()" in runner_source
        and "not candidate_relative_path.is_absolute()" in runner_source
        and "str(ROOT) not in device_syntax_text" in runner_source
        and 'f\'.include "{candidate}"' not in runner_source
        and "m01_xyce_build_preflight_r07" not in runner_source,
        "runner parses fixed-column .prn and emits a relative ASCII include in R11 paths",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        and re.search(
            r"^(?:from\s+run_m01_xyce_build_preflight_r11\s+import|import\s+run_m01_xyce_build_preflight_r11\b)",
            checker_source,
            re.MULTILINE,
        )
        is None,
    ...
    add_check(
        checks,
        "source:contract_checker_no_process",
        re.search(
        ...
        is None,
        "R11 static contract checker is read-only and process-free",
    )
    add_check(
        checks,
        "source:r11_namespace_isolated",
        all("m01_xyce_build_preflight_r10" not in source for source in (runner_source, checker_source, common_source))
        and "M01_XYCE_BUILD_PREFLIGHT_R11" in runner_source
        and "M01_XYCE_BUILD_PREFLIGHT_R11" in checker_source,
        "R11 execution sources use config-driven R10 bindings and only R11 output names",
    )
    add_check(
        checks,
        "source:makefile_r11_targets",
        "m01-xyce-build-preflight-r11-contract-check:" in (ROOT / "Makefile").read_text(encoding="utf-8")
        and "m01-xyce-build-preflight-r11:" in (ROOT / "Makefile").read_text(encoding="utf-8")
        and "m01-xyce-build-preflight-r11-check:" in (ROOT / "Makefile").read_text(encoding="utf-8"),
        "contract, runner and independent-check targets are registered",
    )
    add_check(
        checks,
        "project:next_scope_r11_contract",
        ...
        and "do not rerun R10" in project.get("tcad_track", {}).get("next_scope", ""),
        project.get("tcad_track", {}).get("next_scope", ""),
    )
    add_check(
        checks,
        "project:evidence_boundary_r11",
        "m01_xyce_build_preflight_r11_contract_boundary" in project.get("tcad_track", {})
        and "not parser execution" in project["tcad_track"]["m01_xyce_build_preflight_r11_contract_boundary"],
        "project boundary records R11 as tool/output contract only",
    )
    add_check(
        checks,
        "registry:static_report_is_only_future_artifact",
        machine.get("contract_report_path") == "results/reports/m01_xyce_build_preflight_contract_r11.json"
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R11 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R11_CONTRACT_{report['status']} "
