#!/usr/bin/env python3
"""Independently verify persisted C00 R01 evidence without a simulator process."""
...
REPORT_PATH = ROOT / "results" / "reports" / "c00_active_load_inverter_r01.json"
CHECK_PATH = ROOT / "results" / "reports" / "c00_active_load_inverter_r01_check.json"
...
def generate_netlist(config: dict[str, Any], route: str, analysis: str) -> str:
    ...
    sweep = config["sweep_contract"]
    lines = [f"* C00 R01 {route} IGZO-only active-load inverter {'DC' if analysis == 'dc' else 'transient'}", config["netlist_contract"]["candidate_include"], ".option numdgt=17"]
    if analysis == "dc":
...
def check() -> int:
    ...
    machine = c00["active_load_inverter_r01"]
    ...
    add_check(checks, "git:head_and_origin_match_runner_commit", head == origin == machine.get("runner_pass_commit"), f"head={head} origin={origin}")
    ...
    add_check(checks, "independence:no_subprocess_common_or_runner_import", not {"subprocess", "c00_active_load_inverter_r01_common", "run_c00_active_load_inverter_r01"} & imported_modules, "standard-library independent implementation")
    ...
    scope_ok = all(text.count("XLD") == text.count("XDR") and "sno" not in text.lower() and "hzo" not in text.lower() and "ferroelectric" not in text.lower() for text in expected_netlists.values())
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R01_CHECK_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={CHECK_PATH}")
