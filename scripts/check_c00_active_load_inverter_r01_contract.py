#!/usr/bin/env python3
"""Check the C00 R01 inverter contract without creating a netlist or process."""
...
from c00_active_load_inverter_r01_common import (
    anchor_ids,
    dc_cases,
    generate_dc_netlist,
...
CONFIG_PATH = ROOT / "config" / "c00_active_load_inverter_r01.json"
COMMON_PATH = ROOT / "scripts" / "c00_active_load_inverter_r01_common.py"
RUNNER_PATH = ROOT / "scripts" / "run_c00_active_load_inverter_r01.py"
INDEPENDENT_PATH = ROOT / "scripts" / "check_c00_active_load_inverter_r01.py"
...
def check_contract() -> int:
    ...
    machine = c00.get("active_load_inverter_r01", {})
    ...
    add_check(
        checks,
        "identity:c00_active_load_inverter_r01",
        config.get("project_id") == "DG-IGZO-TFT-PDK"
        and config.get("stage_id") == "C00"
        and config.get("contract_id") == "C00_ACTIVE_LOAD_INVERTER_R01"
    ...
    add_check(checks, "source:independent_has_no_process_or_runner_import", not imports_module(independent_tree, "subprocess") and subprocess_calls(independent_tree) == 0 and not imports_module(independent_tree, "c00_active_load_inverter_r01_common") and not imports_module(independent_tree, "run_c00_active_load_inverter_r01"), "independent checker duplicates regeneration and extraction")
    ...
    add_check(checks, "netlist:forbidden_scopes_absent", all(token not in text.lower() for token in forbidden for text in (ng_dc, xy_dc, ng_tran, xy_tran)), f"forbidden={len(forbidden)}")
    ...
    add_check(checks, "outputs:unique_versioned_paths", len(output_values) == len(set(output_values)) and all("r01" in value or "c00_active_load_inverter_r01" in value for value in output_values), f"paths={len(output_values)}")
    ...
    add_check(
        ...
        machine.get("expected_contract_check_count") == 48
    ...
    add_check(
        checks,
        "registry:44_36_28",
        config["registered_checks"] == {"static_contract": 48, "runner": 36, "independent": 29},
    ...
    print(f"C00_ACTIVE_LOAD_INVERTER_R01_CONTRACT_{status} checks={passed}/{EXPECTED_CHECK_COUNT} report={report_path}")
