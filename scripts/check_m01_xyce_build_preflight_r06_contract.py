#!/usr/bin/env python3
"""Check the R06 Xyce recovery contract without build or simulator execution."""
...
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r06.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r06.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r06.py"
COMMON_PATH = ROOT / "scripts" / "m01_xyce_r06_common.py"
...
def check_contract() -> dict[str, Any]:
    if REPORT_PATH.exists():
        raise RuntimeError(f"Refusing to overwrite existing R06 contract report: {REPORT_PATH}")
    ...
    add_check(
        ...
        "R06 generator prefix is absent before execution",
    )
    ...
    add_check(
        ...
        "only complete R05 SuiteSparse and Trilinos prefixes are reusable",
    )
    ...
    add_check(
        checks,
        "build:unique_external_r06_roots_absent",
        len(new_roots) == len(set(new_roots))
        and all(str(path).startswith("/home/reachgao/.local/") for path in new_roots)
        and all("r06" in str(path) for path in new_roots)
        and all(not path.exists() for path in new_roots),
        "all R06 build and install roots are unique and absent",
    )
    add_check(
        checks,
        "build:r06_roots_are_isolated_from_r05_partial_xyce",
        ...
        "R05 partial Xyce build and install roots are never reused",
    )
    ...
    add_check(
        ...
        and tools["bison"].endswith("/m01-generator-toolchain-r06/bin/bison")
        and tools["flex"].endswith("/m01-generator-toolchain-r06/bin/flex"),
        "serial Xyce configure uses the R06 generator prefix",
    )
    add_check(
        checks,
    ...
    add_check(
        ...
        and all("m01_xyce_build_preflight_r06" in str(path) for path in generator_smoke_paths)
        and all(not path.exists() for path in generator_smoke_paths),
        "minimal Bison/Flex smoke inputs and generated C outputs are future R06 artifacts",
    )
    ...
    add_check(
        checks,
        "outputs:exclusive_unique_r06_namespace",
        ...
        and all("r06" in value for value in output_values)
        and all(not path.exists() for path in future_outputs),
        "R06 artifacts are unique, future and append-only",
    )
    ...
    add_check(
        ...
        "R06 evidence remains tool-only",
    )
    add_check(
        checks,
    ...
    add_check(
        checks,
        "project:next_scope_is_r06_contract",
    ...
    add_check(
        checks,
        "experiment:r06_is_planned_and_r05_failed",
        machine["status"] == "contract_planned"
        and machine["revision"] == 6
        ...
        and machine["implementation_project_check_history"]["failed_report_sha256"]
        == sha256(ROOT / machine["implementation_project_check_history"]["failed_report_path"])
        and machine["implementation_project_check_history"]["failed_checks_passed"] == 655
        and machine["implementation_project_check_history"]["failed_checks_failed"] == 1
        and machine["implementation_project_check_history"]["failure_category"]
        == "contract_checker_source_import_literal_false_positive"
        and machine["implementation_project_check_history"]["fixed_check_status"] == "PASS"
        and machine["implementation_project_check_history"]["fixed_checks_passed"] == 657
        and machine["implementation_project_check_history"]["failure_preserved"] is True
        and m01["xyce_build_preflight_r05"]["status"] == "preflight_failed_build"
        and m01["xyce_build_preflight_r05"]["runner_checks_passed"] == 19,
        "R06 is unexecuted while the R05 failure remains machine-recorded",
    )
    ...
    add_check(
        checks,
        "runner:r06_dedicated_recovery_and_no_formal_execution",
        ...
        "R06 has a dedicated generator recovery runner and no formal device path",
    )
    add_check(
        checks,
        ...
        "independent checker shares only deterministic hash helpers",
    )
    makefile_source = (ROOT / "Makefile").read_text(encoding="utf-8")
    add_check(
        checks,
        "make:r06_targets_registered",
        "m01-xyce-build-preflight-r06-contract-check:" in makefile_source
    ...
    add_check(
        checks,
        "gate:formal_m01_requires_r06_independent_pass",
    ...
    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(
            f"R06 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}"
        )
    ...
    with REPORT_PATH.open("x", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print(
        f"M01_XYCE_BUILD_PREFLIGHT_R06_CONTRACT_{result['status']} "
