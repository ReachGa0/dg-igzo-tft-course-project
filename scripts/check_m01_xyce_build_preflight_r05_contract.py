#!/usr/bin/env python3
"""Check the revision-5 Xyce build/preflight contract without invoking a simulator."""
...
CONFIG_PATH = ROOT / "config" / "m01_xyce_build_preflight_r05.json"
REPORT_PATH = ROOT / "results" / "reports" / "m01_xyce_build_preflight_contract_r05.json"
RUNNER_PATH = ROOT / "scripts" / "run_m01_xyce_build_preflight_r05.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_xyce_build_preflight_r05.py"
...
def check_contract() -> dict[str, Any]:
    ...
    r05 = m01["xyce_build_preflight_r05"]
    checks: list[dict[str, str]] = []

    add_check(
        checks,
        "identity:revision_5_xyce_build_preflight",
        config.get("stage_id") == "M01"
        and config.get("preflight_id") == "M01_XYCE_BUILD_PREFLIGHT_R05"
        and config.get("revision") == 5
    ...
    r03_failure = config["r03_contract_failure_binding"]
    r03_contract_report = ROOT / r03_failure["contract_report"]
    r03_contract_json = load_json(r03_contract_report) if r03_contract_report.is_file() else {}
    add_check(
        checks,
        "binding:r03_contract_failure_preserved",
        r03_failure["must_remain_unchanged"] is True
        and r03_contract_report.is_file()
        and sha256(r03_contract_report) == r03_failure["contract_report_sha256"]
        and r03_contract_json.get("status") == "FAIL"
        and r03_contract_json.get("evidence_level") == "E0"
        and r03_contract_json.get("summary", {}).get("passed") == 21
        and r03_contract_json.get("summary", {}).get("failed") == 4
        and r03_failure["contract_checks_passed"] == 21
        and r03_failure["contract_checks_failed"] == 4,
        "R03 21/25 contract failure is immutable",
    )
    r04_failure = config["r04_contract_failure_binding"]
    r04_contract_report = ROOT / r04_failure["contract_report"]
    r04_contract_json = load_json(r04_contract_report) if r04_contract_report.is_file() else {}
    add_check(
        checks,
        "binding:r04_contract_failure_preserved",
        r04_failure["must_remain_unchanged"] is True
        and r04_contract_report.is_file()
        and sha256(r04_contract_report) == r04_failure["contract_report_sha256"]
        and r04_contract_json.get("status") == "FAIL"
        and r04_contract_json.get("evidence_level") == "E0"
        and r04_contract_json.get("summary", {}).get("passed") == 25
        and r04_contract_json.get("summary", {}).get("failed") == 1
        and r04_failure["contract_checks_passed"] == 25
        and r04_failure["contract_checks_failed"] == 1,
        "R04 25/26 contract failure is immutable",
    )
    scope = config["scope"]
    add_check(
        checks,
    ...
    add_check(
        ...
        "old transcription retained; independent archive hash is the R05 input",
    )
    ...
    add_check(
        checks,
        "build:unique_external_r05_roots",
        ...
        "R05 roots are unique, external and absent before execution",
    )
    add_check(
        checks,
    ...
    add_check(
        checks,
        "outputs:exclusive_unique_r05_namespace",
        ...
        and all("r05" in value for key, value in config["outputs"].items() if key != "contract_report"),
        "R05 run artifacts are unique and append-only",
    )
    ...
    add_check(
        ...
        "R05 evidence remains tool-only",
    )
    add_check(
        checks,
        "project:next_scope_is_r05",
        project["tcad_track"]["next_scope"].startswith("establish and commit M01 Xyce build/tool preflight revision-5")
        and m01["status"] == "preflight_failed_tool_provenance",
        project["tcad_track"]["next_scope"],
    )
    add_check(
        checks,
        "experiment:r05_is_planned_and_prior_failures_bound",
        r05["status"] == "contract_planned"
        and r05["revision"] == 5
        and r05["current_evidence"] == "E0"
        and r05["formal_run_completed"] is False
        and r05["device_netlist_invoked"] is False
        and m01["xyce_build_preflight"]["status"] == "preflight_failed_build"
        and m01["xyce_build_preflight"]["preflight_run_ordinal"] == 1
        and m01["xyce_build_preflight_r03"]["status"] == "contract_failed_checker"
        and m01["xyce_build_preflight_r03"]["contract_checks_passed"] == 21
        and m01["xyce_build_preflight_r04"]["status"] == "contract_failed_checker"
        and m01["xyce_build_preflight_r04"]["contract_checks_passed"] == 25,
        "R05 is unexecuted while R01/R03/R04 failures remain machine-recorded",
    )
    ...
    add_check(
        checks,
        "runner:r05_wrapper_and_no_formal_execution",
        "run_m01_xyce_build_preflight.py" in runner_source
        ...
        "R05 wraps the reviewed runner and injects only registered BLAS/LAPACK options",
    )
    add_check(
        checks,
        "checker:r05_independent_standard_library",
        "check_m01_xyce_build_preflight.py" in checker_source
        and "EXPECTED_CHECK_COUNT = 20" in checker_source
        and "import subprocess" not in checker_source,
        "R05 independent checker has no simulator subprocess path",
    )
    add_check(
        checks,
        "make:r05_targets_registered",
        "m01-xyce-build-preflight-r05-contract-check:" in makefile_source
    ...
    add_check(
        checks,
        "gate:formal_m01_requires_r05_independent_pass",
        ...
        and "R04" in config["evidence_boundary"],
        config["next_gate"],
    )

    if len(checks) != EXPECTED_CHECK_COUNT:
        raise RuntimeError(f"R05 contract registry mismatch: expected={EXPECTED_CHECK_COUNT} actual={len(checks)}")
    ...
    with REPORT_PATH.open("x", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print(f"M01_XYCE_BUILD_PREFLIGHT_R05_CONTRACT_{result['status']} checks={len(checks) - len(failures)}/{len(checks)} report={REPORT_PATH}")
    return result
