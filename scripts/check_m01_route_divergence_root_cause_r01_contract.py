#!/usr/bin/env python3
"""Static contract checker for the M01 route-divergence R01 probe."""
...
from m01_route_divergence_root_cause_r01_common import (
...
CONFIG_PATH = ROOT / "config" / "m01_route_divergence_root_cause_r01.json"
COMMON_PATH = ROOT / "scripts" / "m01_route_divergence_root_cause_r01_common.py"
CHECKER_PATH = ROOT / "scripts" / "check_m01_route_divergence_root_cause_r01_contract.py"
RUNNER_PATH = ROOT / "scripts" / "run_m01_route_divergence_root_cause_r01.py"
INDEPENDENT_PATH = ROOT / "scripts" / "check_m01_route_divergence_root_cause_r01.py"
...
def check() -> int:
    ...
    machine = experiment_map["M01"].get("route_divergence_root_cause_r01", {})
    ...
    implementation_failure = config["implementation_project_check_failure"]
    implementation_failure_path = ROOT / implementation_failure["path"]
    implementation_failure_report = load_json(implementation_failure_path)
    source_scan_failure = config["implementation_source_scan_failure"]
    source_scan_failure_path = ROOT / source_scan_failure["path"]
    source_scan_failure_report = load_json(source_scan_failure_path)
    ...
    add_check(
        ...
        and config.get("contract_id") == "M01_ROUTE_DIVERGENCE_ROOT_CAUSE_R01"
    ...
    add_check(
        checks,
        "binding:r02_commit_and_sources",
        ...
        f"bound={len(r02_source_pairs)} commit={binding['bound_commit'][:7]}",
    )
    report_pairs = [
    ...
    run_diag = r02_run.get("route_diagnostics", {})
    add_check(
        checks,
        "observation:r02_divergence_values",
        observed["route_agreement_claimed"] is False
        and observed["root_cause_confirmed_before_probe"] is False
        and math.isclose(run_diag.get("ngspice_current_max_a_per_cm", -1.0), observed["ngspice_current_max_a_per_cm"], rel_tol=0.0, abs_tol=0.0)
        and math.isclose(run_diag.get("xyce_current_max_a_per_cm", -1.0), observed["xyce_current_max_a_per_cm"], rel_tol=0.0, abs_tol=0.0)
        and math.isclose(run_diag.get("maximum_log_difference_dec", -1.0), observed["maximum_log_difference_dec"], rel_tol=0.0, abs_tol=0.0),
        f"max_log={observed['maximum_log_difference_dec']:.6g}",
    )
    ...
    portable_round_trip = (
        ...
        .replace("RCA_Q", "M00_Q")
        .replace("BPORT", "BIDS")
        .replace("IGZO_DG_PORTABLE_PROBE_R01", "IGZO_DG_BEHAVIORAL_R02")
    )
    add_check(
        checks,
        ...
        and "IGZO_DG_PORTABLE_PROBE_R01" in portable
    ...
    expected_xy = [
        ...
        "-o", "results/compact/m01_route_divergence_root_cause_r01/xyce_probe",
    ...
    source_paths = [
        ...
        implementation_failure["path"],
        source_scan_failure["path"],
    ]
    add_check(
        checks,
        ...
        and implementation_failure_path.is_file()
        ...
        and implementation_failure["simulator_processes_invoked"] == 0
        and source_scan_failure_path.is_file()
        ...
        and source_scan_failure["simulator_processes_invoked"] == 0
    ...
    payload = {
        ...
        "next_gate": "Commit and push this static result before the one permitted two-process minimal root-cause probe.",
    }
    write_json(report_path, payload)
    print(f"M01_ROUTE_DIVERGENCE_R01_CONTRACT_{payload['status']} checks={passed}/{EXPECTED_CHECK_COUNT} report={report_path}")
